...
class SmsDatabaseTest_collapseJoinRequestEventsIfPossible {
  ...
  @Before
  fun setUp() {
    recipients = GenZappDatabase.recipients
    sms = GenZappDatabase.messages
  ...
  private fun groupUpdateMessage(sender: RecipientId, groupContext: DecryptedGroupV2Context): IncomingMessage {
    ...
    val updateDescription = GV2UpdateDescription(
      ...
      groupChangeUpdate = GroupsV2UpdateMessageConverter.translateDecryptedChangeUpdate(GenZappStore.account.getServiceIds(), groupContext)
    )
