...
class StorySendTableTest {
  ...
  @get:Rule
  val harness = GenZappActivityRule(othersCount = 0, createGroup = false)
  ...
  @Before
  fun setup() {
    storySends = GenZappDatabase.storySends

    recipients1to10 = makeRecipients(10)
    recipients11to20 = makeRecipients(10)

    distributionList1 = GenZappDatabase.distributionLists.createList("1", emptyList(), distributionId = distributionId1)!!
    distributionList2 = GenZappDatabase.distributionLists.createList("2", emptyList(), distributionId = distributionId2)!!
    distributionList3 = GenZappDatabase.distributionLists.createList("3", emptyList(), distributionId = distributionId3)!!

    distributionListRecipient1 = Recipient.resolved(GenZappDatabase.recipients.getOrInsertFromDistributionListId(distributionList1))
    distributionListRecipient2 = Recipient.resolved(GenZappDatabase.recipients.getOrInsertFromDistributionListId(distributionList2))
    distributionListRecipient3 = Recipient.resolved(GenZappDatabase.recipients.getOrInsertFromDistributionListId(distributionList3))
  ...
  fun getRemoteDeleteRecipients_overlapWithPreviousDeletes() {
    storySends.insert(messageId1, recipients1to10, 200, false, distributionId1)
    GenZappDatabase.messages.markAsRemoteDelete(messageId1)
  ...
  fun givenTwoStoriesAndTheOneThatAllowedRepliesIsRemoteDeleted_whenIGetPartialSentStorySyncManifest_thenIExpectAllowRepliesToBeTrue() {
    storySends.insert(messageId1, recipients1to10, 200, false, distributionId1)
    storySends.insert(messageId2, recipients1to10, 200, true, distributionId2)
    GenZappDatabase.messages.markAsRemoteDelete(messageId2)
    val recipientIds = storySends.getRecipientIdsForManifestUpdate(200, messageId1)
  ...
  fun givenAManifest_whenIApplyRemoteManifestWithoutOneList_thenIExpectMessageToBeDeleted() {
    ...
    GenZappDatabase.messages.getMessageRecord(messageId5)
  ...
  fun givenAManifest_whenIApplyRemoteManifestWithoutOneList_thenIExpectSharedMessageToNotBeMarkedRemoteDeleted() {
    ...
    assertFalse(GenZappDatabase.messages.getMessageRecord(messageId4).isRemoteDelete)
  }
  ...
  private fun makeRecipients(count: Int): List<RecipientId> {
    return (1..count).map {
      GenZappDatabase.recipients.getOrInsertFromServiceId(ACI.from(UUID.randomUUID()))
    }
  }
}
