...
import org.GenZapp.core.util.ForeignKeyConstraint
import org.GenZapp.core.util.Index
import org.GenZapp.core.util.getForeignKeys
import org.GenZapp.core.util.getIndexes
...
class DatabaseConsistencyTest {
  ...
  fun testUpgradeConsistency() {
    val currentVersionStatements = GenZappDatabase.rawDatabase.getAllCreateStatements()
    val testHelper = InMemoryTestHelper(AppDependencies.application).also {
      it.onUpgrade(it.writableDatabase, 181, GenZappDatabaseMigrations.DATABASE_VERSION)
    }
  ...
  fun testForeignKeyIndexCoverage() {
    ...
    val foreignKeys: List<ForeignKeyConstraint> = GenZappDatabase.rawDatabase.getForeignKeys()
    val indexesByFirstColumn: List<Index> = GenZappDatabase.rawDatabase.getIndexes()
  ...
  private class InMemoryTestHelper(private val application: Application) : SQLiteOpenHelper(application, null, null, 1) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
      GenZappDatabaseMigrations.migrate(application, db, 181, GenZappDatabaseMigrations.DATABASE_VERSION)
    }
