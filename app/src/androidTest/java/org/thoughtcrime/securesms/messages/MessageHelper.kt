 ...
 */
class MessageHelper(private val harness: GenZappActivityRule, var startTime: Long = System.currentTimeMillis()) {
  ...
  init {
    ...
    every { ThreadUpdateJob.enqueue(capture(threadIdSlot)) } answers {
      GenZappDatabase.threads.update(threadIdSlot.captured, false)
    }
  }
  ...
  fun outgoingText(conversationId: RecipientId = alice, successfulSend: Boolean = true, updateMessage: ((OutgoingMessage) -> OutgoingMessage)? = null): MessageData {
    ...
    val messageId = GenZappDatabase.messages.insertMessageOutbox(message, threadId, false, null)
  ...
  fun outgoingGroupChange(): MessageData {
    ...
    val decryptedGroupV2Context = DecryptedGroupV2Context(
      ...
      groupState = GenZappDatabase.groups.getGroup(group.groupId).get().requireV2GroupProperties().decryptedGroup
    )

    val updateDescription = GV2UpdateDescription.Builder()
      .gv2ChangeDescription(decryptedGroupV2Context)
      .groupChangeUpdate(GroupsV2UpdateMessageConverter.translateDecryptedChange(GenZappStore.account.getServiceIds(), decryptedGroupV2Context))
      .build()
