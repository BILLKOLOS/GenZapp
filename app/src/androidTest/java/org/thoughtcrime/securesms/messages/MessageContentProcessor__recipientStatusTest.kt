...
class MessageContentProcessor__recipientStatusTest {
  ...
  fun syncGroupSentTextMessageWithRecipientUpdateFollowup() {
    ...
    val threadId = GenZappDatabase.threads.getThreadIdFor(groupRecipientId)!!
    val firstSyncMessages = MessageTableTestUtils.getMessages(threadId)
    val firstMessageId = firstSyncMessages[0].id
    val firstReceiptInfo = GenZappDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
    ...
    val secondReceiptInfo = GenZappDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
