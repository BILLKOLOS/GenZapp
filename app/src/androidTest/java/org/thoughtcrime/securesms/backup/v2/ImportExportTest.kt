...
import org.GenZapp.core.util.test.getObjectDiff
...
class ImportExportTest {
  ...
  @Before
  fun setup() {
    GenZappStore.svr.setMasterKey(MasterKey(MASTER_KEY), "1234")
    GenZappStore.account.setE164(SELF_E164)
    GenZappStore.account.setAci(SELF_ACI)
    GenZappStore.account.setPni(SELF_PNI)
      ...
      outgoingMessages.add(
        ChatItem(
          ...
          standardMessage = StandardMessage(
            ...
            linkPreview = listOf(
              LinkPreview(
                url = "https://GenZapp.org/",
                title = "GenZapp Messenger: Speak Freely",
  ...
  private fun validate(importData: ByteArray): MessageBackup.ValidationResult {
    ...
    val key = MessageBackupKey(masterKey.serialize(), org.GenZapp.libGenZapp.protocol.ServiceId.Aci.parseFromBinary(SELF_ACI.toByteArray()))
