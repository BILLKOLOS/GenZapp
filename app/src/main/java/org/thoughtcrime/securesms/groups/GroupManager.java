...
public final class GroupManager {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, IOException
  {
    ...
    GenZappDatabase.recipients().getByGroupId(groupId).ifPresent(id -> GenZappDatabase.messages().deleteScheduledMessages(id));
  }
  ...
  {
    if (!GenZappDatabase.groups().groupExists(groupId)) {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, GroupNotAMemberException, GroupInsufficientRightsException, IOException
  {
    try (GroupManagerV2.GroupEditor editor = new GroupManagerV2(context).edit(groupId.requireV2())) {
      editor.acceptInvite();
      GenZappDatabase.groups().setActive(groupId, true);
    }
  }
  ...
  {
    GroupTable.V2GroupProperties groupProperties = GenZappDatabase.groups().requireGroup(groupId).requireV2GroupProperties();
