...
import org.GenZapp.storageservice.protos.groups.local.DecryptedGroupChange
...
import org.whispersystems.GenZappservice.api.groupsv2.DecryptedGroupUtil
import org.whispersystems.GenZappservice.api.groupsv2.GroupChangeReconstruct
import org.whispersystems.GenZappservice.api.groupsv2.GroupHistoryPage
import org.whispersystems.GenZappservice.api.groupsv2.InvalidGroupStateException
import org.whispersystems.GenZappservice.api.groupsv2.NotAbleToApplyGroupV2ChangeException
import org.whispersystems.GenZappservice.api.groupsv2.ReceivedGroupSendEndorsements
...
import org.whispersystems.GenZappservice.internal.push.exceptions.GroupNotFoundException
import org.whispersystems.GenZappservice.internal.push.exceptions.NotInGroupException
...
class GroupsV2StateProcessor private constructor(
  ...
  private val groupsApi = AppDependencies.GenZappServiceAccountManager.getGroupsV2Api()
  ...
  fun forceSanityUpdateFromServer(timestamp: Long): GroupUpdateResult {
    val currentLocalState: DecryptedGroup? = GenZappDatabase.groups.getGroup(groupId).map { it.requireV2GroupProperties().decryptedGroup }.orNull()
  ...
  fun updateGroupSendEndorsements() {
    ...
    if (receivedGroupSendEndorsements != null) {
      ...
      GenZappDatabase.groups.updateGroupSendEndorsements(groupId, receivedGroupSendEndorsements)
    } else {
  ...
  fun updateLocalGroupToRevision(
    ...
    groupRecord: Optional<GroupRecord> = GenZappDatabase.groups.getGroup(groupId),
  ...
  private fun canApplyP2pChange(
    ...
    if (GenZappStore.internal.gv2IgnoreP2PChanges()) {
      ...
      if (updatedGroupState == null || updatedGroupState == remoteGroupStateDiff.previousGroupState) {
        ...
        if (currentLocalState != null && applyGroupStateDiffResult.remainingRemoteGroupChanges.isEmpty()) {
          ...
          if (endorsements != null) {
            ...
            GenZappDatabase.groups.updateGroupSendEndorsements(groupId, endorsements)
          }
        }
  ...
  private fun localIsAtLeast(groupRecord: Optional<GroupRecord>, revision: Int): Boolean {
    if (revision == LATEST || groupRecord.isEmpty || GenZappDatabase.groups.isUnknownGroup(groupRecord)) {
      return false
    }
  ...
  private fun saveGroupUpdate(
    ...
    if (updatedGroupState == null || updatedGroupState == groupStateDiff.previousGroupState) {
      ...
      if (groupSendEndorsements != null) {
        ...
        GenZappDatabase.groups.updateGroupSendEndorsements(groupId, groupSendEndorsements)
      }
  ...
  private fun saveGroupState(groupStateDiff: GroupStateDiff, updatedGroupState: DecryptedGroup, groupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    val needsAvatarFetch = if (previousGroupState == null) {
      val groupId = GenZappDatabase.groups.create(groupMasterKey, updatedGroupState, groupSendEndorsements)

      if (groupId == null) {
        Log.w(TAG, "$logPrefix Group create failed, trying to update")
        GenZappDatabase.groups.update(groupMasterKey, updatedGroupState, groupSendEndorsements)
      }
  ...
  internal class ProfileAndMessageHelper(private val aci: ACI, private val masterKey: GroupMasterKey, private val groupId: GroupId.V2) {
      ...
      if (selfAsMember != null) {
        ...
        if (addedBy != null) {
          ...
          if (addedBy.isBlocked && (previousGroupState == null || !DecryptedGroupUtil.isRequesting(previousGroupState, aci))) {
            ...
            GenZappDatabase.recipients.setProfileSharing(Recipient.externalGroupExact(groupId).id, true)
          } else {
    ...
    fun leaveGroupLocally(serviceIds: ServiceIds) {
      if (!GenZappDatabase.groups.isActive(groupId)) {
        Log.w(TAG, "Group $groupId has already been left.")

        val groupRecipient = Recipient.externalGroupExact(groupId)
        val threadId = GenZappDatabase.threads.getThreadIdFor(groupRecipient.id)

        if (threadId != null) {
          GenZappDatabase.drafts.clearDrafts(threadId)
          GenZappDatabase.threads.update(threadId, unarchive = false, allowDeletion = false)
        }
      ...
      val decryptedGroup = GenZappDatabase
        .groups
      ...
      try {
        val threadId = GenZappDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
        val id = GenZappDatabase.messages.insertMessageOutbox(leaveMessage, threadId, false, null)
        GenZappDatabase.messages.markAsSent(id, true)
        GenZappDatabase.drafts.clearDrafts(threadId)
        GenZappDatabase.threads.update(threadId, unarchive = false, allowDeletion = false)
      ...
      GenZappDatabase.groups.setActive(groupId, false)
      GenZappDatabase.groups.remove(groupId, Recipient.self().id)
    }
    ...
    fun persistLearnedProfileKeys(profileKeys: ProfileKeySet) {
      val updated = GenZappDatabase.recipients.persistProfileKeySet(profileKeys)
    ...
    fun storeMessage(decryptedGroupV2Context: DecryptedGroupV2Context, timestamp: Long, serverGuid: String?) {
      ...
      val serviceIds = GenZappStore.account.getServiceIds()
      ...
      if (outgoing) {
        try {
          val recipientId = GenZappDatabase.recipients.getOrInsertFromGroupId(groupId)
          ...
          val messageId = GenZappDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, null)
        ...
        try {
          ...
          val insertResult = GenZappDatabase.messages.insertMessageInbox(groupMessage)

          if (insertResult.isPresent) {
            GenZappDatabase.threads.update(insertResult.get().threadId, unarchive = false, allowDeletion = false)
