...
class ReviewCardViewHolder extends RecyclerView.ViewHolder {
  ...
  private final Runnable                        onGenZappConnectionClicked;
                              ...
                              @NonNull Callbacks callbacks)
  {
    ...
    onGenZappConnectionClicked = callbacks::onGenZappConnectionClicked;
  }
  ...
  private List<ReviewTextRow> getNonContactSublines(@NonNull ReviewCard reviewCard) {
    ...
    if (reviewCard.getReviewRecipient().isProfileSharing() && !reviewCard.getReviewRecipient().isSelf()) {
      reviewTextRows.add(ReviewTextRow.GenZapp_CONNECTION);
    }
  ...
  private void presentReviewTextRows(@NonNull List<ReviewTextRow> reviewTextRows, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    for (int i = 0; i < Math.min(reviewTextRows.size(), subtextGroups.size()); i++) {
      ...
      switch (row) {
        case GenZapp_CONNECTION -> presentGenZappConnection(group.first, group.second, context, reviewCard);
  ...
  private void presentGenZappConnection(@NonNull TextView line, @NonNull ImageView icon, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    SpannableStringBuilder builder = new SpannableStringBuilder(context.getString(R.string.AboutSheet__GenZapp_connection));
    ...
    line.setOnClickListener(v -> onGenZappConnectionClicked.run());
  }
  ...
  interface Callbacks {
    ...
    void onGenZappConnectionClicked();
  }
