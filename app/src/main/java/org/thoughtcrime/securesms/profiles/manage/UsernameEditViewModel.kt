...
internal class UsernameEditViewModel private constructor(private val mode: UsernameEditMode) : ViewModel() {
  ...
  private val uiState: RxStore<State> = RxStore(
    defaultValue = State(
      ...
      usernameState = GenZappStore.account.username?.let { UsernameState.Set(Username(it)) } ?: UsernameState.NoUsername
    ),
  ...
  private val stateMachineStore = RxStore<UsernameEditStateMachine.State>(
    defaultValue = UsernameEditStateMachine.NoUserEntry(
      nickname = GenZappStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "",
      discriminator = GenZappStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "",
  ...
  init {
    ...
    if (mode == UsernameEditMode.RECOVERY) {
      onNicknameUpdated(GenZappStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "")
      onDiscriminatorUpdated(GenZappStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "")
    }
  }
  ...
  fun onNicknameUpdated(nickname: String) {
    uiState.update { state: State ->
      if (nickname.isBlank() && GenZappStore.account.username != null) {
        return@update State(
  ...
  fun onDiscriminatorUpdated(discriminator: String) {
    uiState.update { state: State ->
      if (discriminator.isBlank() && GenZappStore.account.username != null) {
        return@update State(
          buttonState = ButtonState.DELETE,
          usernameStatus = UsernameStatus.NONE,
          usernameState = UsernameState.NoUsername
  ...
  fun onUsernameSkipped() {
    GenZappStore.uiHints.markHasSetOrSkippedUsernameCreation()
  ...
  fun isSameUsernameRecovery(): Boolean {
    ...
    return mode == UsernameEditMode.RECOVERY &&
      ...
      usernameState.requireUsername().username.lowercase() == GenZappStore.account.username?.lowercase()
  }
  ...
  fun onUsernameSubmitted(userConfirmedResetOk: Boolean) {
    ...
    if (!isCaseChange && GenZappStore.account.username.isNotNullOrBlank() && !userConfirmedResetOk) {
    ...
    if (usernameState.requireUsername().username == GenZappStore.account.username && mode != UsernameEditMode.RECOVERY) {
  ...
  private fun isCaseChange(state: UsernameEditStateMachine.State): Boolean {
    ...
    val oldLower = GenZappStore.account.username?.split(Usernames.DELIMITER)?.firstOrNull()?.lowercase()
  ...
  private fun onUsernameStateUpdateDebounced(state: UsernameEditStateMachine.State) {
    ...
    if (isCaseChange(state)) {
      val discriminator = GenZappStore.account.username?.split(Usernames.DELIMITER)?.lastOrNull() ?: error("Unexpected case change, no discriminator!")
      uiState.update {
        State(
