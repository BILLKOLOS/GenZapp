...
class UsernameShareBottomSheet : DSLSettingsBottomSheetFragment() {
  ...
  private fun getConfiguration(recipient: Recipient): DSLConfiguration {
    return configure {
      noPadTextPref(
        title = DSLSettingsText.from(
          ...
          DSLSettingsText.ColorModifier(
            ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnSurfaceVariant)
          )
        )
      )
      ...
      customPref(
        CopyButton.Model(
          text = getString(R.string.GenZapp_me_username_url, Base64.encodeUrlSafeWithoutPadding(Username.hash(username))),
          onClick = {
            copyToClipboard(it)
          }
        )
