...
import org.GenZapp.core.util.Result
import org.GenZapp.core.util.Result.Companion.failure
import org.GenZapp.core.util.Result.Companion.success
...
import org.whispersystems.GenZappservice.api.push.exceptions.RateLimitException
import org.whispersystems.GenZappservice.api.push.exceptions.UsernameIsNotAssociatedWithAnAccountException
import org.whispersystems.GenZappservice.api.push.exceptions.UsernameIsNotReservedException
import org.whispersystems.GenZappservice.api.push.exceptions.UsernameMalformedException
import org.whispersystems.GenZappservice.api.push.exceptions.UsernameTakenException
import org.whispersystems.GenZappservice.api.util.Usernames
 ...
 * https://GenZapp.me/#eu/<32 bytes of entropy><16 bytes of serverId uuid>
 *
...
object UsernameRepository {
  ...
  private val URL_REGEX = """(https://)?GenZapp.me/?#eu/([a-zA-Z0-9+\-_/]+)""".toRegex()

  private const val BASE_URL = "https://GenZapp.me/#eu/"
  private const val USERNAME_SYNC_ERROR_THRESHOLD = 3

  private val accountManager: GenZappServiceAccountManager get() = AppDependencies.GenZappServiceAccountManager
  ...
  fun reclaimUsernameIfNecessary(): UsernameReclaimResult {
    ...
    when (result) {
      ...
      UsernameReclaimResult.PERMANENT_ERROR -> {
        ...
        GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED
        GenZappStore.misc.needsUsernameRestore = false
      }
  ...
  fun createOrResetUsernameLink(): Single<UsernameLinkResetResult> {
    ...
    val usernameString = GenZappStore.account.username
    ...
    return Single
      .fromCallable {
        try {
          GenZappStore.account.usernameLink = null

          Log.d(TAG, "[createOrResetUsernameLink] Creating username link...")
          val components = accountManager.createUsernameLink(username)
          GenZappStore.account.usernameLink = components

          if (GenZappStore.account.usernameSyncState == AccountValues.UsernameSyncState.LINK_CORRUPTED) {
            GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
            GenZappStore.account.usernameSyncErrorCount = 0
          }
  ...
  fun fetchAciForUsername(username: String): Single<UsernameAciFetchResult> {
    return Single.fromCallable {
      try {
        val aci: ACI = AppDependencies.GenZappServiceAccountManager.getAciByUsername(Username(username))
  ...
  fun onUsernameConsistencyValidated() {
    ...
    if (GenZappStore.account.usernameSyncErrorCount > 0) {
      Log.i(TAG, "Username consistency validated. There were previously ${GenZappStore.account.usernameSyncErrorCount} error(s).")
      GenZappStore.account.usernameSyncErrorCount = 0
    }
  }
  ...
  fun onUsernameMismatchDetected() {
    GenZappStore.account.usernameSyncErrorCount++

    if (GenZappStore.account.usernameSyncErrorCount >= USERNAME_SYNC_ERROR_THRESHOLD) {
      Log.w(TAG, "We've now seen ${GenZappStore.account.usernameSyncErrorCount} mismatches in a row. Marking username and link as corrupted.")
      GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED
      GenZappStore.account.usernameSyncErrorCount = 0
    } else {
      Log.w(TAG, "Username mismatch reported. At ${GenZappStore.account.usernameSyncErrorCount} / $USERNAME_SYNC_ERROR_THRESHOLD tries.")
    }
  }
  ...
  fun onUsernameLinkMismatchDetected() {
    ...
    if (GenZappStore.account.usernameSyncErrorCount >= USERNAME_SYNC_ERROR_THRESHOLD) {
      Log.w(TAG, "We've now seen ${GenZappStore.account.usernameSyncErrorCount} mismatches in a row. Marking link as corrupted.")
      GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.LINK_CORRUPTED
      GenZappStore.account.usernameLink = null
      GenZappStore.account.usernameSyncErrorCount = 0
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
      Log.w(TAG, "Link mismatch reported. At ${GenZappStore.account.usernameSyncErrorCount} / $USERNAME_SYNC_ERROR_THRESHOLD tries.")
    }
  }
  ...
  private fun updateUsernameDisplayForCurrentLinkInternal(updatedUsername: Username): UsernameSetResult {
    ...
    return try {
      val oldUsernameLink = GenZappStore.account.usernameLink ?: return UsernameSetResult.USERNAME_INVALID
      ...
      GenZappStore.account.username = updatedUsername.username
      GenZappStore.account.usernameLink = usernameLinkComponents
      GenZappDatabase.recipients.setUsername(Recipient.self().id, updatedUsername.username)
      GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
      GenZappStore.account.usernameSyncErrorCount = 0
  ...
  private fun confirmUsernameAndCreateNewLinkInternal(username: Username): UsernameSetResult {
    ...
    return try {
      ...
      GenZappStore.account.username = username.username
      GenZappStore.account.usernameLink = linkComponents
      GenZappDatabase.recipients.setUsername(Recipient.self().id, username.username)
      GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
      GenZappStore.account.usernameSyncErrorCount = 0
  ...
  private fun deleteUsernameInternal(): UsernameDeleteResult {
    ...
    return try {
      ...
      GenZappDatabase.recipients.setUsername(Recipient.self().id, null)
      GenZappStore.account.username = null
      GenZappStore.account.usernameLink = null
      GenZappStore.account.usernameSyncState = AccountValues.UsernameSyncState.IN_SYNC
      GenZappStore.account.usernameSyncErrorCount = 0
      GenZappDatabase.recipients.markNeedsSync(Recipient.self().id)
