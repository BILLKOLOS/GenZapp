...
class MediaPreviewV2Fragment : LoggingFragment(R.layout.fragment_media_preview_v2), MediaPreviewFragment.Events {
  ...
  private fun initializeToolbar(toolbar: MaterialToolbar) {
    ...
    toolbar.setTitleTextAppearance(requireContext(), R.style.GenZapp_Text_TitleMedium)
    toolbar.setSubtitleTextAppearance(requireContext(), R.style.GenZapp_Text_BodyMedium)
  ...
  private fun getSubTitleText(mediaRecord: MediaTable.MediaRecord): CharSequence {
    ...
    val onSurfaceColor = ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnSurface)
  ...
  fun performSaveToDisk(mediaItem: MediaTable.MediaRecord) {
    ...
    val uri = attachment?.uri
    if (attachment != null && uri != null) {
      saveTask.executeOnExecutor(GenZappExecutors.BOUNDED_IO, SaveAttachmentTask.Attachment(uri, attachment.contentType, saveDate, null))
    }
  }
  ...
  private fun canRemotelyDelete(attachment: DatabaseAttachment): Boolean {
    ...
    val attachmentCount = GenZappDatabase.attachments.getAttachmentsForMessage(mmsId).size
    return attachmentCount <= 1 && MessageConstraintsUtil.isValidRemoteDeleteSend(listOf(GenZappDatabase.messages.getMessageRecord(mmsId)), System.currentTimeMillis())
  }
