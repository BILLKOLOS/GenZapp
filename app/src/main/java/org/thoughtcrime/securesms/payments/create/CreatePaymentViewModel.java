...
import org.whispersystems.GenZappservice.api.payments.FormatterOptions;
...
public class CreatePaymentViewModel extends ViewModel {
  ...
  private CreatePaymentViewModel(@NonNull PayeeParcelable payee, @Nullable CharSequence note) {
    this.payee            = payee;
    this.spendableBalance = Transformations.map(GenZappStore.payments().liveMobileCoinBalance(), Balance::getTransferableAmount);
    ...
    this.enclaveFailure   = LiveDataUtil.mapDistinct(GenZappStore.payments().enclaveFailure(), isFailure -> isFailure);
    ...
    LiveData<Optional<CurrencyExchange.ExchangeRate>> liveExchangeRate = LiveDataUtil.mapAsync(GenZappStore.payments().liveCurrentCurrency(),
                                                                                               currency -> {
                                                                                                 try {
  ...
  private @NonNull InputState updateAmount(@NonNull Context context, @NonNull InputState inputState, @NonNull AmountKeyboardGlyph glyph) {
    switch (inputState.getInputTarget()) {
      case FIAT_MONEY:
        return updateFiatAmount(context, inputState, glyph, GenZappStore.payments().currentCurrency());
      case MONEY:
