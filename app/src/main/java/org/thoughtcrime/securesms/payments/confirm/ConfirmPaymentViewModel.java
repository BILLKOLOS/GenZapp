...
final class ConfirmPaymentViewModel extends ViewModel {
                          ...
                          @NonNull ConfirmPaymentRepository confirmPaymentRepository)
  {
    ...
    this.store.update(GenZappStore.payments().liveMobileCoinBalance(), (balance, state) -> state.updateBalance(balance.getFullAmount()));
    ...
    LiveData<ConfirmPaymentState.Status> timeoutGenZapp  = Transformations.switchMap(statusLiveData,
                                                                                    s -> {
    ...
    this.store.update(timeoutGenZapp, this::handleTimeout);
  }
