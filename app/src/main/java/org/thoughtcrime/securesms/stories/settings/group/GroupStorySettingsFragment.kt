...
class GroupStorySettingsFragment : DSLSettingsFragment(menuId = R.menu.story_group_menu) {
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    ...
    if (item.itemId == R.id.action_overflow) {
      GenZappContextMenu.Builder(toolbar, requireView() as ViewGroup)
        .preferredHorizontalPosition(GenZappContextMenu.HorizontalPosition.END)
        .preferredVerticalPosition(GenZappContextMenu.VerticalPosition.BELOW)
        .offsetX(16.dp)
        .offsetY((-4).dp)
        .show(
          listOf(
    ...
    return configure {
      ...
      textPref(
        title = DSLSettingsText.from(
          ...
          DSLSettingsText.TextAppearanceModifier(R.style.GenZapp_Text_BodyMedium),
          DSLSettingsText.ColorModifier(ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnSurfaceVariant))
        )
      )
      ...
      clickPref(
        title = DSLSettingsText.from(
          ...
          DSLSettingsText.ColorModifier(ContextCompat.getColor(requireContext(), R.color.GenZapp_colorError))
        ),
        onClick = {
