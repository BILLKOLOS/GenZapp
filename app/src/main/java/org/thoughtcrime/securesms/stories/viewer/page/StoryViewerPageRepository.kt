...
import org.GenZapp.core.util.BreakIteratorCompat
...
open class StoryViewerPageRepository(context: Context, private val storyViewStateCache: StoryViewStateCache) {
    ...
    return Observable.create { emitter ->
      ...
      fun refresh() {
        val stories = if (recipient.isMyStory) {
          GenZappDatabase.messages.getAllOutgoingStories(false, 100)
        } else if (isOutgoingOnly) {
          GenZappDatabase.messages.getOutgoingStoriesTo(recipientId)
        } else {
          GenZappDatabase.messages.getAllStoriesFor(recipientId, 100)
        }
  ...
  private fun getStoryPostFromRecord(recipientId: RecipientId, originalRecord: MessageRecord): Observable<StoryPost> {
    return Observable.create { emitter ->
      fun refresh(record: MessageRecord) {
        val recipient = Recipient.resolved(recipientId)
        val viewedCount = GenZappDatabase.groupReceipts.getGroupReceiptInfo(record.id).filter { it.status == GroupReceiptTable.STATUS_VIEWED }.size
        val story = StoryPost(
          id = record.id,
          ...
          replyCount = GenZappDatabase.messages.getNumberOfStoryReplies(record.id),
      ...
      val messageUpdateObserver = DatabaseObserver.MessageObserver {
        if (it.id == recordId) {
          try {
            val messageRecord = GenZappDatabase.messages.getMessageRecord(recordId)
      ...
      val conversationObserver = DatabaseObserver.Observer {
        try {
          refresh(GenZappDatabase.messages.getMessageRecord(recordId))
        } catch (e: NoSuchMessageException) {
  ...
  fun hideStory(recipientId: RecipientId): Completable {
    return Completable.fromAction {
      GenZappDatabase.recipients.setHideStory(recipientId, true)
  ...
  fun markViewed(storyPost: StoryPost) {
    if (!storyPost.conversationMessage.messageRecord.isOutgoing) {
      GenZappExecutors.SERIAL.execute {
        val markedMessageInfo = GenZappDatabase.messages.setIncomingMessageViewed(storyPost.id)
        if (markedMessageInfo != null) {
          AppDependencies.databaseObserver.notifyConversationListListeners()

          if (storyPost.sender.isReleaseNotes) {
            GenZappStore.story.userHasViewedOnboardingStory = true
            ...
            GenZappDatabase.recipients.updateLastStoryViewTimestamp(recipientId)
