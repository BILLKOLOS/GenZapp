...
object Stories {
  ...
  fun isFeatureEnabled(): Boolean {
    return !GenZappStore.story.isFeatureDisabled
  }
  ...
  fun getRecipientsToSendTo(messageId: Long, sentTimestamp: Long, allowsReplies: Boolean): SendData {
    val recipientIds: List<RecipientId> = GenZappDatabase.storySends.getRecipientsToSendTo(messageId, sentTimestamp, allowsReplies)
  ...
  fun onStorySettingsChanged(distributionListId: DistributionListId) {
    val recipientId = GenZappDatabase.distributionLists.getRecipientId(distributionListId) ?: error("Cannot find recipient id for distribution list.")
    onStorySettingsChanged(recipientId)
  }

  @WorkerThread
  fun onStorySettingsChanged(storyRecipientId: RecipientId) {
    GenZappDatabase.recipients.markNeedsSync(storyRecipientId)
  ...
  fun enqueueNextStoriesForDownload(recipientId: RecipientId, force: Boolean = false, limit: Int) {
    ...
    GenZappDatabase.messages.getUnreadStories(recipientId, limit).use { reader ->
      reader.forEach {
  ...
  fun enqueueAttachmentsFromStoryForDownloadSync(record: MmsMessageRecord, ignoreAutoDownloadConstraints: Boolean) {
    GenZappDatabase.attachments.getAttachmentsForMessage(record.id).filterNot { it.isSticker }.forEach {
