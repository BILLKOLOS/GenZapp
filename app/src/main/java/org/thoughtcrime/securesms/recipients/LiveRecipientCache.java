...
public final class LiveRecipientCache {
  ...
  public LiveRecipientCache(@NonNull Context context) {
    this(context, new FilteredExecutor(GenZappExecutors.newCachedBoundedExecutor("GenZapp-recipients", ThreadUtil.PRIORITY_UI_BLOCKING_THREAD, 1, 4, 15), () -> !GenZappDatabase.inTransaction()));
  }
  ...
  @NonNull Recipient getSelf() {
    ...
    if (selfId == null) {
      ...
      String localE164 = GenZappStore.account().getE164();
  ...
  public void warmUp() {
    ...
    GenZappExecutors.BOUNDED.execute(() -> {
      ...
      if (GenZappStore.registration().isRegistrationComplete() && GenZappStore.account().getAci() != null) {
        try (Cursor cursor = GenZappDatabase.recipients().getNonGroupContacts(false)) {
          int count = 0;
