...
public class RecipientUtil {
  ...
  public static @NonNull ServiceId getOrFetchServiceId(@NonNull Context context, @NonNull Recipient recipient) throws IOException {
    return toGenZappServiceAddress(context, recipient).getServiceId();
  }

  /**
   * This method will do it's best to craft a fully-populated {@link GenZappServiceAddress} based on
   ...
   */
  @WorkerThread
  public static @NonNull GenZappServiceAddress toGenZappServiceAddress(@NonNull Context context, @NonNull Recipient recipient)
      throws IOException
  {
    ...
    if (recipient.getHasServiceId()) {
      return new GenZappServiceAddress(recipient.requireServiceId(), Optional.ofNullable(recipient.resolve().getE164().orElse(null)));
    } else {
      throw new NotFoundException(recipient.getId() + " is not registered!");
    }
  }

  public static @NonNull List<GenZappServiceAddress> toGenZappServiceAddresses(@NonNull Context context, @NonNull List<RecipientId> recipients)
      throws IOException
  {
    return toGenZappServiceAddressesFromResolved(context, Recipient.resolvedList(recipients));
  }

  public static @NonNull List<GenZappServiceAddress> toGenZappServiceAddressesFromResolved(@NonNull Context context, @NonNull List<Recipient> recipients)
      throws IOException
  {
    ...
    return Stream.of(recipients)
                 ...
                 .map(r -> new GenZappServiceAddress(r.requireServiceId(), r.getE164().orElse(null)))
                 .toList();
  }
      ...
      throws GroupChangeBusyException, IOException, GroupChangeFailedException
  {
    ...
    GenZappDatabase.recipients().setBlocked(recipient.getId(), true);

    if (recipient.isSystemContact() || recipient.isProfileSharing() || isProfileSharedViaGroup(recipient)) {
      GenZappDatabase.recipients().setProfileSharing(recipient.getId(), false);
  ...
  public static void unblock(@NonNull Recipient recipient) {
    ...
    GenZappDatabase.recipients().setBlocked(recipient.getId(), false);
    GenZappDatabase.recipients().setProfileSharing(recipient.getId(), true);
    AppDependencies.getJobManager().add(new MultiDeviceBlockedUpdateJob());
    StorageSyncHelper.scheduleSyncForDataChange();
  }
  ...
  public static boolean isMessageRequestAccepted(@NonNull Context context, @Nullable Recipient threadRecipient) {
    ...
    Long threadId = GenZappDatabase.threads().getThreadIdFor(threadRecipient.getId());
    return isMessageRequestAccepted(threadId, threadRecipient);
  }
  ...
  public static void shareProfileIfFirstSecureMessage(@NonNull Recipient recipient) {
    ...
    long    threadId     = GenZappDatabase.threads().getThreadIdIfExistsFor(recipient.getId());
    boolean firstMessage = GenZappDatabase.messages().getOutgoingSecureMessageCount(threadId) == 0;
  ...
  public static boolean setAndSendUniversalExpireTimerIfNecessary(@NonNull Context context, @NonNull Recipient recipient, long threadId) {
    int defaultTimer = GenZappStore.settings().getUniversalExpireTimer();
    if (defaultTimer == 0 || recipient.isGroup() || recipient.isDistributionList() || recipient.getExpiresInSeconds() != 0 || !recipient.isRegistered()) {
      return false;
    }

    if (threadId == -1 || GenZappDatabase.messages().canSetUniversalTimer(threadId)) {
      GenZappDatabase.recipients().setExpireMessages(recipient.getId(), defaultTimer);
      ...
      MessageSender.send(context, outgoingMessage, GenZappDatabase.threads().getOrCreateThreadIdFor(recipient), MessageSender.SendType.GenZapp, null, null);
      return true;
    }
    return false;
  }
  ...
  public static boolean hasSentMessageInThread(@Nullable Long threadId) {
    return threadId != null && GenZappDatabase.messages().getOutgoingSecureMessageCount(threadId) != 0;
  }
  ...
  private static boolean noSecureMessagesAndNoCallsInThread(@Nullable Long threadId) {
    ...
    return GenZappDatabase.messages().getSecureMessageCount(threadId) == 0 &&
           !GenZappDatabase.threads().hasReceivedAnyCallsSince(threadId, 0);
  }

  @WorkerThread
  private static boolean isProfileSharedViaGroup(@NonNull Recipient recipient) {
    return Stream.of(GenZappDatabase.groups().getPushGroupsContainingMember(recipient.getId()))
