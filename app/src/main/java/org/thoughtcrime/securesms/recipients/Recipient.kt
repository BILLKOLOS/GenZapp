...
class Recipient(
  ...
  /** Whether the recipient represents a group. It could be a GenZapp group or MMS group. */
  val isGroup: Boolean
  ...
  /** Whether the recipient represents a GenZapp group. */
  ...
  /** Whether the recipient represents a V1 GenZapp group. These types of groups were deprecated in 2020. */
  ...
  /** Whether the recipient represents a V2 GenZapp group. */
  ...
  val contactPhoto: ContactPhoto?
    ...
    } else if (systemContactPhoto != null && GenZappStore.settings.isPreferSystemContactPhotos) {
  ...
  /** The [RegisteredState] of this recipient. GenZapp groups/lists are always registered. */
  ...
  val wallpaper: ChatWallpaper?
    get() {
      return if (wallpaperValue != null) {
        ...
        GenZappStore.wallpaper.getWallpaper()
      }
    }
  ...
  val hasWallpaper: Boolean
    get() = wallpaperValue != null || GenZappStore.wallpaper.hasWallpaperSet()
  ...
  val chatColors: ChatColors
    get() {
      return if (chatColorsValue != null && chatColorsValue.id !is Auto) {
        ...
        val global = GenZappStore.chatColors.chatColors
  ...
  fun getGroupName(context: Context): String? {
    return if (groupIdValue != null && Util.isEmpty(groupName)) {
      ...
      context.getString(R.string.Recipient_GenZapp_call)
    } else {
      groupName
    }
  }
  ...
  companion object {
    ...
    fun distributionList(distributionListId: DistributionListId): Recipient {
      val id = GenZappDatabase.recipients.getOrInsertFromDistributionListId(distributionListId)
      return resolved(id)
    }
    ...
    fun externalUsername(serviceId: ServiceId, username: String): Recipient {
      ...
      GenZappDatabase.recipients.setUsername(recipient.id, username)
      return recipient
    }

    /**
     * Returns a fully-populated [Recipient] based off of a [GenZappServiceAddress], creating one in the database if necessary.
     */
    @JvmStatic
    @WorkerThread
    fun externalPush(GenZappServiceAddress: GenZappServiceAddress): Recipient {
      return externalPush(GenZappServiceAddress.serviceId, GenZappServiceAddress.number.orElse(null))
    }
    ...
    fun trustedPush(aci: ACI, pni: PNI?, e164: String?): Recipient {
      ...
      val recipientId = GenZappDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
    ...
    fun externalPush(serviceId: ServiceId?, e164: String?): Recipient {
      ...
      val recipientId = RecipientId.from(GenZappServiceAddress(serviceId, e164))
      val resolved = resolved(recipientId)
      ...
      if (!resolved.isRegistered && serviceId != null) {
        Log.w(TAG, "External push was locally marked unregistered. Marking as registered.")
        GenZappDatabase.recipients.markRegistered(recipientId, serviceId)
    ...
    fun externalContact(identifier: String): Recipient {
      val id: RecipientId = if (UuidUtil.isUuid(identifier)) {
        ...
        GenZappDatabase.recipients.getOrInsertFromEmail(identifier)
      } else {
        GenZappDatabase.recipients.getOrInsertFromE164(identifier)
      }
    ...
    fun externalGroupExact(groupId: GroupId): Recipient {
      return resolved(GenZappDatabase.recipients.getOrInsertFromGroupId(groupId))
    }
    ...
    fun externalPossiblyMigratedGroup(groupId: GroupId): Recipient {
      ...
      return try {
        ...
        resolved(GenZappDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId))
      }
    }
     ...
     *
     * If the identifier is a UUID of a GenZapp user, prefer using
    ...
    fun external(context: Context, identifier: String): Recipient {
      ...
      val id: RecipientId = if (serviceId != null) {
        ...
        GenZappDatabase.recipients.getOrInsertFromGroupId(GroupId.parseOrThrow(identifier))
      } else if (NumberUtil.isValidEmail(identifier)) {
        GenZappDatabase.recipients.getOrInsertFromEmail(identifier)
        ...
        GenZappDatabase.recipients.getOrInsertFromE164(e164)
      }
