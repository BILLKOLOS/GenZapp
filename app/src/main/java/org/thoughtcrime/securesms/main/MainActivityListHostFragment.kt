...
class MainActivityListHostFragment : Fragment(R.layout.main_activity_list_host_fragment), ConversationListFragment.Callback, Material3OnScrollHelperBinder, CallLogFragment.Callback {
  ...
  override fun updateNotificationProfileStatus(notificationProfiles: List<NotificationProfile>) {
    ...
    if (activeProfile != null) {
      if (activeProfile.id != GenZappStore.notificationProfile.lastProfilePopup) {
        view?.postDelayed({
          try {
            var fragmentView = view as? ViewGroup ?: return@postDelayed

            GenZappStore.notificationProfile.lastProfilePopup = activeProfile.id
            GenZappStore.notificationProfile.lastProfilePopupTime = System.currentTimeMillis()
    ...
    if (!GenZappStore.notificationProfile.hasSeenTooltip && Util.hasItems(notificationProfiles)) {
      ...
      if (target != null) {
        TooltipPopup.forTarget(target)
          ...
          .setBackgroundTint(ContextCompat.getColor(requireContext(), R.color.GenZapp_button_primary))
          .setTextColor(ContextCompat.getColor(requireContext(), R.color.GenZapp_button_primary_text))
          .setOnDismissListener { GenZappStore.notificationProfile.hasSeenTooltip = true }
