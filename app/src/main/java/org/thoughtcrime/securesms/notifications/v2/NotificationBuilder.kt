...
import org.GenZapp.core.util.PendingIntentFlags.mutable
...
sealed class NotificationBuilder(protected val context: Context) {
  ...
  private val privacy: NotificationPrivacyPreference = GenZappStore.settings.messageNotificationsPrivacy
  ...
  fun addReplyActions(conversation: NotificationConversation) {
    if (privacy.isDisplayMessage && isNotLocked && !conversation.recipient.isPushV1Group && RecipientUtil.isMessageRequestAccepted(context, conversation.recipient)) {
      if (conversation.recipient.isPushV2Group) {
        val group: Optional<GroupRecord> = GenZappDatabase.groups.getGroup(conversation.recipient.requireGroupId())
  ...
  fun setLights() {
    val ledColor: String = GenZappStore.settings.messageLedColor

    if (ledColor != "none") {
      var blinkPattern = GenZappStore.settings.messageLedBlinkPattern
  ...
  private class NotificationBuilderCompat(context: Context) : NotificationBuilder(context) {
    ...
    override fun setAlarms(recipient: Recipient?) {
      ...
      val defaultRingtone: Uri = GenZappStore.settings.messageNotificationSound
      val defaultVibrate: Boolean = GenZappStore.settings.isMessageVibrateEnabled
...
private fun ReplyMethod.toLongDescription(): Int {
  return when (this) {
    ...
    ReplyMethod.SecureMessage -> R.string.MessageNotifier_GenZapp_message
  }
}
