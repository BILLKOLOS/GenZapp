...
data class NotificationConversation(
  ...
  fun getSlideBigPictureUri(context: Context): Uri? {
    return if (notificationItems.size == 1 && GenZappStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
  ...
  fun getContentText(context: Context): CharSequence? {
    val privacy: NotificationPrivacyPreference = GenZappStore.settings.messageNotificationsPrivacy
  ...
  fun getConversationTitle(context: Context): CharSequence? {
    if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayContact) {
      return if (isGroup) getDisplayName(context) else null
    }
    return context.getString(R.string.SingleRecipientNotificationBuilder_GenZapp)
  }
