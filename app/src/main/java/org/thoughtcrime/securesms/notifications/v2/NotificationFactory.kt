...
object NotificationFactory {
  ...
  private fun notifyInThread(context: Context, recipient: Recipient, lastAudibleNotification: Long) {
    if (!GenZappStore.settings.isMessageNotificationsInChatSoundsEnabled ||
    ...
    val uri: Uri = if (NotificationChannels.supported()) {
      ...
      recipient.messageRingtone ?: GenZappStore.settings.messageNotificationSound
    }
  ...
  fun notifyStoryDeliveryFailed(context: Context, recipient: Recipient, thread: ConversationId) {
    ...
    val contentTitle = if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayContact) {
    ...
    val largeIcon = if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayContact) {
  ...
  fun notifyProofRequired(context: Context, recipient: Recipient, thread: ConversationId, visibleThread: ConversationId?) {
    ...
    builder.apply {
      ...
      setContentText(context.getString(R.string.MessageNotifier_verify_to_continue_messaging_on_GenZapp))
      setContentIntent(NotificationPendingIntentHelper.getActivity(context, 0, intent, PendingIntentFlags.mutable()))
    ...
    try {
      ...
      if (threadRecipient != null) {
        GenZappExecutors.BOUNDED.execute {
          GenZappDatabase.recipients.setMessageRingtone(threadRecipient.id, null)
