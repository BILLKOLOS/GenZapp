...
public class MarkReadReceiver extends BroadcastReceiver {
    ...
    if (threads != null) {
      ...
      GenZappExecutors.BOUNDED.execute(() -> {
        ...
        for (ConversationId thread : threads) {
          ...
          List<MarkedMessageInfo> messageIds = GenZappDatabase.threads().setRead(thread, true);
  ...
  public static void processCallEvents(@NonNull List<ConversationId> threads, long timestamp) {
    List<RecipientId> peers = GenZappDatabase.threads().getRecipientIdsForThreadIds(threads.stream()
                                                                                          .filter(it -> it.getGroupStoryId() == null)
                                                                                          .map(ConversationId::getThreadId)
                                                                                          .collect(java.util.stream.Collectors.toList()));

    for (RecipientId peer : peers) {
      CallTable.Call lastCallInThread = GenZappDatabase.calls().markAllCallEventsWithPeerBeforeTimestampRead(peer, timestamp);
  ...
  private static void scheduleDeletion(@NonNull List<ExpirationInfo> expirationInfo) {
    if (expirationInfo.size() > 0) {
      ...
      GenZappDatabase.messages().markExpireStarted(Stream.of(expirationInfo).map(info -> new kotlin.Pair<>(info.getId(), now)).toList());
