...
sealed class NotificationItem(val threadRecipient: Recipient, protected val record: MessageRecord) : Comparable<NotificationItem> {
  ...
  fun getStyledPrimaryText(context: Context, trimmed: Boolean = false): CharSequence {
    return if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayNothing) {
  ...
  fun getPersonName(context: Context): CharSequence {
    return if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayContact) {
      authorRecipient.getDisplayName(context)
    } else {
      context.getString(R.string.SingleRecipientNotificationBuilder_GenZapp)
    }
  }
  ...
  fun getPersonUri(): String? {
    return if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayContact && authorRecipient.isSystemContact) {
  ...
  fun getPrimaryText(context: Context): CharSequence {
    return if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayMessage) {
  ...
  fun getInboxLine(context: Context): CharSequence? {
    return when {
      GenZappStore.settings.messageNotificationsPrivacy.isDisplayNothing -> null
...
class MessageNotification(threadRecipient: Recipient, record: MessageRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return if (thread.groupStoryId != null) {
      GenZappDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId, record.dateReceived)
    } else {
      -1
    }
  }
  ...
  override fun getThumbnailInfo(context: Context): ThumbnailInfo {
    return if (GenZappStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
...
class ReactionNotification(threadRecipient: Recipient, record: MessageRecord, val reaction: ReactionRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return GenZappDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId ?: 0L, record.dateReceived)
  }
