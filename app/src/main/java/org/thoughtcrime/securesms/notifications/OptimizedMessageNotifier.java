...
public class OptimizedMessageNotifier implements MessageNotifier {
  ...
  public OptimizedMessageNotifier(@NonNull Application context) {
    this.limiter                = new LeakyBucketLimiter(3, 1000, new Handler(GenZappExecutors.getAndStartHandlerThread("GenZapp-notifier", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD).getLooper()));
  ...
  public void updateNotification(@NonNull Context context) {
    GenZappDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_GENERAL, () -> {
  ...
  public void updateNotification(@NonNull Context context, @NonNull ConversationId conversationId) {
    GenZappDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_CHAT + conversationId.getThreadId(), () -> {
