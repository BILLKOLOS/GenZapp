  ...
  public void onReceive(final Context context, Intent intent) {
    ...
    if (responseText != null) {
      GenZappExecutors.BOUNDED.execute(() -> {
        ...
        switch (replyMethod) {
          case GroupMessage: {
            OutgoingMessage reply = new OutgoingMessage(recipient,
                                                        ...
                                                        -1,
                                                        0);
            threadId = MessageSender.send(context, reply, -1, MessageSender.SendType.GenZapp, null, null);
            break;
          }
          case SecureMessage: {
        ...
        List<MarkedMessageInfo> messageIds = GenZappDatabase.threads().setRead(threadId, true);
