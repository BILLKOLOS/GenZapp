...
public class NotificationChannels {
  ...
  public synchronized @Nullable String createChannelFor(@NonNull Recipient recipient) {
    ...
    boolean      vibrationEnabled = vibrateState == VibrateState.DEFAULT ? GenZappStore.settings().isMessageVibrateEnabled() : vibrateState == VibrateState.ENABLED;
                                                        ...
                                                        @Nullable String shortcutId)
  {
    ...
    setLedPreference(channel, GenZappStore.settings().getMessageLedColor());
  ...
  public synchronized void updateMessageRingtone(@NonNull Recipient recipient, @Nullable Uri uri) {
    ...
    GenZappDatabase.recipients().setNotificationChannel(recipient.getId(), success ? newChannelId : null);
    ensureCustomChannelConsistency();
  }
  ...
  public boolean updateWithShortcutBasedChannel(@NonNull Recipient recipient) {
    if (Build.VERSION.SDK_INT >= CONVERSATION_SUPPORT_VERSION && TextUtils.isEmpty(recipient.getNotificationChannel())) {
      ...
      if (channel.isPresent()) {
        ...
        GenZappDatabase.recipients().setNotificationChannel(recipient.getId(), channel.get().getId());
        return true;
      }
    }
    return false;
  ...
  private void onCreate(@NonNull NotificationManager notificationManager) {
    ...
    NotificationChannel joinEvents                     = new NotificationChannel(JOIN_EVENTS, context.getString(R.string.NotificationChannel_contact_joined_GenZapp), NotificationManager.IMPORTANCE_DEFAULT);
    ...
    setVibrationEnabled(messages, GenZappStore.settings().isMessageVibrateEnabled());
    messages.setSound(GenZappStore.settings().getMessageNotificationSound(), getRingtoneAudioAttributes());
    setLedPreference(messages, GenZappStore.settings().getMessageLedColor());
