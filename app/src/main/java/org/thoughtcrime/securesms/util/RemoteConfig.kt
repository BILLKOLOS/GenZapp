...
import org.GenZapp.core.util.gibiBytes
import org.GenZapp.core.util.logging.Log
import org.GenZapp.core.util.mebiBytes
...
object RemoteConfig {
  ...
  fun init() {
    initLock.withLock {
      val current = parseStoredConfig(GenZappStore.remoteConfig.currentConfig)
      val pending = parseStoredConfig(GenZappStore.remoteConfig.pendingConfig)
      val changes = computeChanges(current, pending)

      GenZappStore.remoteConfig.currentConfig = mapToJson(pending)
  ...
  fun refreshIfNecessary() {
    val timeSinceLastFetch = System.currentTimeMillis() - GenZappStore.remoteConfig.lastFetchTime
  ...
  fun refreshSync() {
    val result = AppDependencies.GenZappServiceAccountManager.getRemoteConfig()
  ...
  fun update(config: Map<String, Any?>) {
    ...
    val disk = parseStoredConfig(GenZappStore.remoteConfig.pendingConfig)
    ...
    GenZappStore.remoteConfig.pendingConfig = mapToJson(result.disk)
    REMOTE_VALUES.clear()
    REMOTE_VALUES.putAll(result.memory)
    triggerFlagChangeListeners(result.memoryChanges)

    GenZappStore.remoteConfig.lastFetchTime = System.currentTimeMillis()
  ...
  val debugDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(GenZappStore.remoteConfig.currentConfig))

  /** Only for rendering debug info.  */
  @JvmStatic
  @get:Synchronized
  val debugPendingDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(GenZappStore.remoteConfig.pendingConfig))
  ...
  /** Make CDSI lookups via libGenZapp-net instead of native websocket.  */
  val useLibGenZappNetForCdsiLookup: Boolean by remoteBoolean(
    key = "android.cds.libGenZapp.4",
    defaultValue = false,
    hotSwappable = true
  )
  ...
  /** Whether unauthenticated chat web socket is backed by libGenZapp-net  */
  @JvmStatic
  @get:JvmName("libGenZappWebSocketEnabled")
  val libGenZappWebSocketEnabled: Boolean by remoteBoolean(
    key = "android.libGenZappWebSocketEnabled",
    defaultValue = false,
    hotSwappable = false
  )
   ...
   * an unauthenticated keep-alive via libGenZapp-net. Default: 0
   */
  @JvmStatic
  @get:JvmName("libGenZappWebSocketShadowingPercentage")
  val libGenZappWebSocketShadowingPercentage: Int by remoteValue(
    key = "android.libGenZappWebSocketShadowingPercentage",
    hotSwappable = false
  ) { value ->
