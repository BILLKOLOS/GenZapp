...
public class BackupUtil {
  ...
  public static boolean canUserAccessBackupDirectory(@NonNull Context context) {
    if (isUserSelectionRequired(context)) {
      Uri backupDirectoryUri = GenZappStore.settings().getGenZappBackupDirectory();
  ...
  public static void disableBackups(@NonNull Context context) {
    ...
    GenZappStore.settings().setBackupEnabled(false);
    BackupUtil.deleteAllBackups();

    if (BackupUtil.isUserSelectionRequired(context)) {
      Uri backupLocationUri = GenZappStore.settings().getGenZappBackupDirectory();

      if (backupLocationUri == null) {
        return;
      }

      GenZappStore.settings().clearGenZappBackupDirectory();
