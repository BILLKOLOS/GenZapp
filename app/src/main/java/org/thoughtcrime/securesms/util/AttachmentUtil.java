...
public class AttachmentUtil {
  ...
  public static @Nullable MessageRecord deleteAttachment(@NonNull DatabaseAttachment attachment) {
    ...
    int          attachmentCount = GenZappDatabase.attachments()
                                                 .getAttachmentsForMessage(mmsId)
                                                 .size();

    MessageRecord deletedMessageRecord = null;
    if (attachmentCount <= 1) {
      deletedMessageRecord = GenZappDatabase.messages().getMessageRecordOrNull(mmsId);
      GenZappDatabase.messages().deleteMessage(mmsId);
    } else {
      GenZappDatabase.attachments().deleteAttachment(attachmentId);
      if (Recipient.self().getDeleteSyncCapability().isSupported()) {
        MultiDeviceDeleteSyncJob.enqueueAttachmentDelete(GenZappDatabase.messages().getMessageRecordOrNull(mmsId), attachment);
      }
    }
  ...
  private static boolean isFromTrustedConversation(@NonNull Context context, @NonNull DatabaseAttachment attachment) {
    try {
      MessageRecord message = GenZappDatabase.messages().getMessageRecord(attachment.mmsId);

      Recipient fromRecipient = message.getFromRecipient();
      Recipient toRecipient   = GenZappDatabase.threads().getRecipientForThreadId(message.getThreadId());
