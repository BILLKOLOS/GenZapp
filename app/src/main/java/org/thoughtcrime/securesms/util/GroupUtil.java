...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil;
...
public final class GroupUtil {
  ...
  public static @Nullable GroupContextV2 getGroupContextIfPresent(@NonNull Content content) {
    if (content.dataMessage != null && GenZappServiceProtoUtil.INSTANCE.getHasGroupContext(content.dataMessage)) {
      return content.dataMessage.groupV2;
               ...
               GenZappServiceProtoUtil.INSTANCE.getHasGroupContext(content.syncMessage.sent.message))
    {
      return content.syncMessage.sent.message.groupV2;
    } else if (content.storyMessage != null && GenZappServiceProtoUtil.INSTANCE.isValid(content.storyMessage.group)) {
  ...
  public static void setDataMessageGroupContext(@NonNull Context context,
                                                @NonNull GenZappServiceDataMessage.Builder dataMessageBuilder,
                                                @NonNull GroupId.Push groupId)
  {
    if (groupId.isV2()) {
      ...
      GenZappServiceGroupV2            group             = GenZappServiceGroupV2.newBuilder(v2GroupProperties.getGroupMasterKey())
