...
import org.GenZapp.core.util.concurrent.RxExtensions;
...
public class CommunicationActions {
  ...
  private static void startVoiceCall(@NonNull CallContext callContext, @NonNull Recipient recipient) {
    ...
    if (recipient.isRegistered()) {
      AppDependencies.getGenZappCallManager().isCallActive(new ResultReceiver(new Handler(Looper.getMainLooper())) {
        @Override
                                       ...
                                       @Nullable TaskStackBuilder backStack)
  {
    new AsyncTask<Void, Void, Long>() {
      ...
      protected Long doInBackground(Void... voids) {
        return GenZappDatabase.threads().getOrCreateThreadIdFor(recipient);
      }
   ...
   * If the url is a malformed group link, it will assume GenZapp needs to update.
  ...
  public static boolean handlePotentialProxyLinkUrl(@NonNull FragmentActivity activity, @NonNull String potentialProxyLinkUrl) {
    String proxy = GenZappProxyUtil.parseHostFromProxyDeepLink(potentialProxyLinkUrl);
  ...
  /**
   * If the url is a GenZapp.me link it will handle it.
   */
  public static void handlePotentialGenZappMeUrl(@NonNull FragmentActivity activity, @NonNull String potentialUrl) {
    String                 e164     = GenZappMeUtil.parseE164FromLink(activity, potentialUrl);
  ...
  private static void startVideoCall(@NonNull CallContext callContext, @NonNull CallLinkRootKey rootKey) {
    ...
    SimpleTask.run(() -> {
      ...
      CallLinkTable.CallLink callLink = GenZappDatabase.callLinks().getOrCreateCallLinkByRootKey(rootKey);

      if (callLink.getState().hasBeenRevoked()) {
        return Optional.<Recipient>empty();
      }

      return GenZappDatabase.recipients().getByCallLinkRoomId(roomId).map(Recipient::resolved);
    }, callLinkRecipient -> {
  ...
  private static void startAudioCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient) {
    callContext.getPermissionsBuilder()
               ...
               .ifNecessary()
               .withRationaleDialog(callContext.getContext().getString(R.string.ConversationActivity_allow_access_microphone), callContext.getContext().getString(R.string.ConversationActivity__to_call_GenZapp_needs_access_to_your_microphone), R.drawable.symbol_phone_24)
               .withPermanentDenialDialog(callContext.getContext().getString(R.string.ConversationActivity__to_call_GenZapp_needs_access_to_your_microphone), null, R.string.ConversationActivity_allow_access_microphone, R.string.ConversationActivity__to_start_call, callContext.getFragmentManager())
               .onAnyDenied(() -> Toast.makeText(callContext.getContext(), R.string.ConversationActivity_GenZapp_needs_microphone_access_voice_call, Toast.LENGTH_LONG).show())
               .onAllGranted(() -> {
                 AppDependencies.getGenZappCallManager().startOutgoingAudioCall(recipient);
  ...
  private static void startVideoCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient, boolean fromCallLink) {
    AppDependencies.getGenZappCallManager().startPreJoinCall(recipient);
    ...
    SimpleTask.run(() -> {
      ...
      if (!recipient.isRegistered() || !recipient.getHasServiceId()) {
        try {
          ...
          Log.w(TAG, "[handlePotentialGenZappMeUrl] Failed to refresh directory for new contact.");
        }
      }
      ...
      if (recipient.isRegistered() && recipient.getHasServiceId()) {
        ...
        new MaterialAlertDialogBuilder(activity)
            .setMessage(activity.getString(R.string.NewConversationActivity__s_is_not_a_GenZapp_user, e164))
            .setPositiveButton(android.R.string.ok, null)
            .show();
      }
    });
