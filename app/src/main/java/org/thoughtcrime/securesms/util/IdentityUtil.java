...
import org.GenZapp.libGenZapp.protocol.state.SessionStore;
...
public final class IdentityUtil {
  ...
  public static void markIdentityVerified(Context context, Recipient recipient, boolean verified, boolean remote)
  {
    ...
    MessageTable smsDatabase   = GenZappDatabase.messages();
      ...
      while ((groupRecord = reader.getNext()) != null) {
        if (groupRecord.getMembers().contains(recipient.getId()) && groupRecord.isActive() && !groupRecord.isMms()) {
          ...
          if (remote) {
            ...
            RecipientId recipientId    = GenZappDatabase.recipients().getOrInsertFromGroupId(groupRecord.getId());
            ...
            try {
              GenZappDatabase.messages().insertMessageOutbox(outgoing, threadId, false, null);
            } catch (MmsException e) {
              throw new AssertionError(e);
            }
    ...
    if (remote) {
      ...
      long threadId = GenZappDatabase.threads().getOrCreateThreadIdFor(recipient);
  ...
  public static void markIdentityUpdate(@NonNull Context context, @NonNull RecipientId recipientId) {
    ...
    GenZappDatabase.messageLog().deleteAllForRecipient(recipientId);
  }
  ...
  public static void saveIdentity(String user, IdentityKey identityKey) {
    try(GenZappSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      ...
      GenZappProtocolAddress address          = new GenZappProtocolAddress(user, GenZappServiceAddress.DEFAULT_DEVICE_ID);
  ...
  public static void processVerifiedMessage(Context context, Verified verified) throws InvalidKeyException {
    GenZappServiceAddress          destination = new GenZappServiceAddress(ServiceId.parseOrThrow(verified.destinationAci));
