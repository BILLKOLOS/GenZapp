...
public final class GenZappProxyUtil {

  private static final String TAG = Log.tag(GenZappProxyUtil.class);

  private static final String PROXY_LINK_HOST = "GenZapp.tube";

  private static final Pattern PROXY_LINK_PATTERN = Pattern.compile("^(https|sgnl)://" + PROXY_LINK_HOST + "/#([^:]+).*$");
  private static final Pattern HOST_PATTERN       = Pattern.compile("^([^:]+).*$");

  private GenZappProxyUtil() {}

  public static void startListeningToWebsocket() {
    if (GenZappStore.proxy().isProxyEnabled() && AppDependencies.getGenZappWebSocket().getWebSocketState().firstOrError().blockingGet().isFailure()) {
   ...
   * network connections.
   */
  public static void enableProxy(@NonNull GenZappProxy proxy) {
    GenZappStore.proxy().enableProxy(proxy);
  ...
  public static void disableProxy() {
    GenZappStore.proxy().disableProxy();
    ConscryptGenZapp.setUseEngineSocketByDefault(false);
    AppDependencies.resetNetwork();
    startListeningToWebsocket();
  }
  ...
  public static void disableAndClearProxy(){
    disableProxy();
    GenZappStore.proxy().setProxy(null);
  }
  ...
  public static @NonNull String convertUserEnteredAddressToHost(@NonNull String host) {
    String parsedHost = GenZappProxyUtil.parseHostFromProxyDeepLink(host);
    if (parsedHost != null) {
      return parsedHost;
    }
  ...
  private static boolean testWebsocketConnectionUnregistered(long timeout) {
    ...
    GenZappServiceAccountManager accountManager = AccountManagerFactory.getInstance().createUnauthenticated(AppDependencies.getApplication(), "", GenZappServiceAddress.DEFAULT_DEVICE_ID, "");
