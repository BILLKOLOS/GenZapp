...
public class RecaptchaProofActivity extends PassphraseRequiredActivity {
  ...
  private static final String RECAPTCHA_SCHEME = "GenZappcaptcha://";
  ...
  private void handleToken(@NonNull String token) {
    ...
    SimpleTask.run(() -> {
      String challenge = GenZappStore.rateLimit().getChallenge();
      ...
      try {
        for (int i = 0; i < 3; i++) {
          try {
            AppDependencies.getGenZappServiceAccountManager().submitRateLimitRecaptchaChallenge(challenge, token);
