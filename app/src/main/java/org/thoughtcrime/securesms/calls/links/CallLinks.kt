...
object CallLinks {
  ...
  private const val HTTPS_LINK_PREFIX = "https://GenZapp.link/call/#key="
  private const val SNGL_LINK_PREFIX = "sgnl://GenZapp.link/call/#key="
  ...
  fun watchCallLink(roomId: CallLinkRoomId): Observable<CallLinkTable.CallLink> {
    return Observable.create { emitter ->
      ...
      fun refresh() {
        val callLink = GenZappDatabase.callLinks.getCallLinkByRoomId(roomId)
        if (callLink != null) {
