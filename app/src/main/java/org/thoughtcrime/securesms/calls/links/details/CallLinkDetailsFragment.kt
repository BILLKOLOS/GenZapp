...
import org.thoughtcrime.securesms.calls.links.GenZappCallRow
...
class CallLinkDetailsFragment : ComposeFragment(), CallLinkDetailsCallback {
  ...
  override fun onShareLinkViaGenZappClicked() {
    startActivity(
      ShareActivity.sendSimpleText(
        requireContext(),
        getString(R.string.CreateCallLink__use_this_link_to_join_a_GenZapp_call, CallLinks.url(viewModel.rootKeySnapshot))
      )
    )
  }
...
private interface CallLinkDetailsCallback {
  ...
  fun onShareClicked()
  fun onCopyClicked()
  fun onShareLinkViaGenZappClicked()
  fun onDeleteClicked()
  ...
  GenZappTheme(false) {
    CallLinkDetails(
      ...
      object : CallLinkDetailsCallback {
        ...
        override fun onShareLinkViaGenZappClicked() = Unit
...
private fun CallLinkDetails(
  ...
  Scaffolds.Settings(
    ...
    Column(modifier = Modifier.padding(paddingValues)) {
      ...
      Rows.TextRow(
        text = stringResource(id = R.string.CreateCallLinkBottomSheetDialogFragment__share_link_via_GenZapp),
        icon = painterResource(id = R.drawable.symbol_forward_24),
        onClick = callback::onShareLinkViaGenZappClicked
      )
