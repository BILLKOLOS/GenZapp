...
class CallLogPeekHelper : DefaultLifecycleObserver {
  ...
  private val executor: Executor = SerialExecutor(GenZappExecutors.BOUNDED_IO)
    ...
    executor.execute {
      ...
      items.forEach {
        when (it.identifier.peekEntryType) {
          PeekEntryType.CALL_LINK -> AppDependencies.GenZappCallManager.peekCallLinkCall(it.recipientId)
          PeekEntryType.GROUP_CALL -> AppDependencies.GenZappCallManager.peekGroupCall(it.recipientId)
        }
      }
