...
class CreateCallLinkBottomSheetDialogFragment : ComposeBottomSheetDialogFragment() {
  ...
  override fun SheetContent() {
    Column(
      ...
      Rows.TextRow(
        ...
        onClick = this@CreateCallLinkBottomSheetDialogFragment::onShareViaGenZappClicked
      )
  ...
  private fun onShareViaGenZappClicked() {
    lifecycleDisposable += viewModel.commitCallLink().subscribeBy(onSuccess = {
      when (it) {
        is EnsureCallLinkCreatedResult.Success -> {
          startActivity(
            ShareActivity.sendSimpleText(
              requireContext(),
              getString(R.string.CreateCallLink__use_this_link_to_join_a_GenZapp_call, CallLinks.url(viewModel.linkKeyBytes))
            )
          )
        }
