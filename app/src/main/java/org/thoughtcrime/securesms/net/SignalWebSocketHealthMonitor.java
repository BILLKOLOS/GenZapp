...
import org.whispersystems.GenZappservice.api.websocket.HealthMonitor;
 ...
 * timeouts.
 */
public final class GenZappWebSocketHealthMonitor implements HealthMonitor {

  private static final String TAG = Log.tag(GenZappWebSocketHealthMonitor.class);

  /**
   * This is the amount of time in between sent keep alives. Must be greater than {@link GenZappWebSocketHealthMonitor#KEEP_ALIVE_TIMEOUT}
   */
   ...
   * It is required that this value be less than {@link GenZappWebSocketHealthMonitor#KEEP_ALIVE_SEND_CADENCE}
   */
  ...
  private       GenZappWebSocket GenZappWebSocket;
  ...
  public GenZappWebSocketHealthMonitor(@NonNull Application context, @NonNull SleepTimer sleepTimer) {
    this.context    = context;
    this.sleepTimer = sleepTimer;
  }

  public void monitor(@NonNull GenZappWebSocket GenZappWebSocket) {
    executor.execute(() -> {
      Preconditions.checkNotNull(GenZappWebSocket);
      Preconditions.checkArgument(this.GenZappWebSocket == null, "monitor can only be called once");

      this.GenZappWebSocket = GenZappWebSocket;

      //noinspection ResultOfMethodCallIgnored
      GenZappWebSocket.getWebSocketState()
      ...
      //noinspection ResultOfMethodCallIgnored
      GenZappWebSocket.getUnidentifiedWebSocketState()
                     .subscribeOn(Schedulers.computation())
                     .observeOn(Schedulers.computation())
                     .distinctUntilChanged()
    ...
    executor.execute(() -> {
      if (status == 409) {
        ...
        if (healthState.mismatchErrorTracker.addSample(System.currentTimeMillis())) {
          ...
          GenZappWebSocket.forceNewWebSockets();
        }
      }
    });
  }
   ...
   * either WebSocket fails to get a return heartbeat after {@link GenZappWebSocketHealthMonitor#KEEP_ALIVE_TIMEOUT} seconds, both are forced to be recreated.
   */
      ...
      while (shouldKeepRunning && isKeepAliveNecessary()) {
        try {
          ...
          if (shouldKeepRunning && isKeepAliveNecessary()) {
            ...
            GenZappWebSocket.sendKeepAlive();
          }
