 ...
 * tables.
 */
public class KeyValueDatabase extends SQLiteOpenHelper implements GenZappDatabaseOpenHelper, KeyValuePersistentStorage {

  private static final String TAG = Log.tag(KeyValueDatabase.class);

  private static final int    DATABASE_VERSION = 1;
  public  static final String DATABASE_NAME    = "GenZapp-key-value.db";
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    if (GenZappDatabase.hasTable("key_value")) {
      Log.i(TAG, "Found old key_value table. Migrating data.");
      migrateDataFromPreviousDatabase(GenZappDatabase.getRawDatabase(), db);
    }
  }
  ...
  public void onOpen(SQLiteDatabase db) {
    ...
    GenZappExecutors.BOUNDED.execute(() -> {
      if (GenZappDatabase.hasTable("key_value")) {
        ...
        GenZappDatabase.getRawDatabase().execSQL("DROP TABLE key_value");
      }
    });
  }
