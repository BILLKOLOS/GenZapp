...
import org.GenZapp.libGenZapp.protocol.InvalidSessionException
...
class SessionTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(serviceId: ServiceId, address: GenZappProtocolAddress, record: SessionRecord) {
  ...
  fun load(serviceId: ServiceId, address: GenZappProtocolAddress): SessionRecord? {
  ...
  fun load(serviceId: ServiceId, addresses: List<GenZappProtocolAddress>): List<SessionRecord?> {
    ...
    val sessions: HashMap<GenZappProtocolAddress, SessionRecord?> = LinkedHashMap(addresses.size)
      ...
      readableDatabase.query(TABLE_NAME, projection, combinedQuery.where, combinedQuery.whereArgs, null, null, null).use { cursor ->
        while (cursor.moveToNext()) {
          ...
          try {
            ...
            sessions[GenZappProtocolAddress(address, device)] = record
          } catch (e: IOException) {
            Log.w(TAG, e)
          }
        }
  ...
  fun getSubDevices(serviceId: ServiceId, addressName: String): List<Int> {
    ...
    val selection = "$ACCOUNT_ID = ? AND $ADDRESS = ? AND $DEVICE != ?"
    val args = SqlUtil.buildArgs(serviceId, addressName, GenZappServiceAddress.DEFAULT_DEVICE_ID)
  ...
  fun delete(serviceId: ServiceId, address: GenZappProtocolAddress) {
