...
import org.GenZapp.core.util.Bitmask
...
import org.GenZapp.core.util.nullIfBlank
import org.GenZapp.core.util.optionalString
import org.GenZapp.core.util.or
import org.GenZapp.core.util.orNull
import org.GenZapp.core.util.readToList
import org.GenZapp.core.util.readToSet
import org.GenZapp.core.util.readToSingleBoolean
import org.GenZapp.core.util.readToSingleLong
...
import org.GenZapp.core.util.toInt
...
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.groups
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.identities
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.runPostSuccessfulTransaction
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.sessions
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.threads
...
open class RecipientTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    const val SEARCH_PROFILE_NAME = "search_GenZapp_profile"
    ...
    @JvmStatic
    fun maskCapabilitiesToLong(capabilities: GenZappServiceProfile.Capabilities): Long {
      var value: Long = 0
  ...
  fun applyStorageSyncContactInsert(insert: GenZappContactRecord) {
    val db = writableDatabase
  ...
  fun applyStorageSyncContactUpdate(update: StorageRecordUpdate<GenZappContactRecord>) {
    val db = writableDatabase
  ...
  fun applyStorageSyncGroupV1Insert(insert: GenZappGroupV1Record) {
  ...
  fun applyStorageSyncGroupV1Update(update: StorageRecordUpdate<GenZappGroupV1Record>) {
  ...
  fun applyStorageSyncGroupV2Insert(insert: GenZappGroupV2Record) {
  ...
  fun applyStorageSyncGroupV2Update(update: StorageRecordUpdate<GenZappGroupV2Record>) {
  ...
  fun applyStorageSyncAccountUpdate(update: StorageRecordUpdate<GenZappAccountRecord>) {
  ...
  fun setCapabilities(id: RecipientId, capabilities: GenZappServiceProfile.Capabilities) {
    val values = ContentValues(1).apply {
  ...
  fun markHidden(id: RecipientId, clearProfileKey: Boolean = false, showMessageRequest: Boolean = false) {
    ...
    if (updated) {
      GenZappDatabase.distributionLists.removeMemberFromAllLists(id)
      GenZappDatabase.messages.deleteStoriesForRecipient(id)
      rotateStorageId(id)
      AppDependencies.databaseObserver.notifyRecipientChanged(id)
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
  ...
  fun updateSelfE164(e164: String, pni: PNI) {
    ...
    try {
      ...
      val newId = getAndPossiblyMerge(aci = GenZappStore.account.requireAci(), pni = pni, e164 = e164, pniVerified = true, changeSelf = true)
      ...
      GenZappDatabase.pendingPniSignatureMessages.deleteAll()
      ...
      when (operation) {
        ...
        is PnpOperation.SessionSwitchoverInsert -> {
          if (hadThreadMerge) {
            ...
            if (threadId != null) {
              ...
              try {
                GenZappDatabase.messages.insertSessionSwitchoverEvent(operation.recipientId, threadId, event)
              } catch (e: Exception) {
                ...
                if (record.aci != null && record.aci == GenZappStore.account.aci) {
                  if (pnisWithSessions.contains(GenZappStore.account.pni!!)) {
                ...
                if (record.pni != null && record.pni == GenZappStore.account.pni) {
                  if (pnisWithSessions.contains(GenZappStore.account.pni!!)) {
                ...
                if (record.e164 != null && record.e164 == GenZappStore.account.e164) {
                  if (pnisWithSessions.contains(GenZappStore.account.pni!!)) {
                ...
                if (pnisWithSessions.isEmpty()) {
                  ...
                  if (pnisWithSessions.first() == GenZappStore.account.pni) {
        ...
        is PnpOperation.ChangeNumberInsert -> {
          if (changeSet.id is PnpIdResolver.PnpNoopId) {
            GenZappDatabase.messages.insertNumberChangeMessages(changeSet.id.recipientId)
          } else {
  ...
  private fun notSelf(e164: String?, pni: PNI?, aci: ACI?): Boolean {
    return (e164 == null || e164 != GenZappStore.account.e164) &&
      (pni == null || pni != GenZappStore.account.pni) &&
      (aci == null || aci != GenZappStore.account.aci)
  }
  ...
  /** All e164's that are eligible for having a GenZapp link added to their system contact entry. */
  ...
  fun getGenZappContacts(includeSelf: Boolean): Cursor? {
    return getGenZappContacts(includeSelf, "$SORT_NAME, $SYSTEM_JOINED_NAME, $SEARCH_PROFILE_NAME, $USERNAME, $E164")
  }

  fun getGenZappContactsCount(includeSelf: Boolean): Int {
    return getGenZappContacts(includeSelf)?.count ?: 0
  }

  private fun getGenZappContacts(includeSelf: Boolean, orderBy: String? = null): Cursor? {
  ...
  fun queryGenZappContacts(contactSearchQuery: ContactSearchQuery): Cursor? {
  ...
  fun queryGenZappContactLetterHeaders(inputQuery: String, includeSelf: Boolean, includePush: Boolean, includeSms: Boolean): Map<RecipientId, String> {
    val searchSelection = ContactSearchSelection.Builder()
  ...
  fun getNonGenZappContacts(): Cursor? {
  ...
  fun queryNonGenZappContacts(inputQuery: String): Cursor? {
  ...
  fun applyBlockedUpdate(blocked: List<GenZappServiceAddress>, groupIds: List<ByteArray?>) {
    val blockedE164 = blocked
      .filter { b: GenZappServiceAddress -> b.number.isPresent }
      .map { b: GenZappServiceAddress -> b.number.get() }
      .toList()

    val blockedUuid = blocked
      .map { b: GenZappServiceAddress -> b.serviceId.toString().lowercase() }
      .toList()
  ...
  private fun merge(primaryId: RecipientId, secondaryId: RecipientId, newPni: PNI? = null, pniVerified: Boolean): MergeResult {
    ...
    if (threadMerge.neededMerge) {
      ...
      GenZappDatabase.messages.insertThreadMergeEvent(primaryRecord.id, threadMerge.threadId, mergeEvent.build())
    }
  ...
  private fun getValuesForStorageContact(contact: GenZappContactRecord, isInsert: Boolean): ContentValues {
  ...
  private fun getValuesForStorageGroupV1(groupV1: GenZappGroupV1Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  private fun getValuesForStorageGroupV2(groupV2: GenZappGroupV2Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  fun updatePendingSelfData(selfId: RecipientId) {
    GenZappDatabase.messages.updatePendingSelfData(RecipientId.from(PLACEHOLDER_SELF_ID), selfId)
  ...
  private fun updateProfileValuesForMerge(values: ContentValues, record: RecipientRecord) {
    values.apply {
      ...
      putNull(EXPIRING_PROFILE_KEY_CREDENTIAL)
      put(PROFILE_AVATAR, record.GenZappProfileAvatar)
      put(PROFILE_GIVEN_NAME, record.GenZappProfileName.givenName)
      put(PROFILE_FAMILY_NAME, record.GenZappProfileName.familyName)
      put(PROFILE_JOINED_NAME, record.GenZappProfileName.toString())
    }
  }
  ...
  internal class ContactSearchSelection private constructor(val where: String, val args: Array<String>) {
      ...
      fun build(): ContactSearchSelection {
        ...
        if (includeRegistered) {
          ...
          if (Util.isEmpty(searchQuery)) {
            stringBuilder.append(GenZapp_CONTACT)
          } else {
            stringBuilder.append(QUERY_GenZapp_CONTACT)
        ...
        if (includeNonRegistered) {
          ...
          if (Util.isEmpty(searchQuery)) {
            stringBuilder.append(NON_GenZapp_CONTACT)
          } else {
            stringBuilder.append(QUERY_NON_GenZapp_CONTACT)
            args.add(searchQuery)
            args.add(searchQuery)
            args.add(searchQuery)
          }
    ...
    companion object {
      ...
      const val NON_GenZapp_CONTACT = "$REGISTERED != ? AND $SYSTEM_CONTACT_URI NOT NULL AND ($E164 NOT NULL OR $EMAIL NOT NULL)"
      val QUERY_NON_GenZapp_CONTACT = "$NON_GenZapp_CONTACT AND ($E164_SEARCH OR $EMAIL GLOB ? OR $SYSTEM_JOINED_NAME GLOB ?)"
      const val GenZapp_CONTACT = "$REGISTERED = ? AND (NULLIF($SYSTEM_JOINED_NAME, '') NOT NULL OR $PROFILE_SHARING = ?) AND ($SORT_NAME NOT NULL OR $USERNAME NOT NULL)"
      val QUERY_GenZapp_CONTACT = "$GenZapp_CONTACT AND ($E164_SEARCH OR $SORT_NAME GLOB ? OR $USERNAME GLOB ?)"
