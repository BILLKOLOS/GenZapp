 ...
 */
class CallLinkTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  companion object {
    ...
    private fun GenZappCallLinkState.serialize(): ContentValues {
  ...
  fun insertCallLink(
    ...
    val recipientId: RecipientId = writableDatabase.withinTransaction { db ->
      val recipientId = GenZappDatabase.recipients.getOrInsertFromCallLinkRoomId(callLink.roomId)
  ...
  fun deleteNonAdminCallLinksOnOrBefore(timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      GenZappDatabase.calls.updateAdHocCallEventDeletionTimestamps(skipSync = true)
    }
  }
