...
import org.GenZapp.core.util.SqlUtil.buildCustomCollectionQuery
import org.GenZapp.core.util.SqlUtil.buildSingleCollectionQuery
import org.GenZapp.core.util.SqlUtil.buildTrueUpdateQuery
import org.GenZapp.core.util.SqlUtil.getNextAutoIncrementId
...
import org.GenZapp.core.util.readToSingleLongOrNull
...
import org.GenZapp.core.util.toSingleLine
...
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.attachments
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.calls
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.distributionLists
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.groupReceipts
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.groups
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.mentions
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.messageLog
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.messages
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.reactions
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.recipients
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.storySends
...
open class MessageTable(context: Context?, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper), MessageTypes, RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  fun updateGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      val updateDetail = GroupCallUpdateDetailsUtil.parse(message.body)
      val containsSelf = joinedUuids.contains(GenZappStore.account.requireAci().rawUuid)
  ...
  fun updatePreviousGroupCall(
    ...
    return writableDatabase.withinTransaction { db ->
      ...
      MmsReader(cursor).use { reader ->
        ...
        val containsSelf = peekJoinedUuids.contains(GenZappStore.account.requireAci().rawUuid)
  ...
  fun insertProfileNameChangeMessages(recipient: Recipient, newProfileName: String, previousProfileName: String) {
    writableDatabase.withinTransaction { db ->
      ...
      groupRecords.filter { it.isV2Group }.forEach {
        GenZappDatabase.nameCollisions.handleGroupNameCollisions(it.id.requireV2(), setOf(recipient.id))
      }
    }
  }
  ...
  fun insertLearnedProfileNameChangeMessage(recipient: Recipient, e164: String?, username: String?) {
    ...
    val threadId: Long? = GenZappDatabase.threads.getThreadIdFor(recipient.id)
  ...
  private fun insertGroupV1MigrationMembershipChanges(recipientId: RecipientId, threadId: Long, membershipChange: GroupMigrationMembershipChange) {
    ...
    databaseHelper.GenZappWritableDatabase.insert(TABLE_NAME, null, values)
  }
  ...
  fun ensureMigration() {
    databaseHelper.GenZappWritableDatabase
  }
  ...
  fun markOnboardingStoryRead() {
    val recipientId = GenZappStore.releaseChannel.releaseChannelRecipientId ?: return
  ...
  fun setNetworkFailures(messageId: Long, failures: Set<NetworkFailure?>?) {
    try {
      setDocument(databaseHelper.GenZappWritableDatabase, messageId, NETWORK_FAILURES, NetworkFailureSet(failures))
    } catch (e: IOException) {
      Log.w(TAG, e)
    }
  }
  ...
  private fun rawQueryWithAttachments(where: String, arguments: Array<String>?, reverse: Boolean = false, limit: Long = 0): Cursor {
    val database = databaseHelper.GenZappReadableDatabase
    var rawQueryString = """
      SELECT 
  ...
  fun insertMessageOutbox(
    ...
    contentValues.put(FROM_DEVICE_ID, GenZappStore.account.deviceId)
  ...
  private fun insertMediaMessage(
    ...
    val (messageId, insertedAttachments) = writableDatabase.withinTransaction { db ->
      ...
      GenZappDatabase.mentions.insert(threadId, messageId, mentions)

      val insertedAttachments = GenZappDatabase.attachments.insertAttachmentsForMessage(messageId, allAttachments, quoteAttachments)
  ...
  fun getAllRateLimitedMessageIds(): Set<Long> {
    val db = databaseHelper.GenZappReadableDatabase
  ...
  fun deleteMessages(messagesToDelete: List<MessageTable.SyncMessageId>): List<SyncMessageId> {
    ...
    threads
      .forEach { threadId ->
        GenZappDatabase.threads.update(threadId, unarchive = false)
        notifyConversationListeners(threadId)
      }
  ...
  fun setMismatchedIdentities(messageId: Long, mismatches: Set<IdentityKeyMismatch?>) {
    try {
      setDocument(
        database = databaseHelper.GenZappWritableDatabase,
        messageId = messageId,
        column = MISMATCHED_IDENTITIES,
  ...
  private fun getReleaseChannelThreadId(hasSeenReleaseChannelStories: Boolean): Long {
    ...
    val releaseChannelRecipientId = GenZappStore.releaseChannel.releaseChannelRecipientId ?: return -1L
