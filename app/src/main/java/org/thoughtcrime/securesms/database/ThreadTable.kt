...
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.drafts
...
class ThreadTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun allowedToUnarchive(threadId: Long): Boolean {
    if (!GenZappStore.settings.shouldKeepMutedChatsArchived()) {
      return true
    }
  ...
  fun trimAllThreads(length: Int, trimBeforeDate: Long) {
    ...
    val syncThreadTrimDeletes = GenZappStore.settings.shouldSyncThreadTrimDeletes() && Recipient.self().deleteSyncCapability.isSupported
  ...
  private fun trimThreadInternal(
    ...
    if (finalTrimBeforeDate != NO_TRIM_BEFORE_DATE_SET) {
      ...
      if (deletes > 0) {
        ...
        GenZappDatabase.calls.updateCallEventDeletionTimestamps()
  ...
  fun getRecentConversationList(limit: Int, includeInactiveGroups: Boolean, individualsOnly: Boolean, groupsOnly: Boolean, hideV1Groups: Boolean, hideSms: Boolean, hideSelf: Boolean): Cursor {
    ...
    if (GenZappStore.releaseChannel.releaseChannelRecipientId != null) {
      where += " AND $TABLE_NAME.$RECIPIENT_ID != ${GenZappStore.releaseChannel.releaseChannelRecipientId!!.toLong()}"
    }
  ...
  fun applyStorageSyncUpdate(recipientId: RecipientId, record: GenZappContactRecord) {
    applyStorageSyncUpdate(recipientId, record.isArchived, record.isForcedUnread)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: GenZappGroupV1Record) {
    applyStorageSyncUpdate(recipientId, record.isArchived, record.isForcedUnread)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: GenZappGroupV2Record) {
    applyStorageSyncUpdate(recipientId, record.isArchived, record.isForcedUnread)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: GenZappAccountRecord) {
    writableDatabase.withinTransaction { db ->
  ...
  private fun hasMoreRecentDraft(threadId: Long, timestamp: Long): Boolean {
    val drafts: DraftTable.Drafts = GenZappDatabase.drafts.getDrafts(threadId)
  ...
  fun merge(primaryRecipientId: RecipientId, secondaryRecipientId: RecipientId): MergeResult {
    check(databaseHelper.GenZappWritableDatabase.inTransaction()) { "Must be in a transaction!" }
