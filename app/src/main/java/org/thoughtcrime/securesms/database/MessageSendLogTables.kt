 ...
 */
class MessageSendLogTables constructor(context: Context?, databaseHelper: GenZappDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun addRecipientToExistingEntryIfPossible(payloadId: Long, recipientId: RecipientId, sentTimestamp: Long, sendMessageResult: SendMessageResult, contentHint: ContentHint, messageId: MessageId, urgent: Boolean): Long {
    ...
    if (sendMessageResult.isSuccess && sendMessageResult.success.content.isPresent) {
      val db = databaseHelper.GenZappWritableDatabase
  ...
  fun deleteAll() {
    databaseHelper.GenZappWritableDatabase.delete(MslPayloadTable.TABLE_NAME, null, null)
  }
