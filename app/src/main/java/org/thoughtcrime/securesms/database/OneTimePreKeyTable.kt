...
class OneTimePreKeyTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun delete(serviceId: ServiceId, keyId: Int) {
    val database = databaseHelper.GenZappWritableDatabase
