 ...
 */
class SenderKeySharedTable internal constructor(context: Context?, databaseHelper: GenZappDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
...
/**
   ...
   */
  fun markAsShared(distributionId: DistributionId, addresses: Collection<GenZappProtocolAddress>) {
   ...
   */
  fun getSharedWith(distributionId: DistributionId): Set<GenZappProtocolAddress> {
   ...
   */
  fun delete(distributionId: DistributionId, addresses: Collection<GenZappProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
   ...
   */
  fun deleteAllFor(addresses: Collection<GenZappProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
        db.delete(TABLE_NAME)
