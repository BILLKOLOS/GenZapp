...
open class GenZappDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret, private val name: String = DATABASE_NAME) :
  SQLiteOpenHelper(
    context,
    DATABASE_NAME,
    databaseSecret.asString(),
    null,
    GenZappDatabaseMigrations.DATABASE_VERSION,
    0,
  ...
  override fun onUpgrade(db: net.zetetic.database.sqlcipher.SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    try {
      ...
      GenZappDatabaseMigrations.migrate(context, db, oldVersion, newVersion)
    } finally {
      db.setForeignKeyConstraintsEnabled(true)
    ...
    GenZappDatabaseMigrations.migratePostTransaction(context, oldVersion)
    Log.i(TAG, "Upgrade complete. Took " + (System.currentTimeMillis() - startTime) + " ms.")
  }

  override fun getReadableDatabase(): net.zetetic.database.sqlcipher.SQLiteDatabase {
    throw UnsupportedOperationException("Call getGenZappReadableDatabase() instead!")
  }

  override fun getWritableDatabase(): net.zetetic.database.sqlcipher.SQLiteDatabase {
    throw UnsupportedOperationException("Call getGenZappWritableDatabase() instead!")
  }
  ...
  open val GenZappReadableDatabase: SQLiteDatabase
    get() = SQLiteDatabase(super.getReadableDatabase())

  open val GenZappWritableDatabase: SQLiteDatabase
  ...
  open fun markCurrent(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    db.version = GenZappDatabaseMigrations.DATABASE_VERSION
  }
  ...
  companion object {
    private val TAG = Log.tag(GenZappDatabase::class.java)
    const val DATABASE_NAME = "GenZapp.db"

    @JvmStatic
    @Volatile
    var instance: GenZappDatabase? = null
      private set

    @JvmStatic
    fun init(application: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) {
      if (instance == null) {
        synchronized(GenZappDatabase::class.java) {
          if (instance == null) {
            instance = GenZappDatabase(application, databaseSecret, attachmentSecret)
          }
        }
      }
    }
    ...
    @JvmStatic
    @VisibleForTesting
    fun setGenZappDatabaseInstanceForTesting(GenZappDatabase: GenZappDatabase) {
      this.instance = GenZappDatabase
    }
    ...
    fun runPostSuccessfulTransaction(dedupeKey: String, task: Runnable) {
      instance!!.GenZappWritableDatabase.runPostSuccessfulTransaction(dedupeKey, task)
    }

    @JvmStatic
    fun runPostSuccessfulTransaction(task: Runnable) {
      instance!!.GenZappWritableDatabase.runPostSuccessfulTransaction(task)
    }
    ...
    fun triggerDatabaseAccess() {
      instance!!.GenZappWritableDatabase
    }
    ...
    fun <T> runInTransaction(block: (SQLiteDatabase) -> T): T {
      return instance!!.GenZappWritableDatabase.withinTransaction {
        block(it)
      }
    }
