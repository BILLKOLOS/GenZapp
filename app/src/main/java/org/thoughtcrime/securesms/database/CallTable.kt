...
import org.GenZapp.core.util.IntSerializer
import org.GenZapp.core.util.Serializer
...
import org.GenZapp.core.util.flatten
...
import org.GenZapp.core.util.requireObject
...
import org.GenZapp.ringrtc.CallManager.RingUpdate
...
import org.whispersystems.GenZappservice.internal.push.SyncMessage.CallEvent
 ...
 */
class CallTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun insertOneToOneCall(callId: Long, timestamp: Long, peer: RecipientId, type: Type, direction: Direction, event: Event) {
    ...
    writableDatabase.withinTransaction {
      val result = GenZappDatabase.messages.insertCallLog(peer, messageType, timestamp, direction == Direction.OUTGOING)
    ...
    return writableDatabase.withinTransaction {
      ...
      if (call != null) {
        ...
        if (call.messageId == null) {
          ...
          GenZappDatabase.messages.updateCallLog(call.messageId, call.messageType)
        }
  ...
  fun markCallDeletedFromSyncEvent(call: Call) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        GenZappDatabase.messages.deleteMessage(call.messageId)
      }
    }
  ...
  fun insertAcceptedGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId: MessageId? = if (type == Type.GROUP_CALL) {
        GenZappDatabase.messages.insertGroupCall(
  ...
  private fun handleGroupUpdate(
    ...
    writableDatabase.withinTransaction {
      ...
      val messageId: MessageId = if (call != null) {
        ...
        GenZappDatabase.messages.updateGroupCall(
          call.messageId,
          peekGroupCallEraId,
          peekJoinedUuids,
          isCallFull,
  ...
  fun updateGroupCallFromPeek(
    ...
    val recipientId = GenZappDatabase.threads.getRecipientIdForThreadId(threadId)
    ...
    val sameEraId = GenZappDatabase.messages.updatePreviousGroupCall(
      threadId = threadId,
  ...
  private fun createEventFromRingState(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId = GenZappDatabase.messages.insertGroupCall(
  ...
  fun setTimestamp(callId: Long, recipientId: RecipientId, timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        GenZappDatabase.messages.updateCallTimestamps(call.messageId, timestamp)
      }
    }
  ...
  fun deleteNonAdHocCallEvents(callRowIds: Set<Long>) {
    ...
    GenZappDatabase.messages.deleteCallUpdates(messageIds)
  ...
  fun deleteAllNonAdHocCallEventsExcept(callRowIds: Set<Long>, missedOnly: Boolean) {
    ...
    if (callRowIds.isEmpty()) {
      ...
      threadIds.forEach {
        GenZappDatabase.threads.update(
          threadId = it,
          unarchive = false,
          allowDeletion = true
        )
  ...
  fun markRingingCallsAsMissed() {
    writableDatabase.withinTransaction { db ->
      ...
      GenZappDatabase.messages.clearIsRingingOnLocalDeviceFlag(messageIds)
    }
  }
  ...
  fun getCalls(offset: Int, limit: Int, searchTerm: String?, filter: CallLogFilter): List<CallLogRow.Call> {
    return getCallsCursor(false, offset, limit, searchTerm, filter).readToList { cursor ->
      ...
      val canUserBeginCall = if (peer.isGroup) {
        val record = GenZappDatabase.groups.getGroup(peer.id)
      ...
      CallLogRow.Call(
        ...
        callLinkPeekInfo = AppDependencies.GenZappCallManager.peekInfoSnapshot[peer.id],
