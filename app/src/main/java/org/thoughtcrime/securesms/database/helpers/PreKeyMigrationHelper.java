  ...
  public static boolean migratePreKeys(Context context, SQLiteDatabase database) {
    ...
    if (oneTimePreKeyIndex.exists()) {
      try {
        ...
        GenZappStore.account().aciPreKeys().setNextEcOneTimePreKeyId(index.nextPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
    ...
    if (signedPreKeyIndex.exists()) {
      try {
        ...
        GenZappStore.account().aciPreKeys().setNextSignedPreKeyId(index.nextSignedPreKeyId);
        GenZappStore.account().aciPreKeys().setActiveSignedPreKeyId(index.activeSignedPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
