...
class MediaTable internal constructor(context: Context?, databaseHelper: GenZappDatabase?) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    private val ALL_MEDIA_QUERY = String.format(
      ...
      """
        ${AttachmentTable.DATA_FILE} IS NOT NULL AND
        ${AttachmentTable.CONTENT_TYPE} NOT LIKE 'text/x-GenZapp-plain' AND
        ${MessageTable.LINK_PREVIEWS} IS NULL
      """
    )
      ...
      """
        ...
        (
          ...
          (
            ...
            ${AttachmentTable.CONTENT_TYPE} NOT LIKE 'text/x-GenZapp-plain'
          )
        )"""
    )
  ...
  data class MediaRecord constructor(
    ...
    companion object {
      ...
      fun from(cursor: Cursor): MediaRecord {
        val attachments = GenZappDatabase.attachments.getAttachments(cursor)
