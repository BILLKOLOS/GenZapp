...
class RemappedRecords {
  ...
  void deleteThread(long oldId) {
    ...
    GenZappDatabase.remappedRecords().deleteThreadMapping(oldId);
  }
  ...
  void addRecipient(@NonNull RecipientId oldId, @NonNull RecipientId newId) {
    ...
    GenZappDatabase.remappedRecords().addRecipientMapping(oldId, newId);
  }
  ...
  void addThread(long oldId, long newId) {
    ...
    GenZappDatabase.remappedRecords().addThreadMapping(oldId, newId);
  }
  ...
  private void ensureRecipientMapIsPopulated() {
    if (recipientMap == null) {
      recipientMap = GenZappDatabase.remappedRecords().getAllRecipientMappings();
    }
  }

  private void ensureThreadMapIsPopulated() {
    if (threadMap == null) {
      threadMap = GenZappDatabase.remappedRecords().getAllThreadMappings();
    }
  }
