...
import org.GenZapp.core.util.SqlUtil.appendArg
import org.GenZapp.core.util.SqlUtil.buildArgs
import org.GenZapp.core.util.SqlUtil.buildCaseInsensitiveGlobPattern
import org.GenZapp.core.util.SqlUtil.buildCollectionQuery
...
import org.GenZapp.core.util.readToMap
...
import org.whispersystems.GenZappservice.api.groupsv2.findMemberByAci
import org.whispersystems.GenZappservice.api.groupsv2.findPendingByServiceId
import org.whispersystems.GenZappservice.api.groupsv2.findRequestingByAci
import org.whispersystems.GenZappservice.api.groupsv2.toAciList
import org.whispersystems.GenZappservice.api.groupsv2.toAciListWithUnknowns
...
class GroupTable(context: Context?, databaseHelper: GenZappDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  class MembershipTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper) {
    companion object {
  ...
  fun queryGroupsByMemberName(inputQuery: String): Cursor {
    ...
    return databaseHelper.GenZappReadableDatabase.query(statement, subquery.whereArgs)
  }
  ...
  fun queryGroupsByTitle(inputQuery: String, includeInactive: Boolean, excludeV1: Boolean, excludeMms: Boolean): Reader {
    ...
    val cursor = databaseHelper.GenZappReadableDatabase.query(statement, query.whereArgs)
  ...
  private fun queryGroupsByRecency(groupQuery: GroupQuery): Reader {
    ...
    return Reader(databaseHelper.GenZappReadableDatabase.rawQuery(sql, query.whereArgs))
  }
  ...
  @CheckReturnValue
  fun create(groupId: GroupId.V1, title: String?, members: Collection<RecipientId>, avatar: GenZappServiceAttachmentPointer?): Boolean {
  ...
  private fun create(
    ...
    avatar: GenZappServiceAttachmentPointer?,
    groupMasterKey: GroupMasterKey?,
    ...
    if (avatar != null) {
      values.put(AVATAR_ID, (avatar.remoteId as GenZappServiceAttachmentRemoteId.V2).cdnId)
    ...
    if (groupId.isV2) {
      GenZappDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), members.toSet())
    }

    return true
  }

  fun update(groupId: GroupId.V1, title: String?, avatar: GenZappServiceAttachmentPointer?) {
    val contentValues = ContentValues().apply {
      ...
      if (avatar != null) {
        put(AVATAR_ID, (avatar.remoteId as GenZappServiceAttachmentRemoteId.V2).cdnId)
  ...
  fun update(groupId: GroupId.V2, decryptedGroup: DecryptedGroup, receivedGroupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    if (groupId.isV2 && addedMembers.isNotEmpty()) {
      GenZappDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), addedMembers.toSet())
    }
  }
  ...
  fun getGroupSendFullToken(threadId: Long, recipientId: RecipientId): GroupSendFullToken? {
    val threadRecipient = GenZappDatabase.threads.getRecipientForThreadId(threadId)
  ...
  fun getGroupSendFullToken(groupId: GroupId.V2, recipientId: RecipientId): GroupSendFullToken? {
    val groupRecord = GenZappDatabase.groups.getGroup(groupId).orElse(null) ?: return null
    val endorsement = GenZappDatabase.groups.getGroupSendEndorsement(groupId, recipientId) ?: return null
  ...
  class V2GroupProperties(val groupMasterKey: GroupMasterKey, val groupRevision: Int, private val decryptedGroupBytes: ByteArray) {
    ...
    fun getMemberRecipientIds(memberSet: MemberSet): List<RecipientId> {
      ...
      val selfAci = GenZappStore.account.requireAci()
