 ...
 */
class PendingPniSignatureMessageTable(context: Context, databaseHelper: GenZappDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
    ...
    pendingPniSignaturesCacheLock.writeLock().withLock {
      writableDatabase.withinTransaction { db ->
        ...
        if (!stillPending) {
          ...
          GenZappDatabase.recipients.clearNeedsPniSignature(recipientId)
