...
import org.GenZapp.core.util.drain
import org.GenZapp.core.util.exists
import org.GenZapp.core.util.forEach
import org.GenZapp.core.util.groupBy
import org.GenZapp.core.util.isNull
...
import org.thoughtcrime.securesms.database.GenZappDatabase.Companion.stickers
...
class AttachmentTable(
  context: Context,
  databaseHelper: GenZappDatabase,
  ...
  fun deleteAttachments(toDelete: List<SyncAttachmentId>): List<SyncMessageId> {
    ...
    for (syncAttachmentId in toDelete) {
      val messageId = GenZappDatabase.messages.getMessageIdOrNull(syncAttachmentId.syncMessageId)
      if (messageId != null) {
  ...
  fun finalizeAttachmentAfterDownload(mmsId: Long, attachmentId: AttachmentId, inputStream: InputStream) {
    ...
    val transferFile: File? = getTransferFile(databaseHelper.GenZappReadableDatabase, attachmentId)
