...
public final class StorageSyncValidations {
  ...
  public static void validate(@NonNull StorageSyncHelper.WriteOperationResult result,
                              @NonNull GenZappStorageManifest previousManifest,
  ...
  public static void validateForcePush(@NonNull GenZappStorageManifest manifest, @NonNull List<GenZappStorageRecord> inserts, @NonNull Recipient self) {
    validateManifestAndInserts(manifest, inserts, self);
  }

  private static void validateManifestAndInserts(@NonNull GenZappStorageManifest manifest, @NonNull List<GenZappStorageRecord> inserts, @NonNull Recipient self) {
    int accountCount = 0;
    ...
    Set<StorageId>  insertSet = new HashSet<>(Stream.of(inserts).map(GenZappStorageRecord::getId).toList());
    ...
    for (GenZappStorageRecord insert : inserts) {
      ...
      if (insert.getContact().isPresent()) {
        GenZappContactRecord contact = insert.getContact().get();
