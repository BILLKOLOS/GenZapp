...
class ChatItemImportInserter(
  ...
  private fun ChatItem.toMessageInsert(fromRecipientId: RecipientId, chatRecipientId: RecipientId, threadId: Long): MessageInsert {
      ...
      if (contacts.isNotEmpty()) {
        followUp = { messageRowId ->
          val attachmentMap = if (contactAttachments.isNotEmpty()) {
            GenZappDatabase.attachments.insertAttachmentsForMessage(messageRowId, contactAttachments, emptyList())
          } else {
            emptyMap()
          }
          db.update(
            MessageTable.TABLE_NAME,
            contentValuesOf(
              MessageTable.SHARED_CONTACTS to GenZappDatabase.messages.getSerializedSharedContacts(attachmentMap, contacts)
            ),
            "${MessageTable.ID} = ?",
            SqlUtil.buildArgs(messageRowId)
          )
    ...
    if (this.standardMessage != null) {
      ...
      if (attachments.isNotEmpty() || linkPreviewAttachments.isNotEmpty() || quoteAttachments.isNotEmpty()) {
        followUp = { messageRowId ->
          val attachmentMap = GenZappDatabase.attachments.insertAttachmentsForMessage(messageRowId, attachments + linkPreviewAttachments, quoteAttachments)
          if (linkPreviews.isNotEmpty()) {
            db.update(
              MessageTable.TABLE_NAME,
              contentValuesOf(
                MessageTable.LINK_PREVIEWS to GenZappDatabase.messages.getSerializedLinkPreviews(attachmentMap, linkPreviews)
              ),
              "${MessageTable.ID} = ?",
              SqlUtil.buildArgs(messageRowId)
            )
    ...
    if (this.stickerMessage != null) {
      ...
      if (attachment != null) {
        followUp = { messageRowId ->
          GenZappDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(attachment), emptyList())
        }
      }
    }
  ...
  private fun tryRestorePayment(chatItem: ChatItem, chatRecipientId: RecipientId): UUID? {
    ...
    return GenZappDatabase.payments.restoreFromBackup(
      chatRecipientId,
    ...
    when {
      updateMessage.simpleUpdate != null -> {
        ...
        typeFlags = when (updateMessage.simpleUpdate.type) {
          ...
          SimpleChatUpdate.Type.JOINED_GenZapp -> MessageTypes.JOINED_TYPE or typeWithoutBase
  ...
  private fun ContentValues.addFailedPaymentNotification(chatItem: ChatItem, amount: Money, fee: Money, chatRecipientId: RecipientId) {
    val uuid = GenZappDatabase.payments.restoreFromBackup(
      chatRecipientId,
      0,
      0,
  ...
  private fun FilePointer?.toLocalAttachment(voiceNote: Boolean, borderless: Boolean, gif: Boolean, wasDownloaded: Boolean, stickerLocator: StickerLocator? = null, contentType: String? = this?.contentType, fileName: String? = this?.fileName, uuid: ByteString? = null): Attachment? {
    ...
    if (attachmentLocator != null) {
      val GenZappAttachmentPointer = GenZappServiceAttachmentPointer(
        attachmentLocator.cdnNumber,
        GenZappServiceAttachmentRemoteId.from(attachmentLocator.cdnKey),
        contentType,
      ...
      return PointerAttachment.forPointer(
        pointer = Optional.of(GenZappAttachmentPointer),
