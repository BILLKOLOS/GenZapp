...
class ChatItemExportIterator(private val cursor: Cursor, private val batchSize: Int, private val archiveMedia: Boolean) : Iterator<ChatItem?>, Closeable {
  ...
  override fun next(): ChatItem? {
    ...
    val reactionsById: Map<Long, List<ReactionRecord>> = GenZappDatabase.reactions.getReactionsForMessages(records.keys)
    val mentionsById: Map<Long, List<Mention>> = GenZappDatabase.mentions.getMentionsForMessages(records.keys)
    val attachmentsById: Map<Long, List<DatabaseAttachment>> = GenZappDatabase.attachments.getAttachmentsForMessages(records.keys)
    val groupReceiptsById: Map<Long, List<GroupReceiptTable.GroupReceiptInfo>> = GenZappDatabase.groupReceipts.getGroupReceiptInfoForMessages(records.keys)
    ...
    for ((id, record) in records) {
      ...
      when {
        ...
        MessageTypes.isJoinedType(record.type) -> {
          builder.updateMessage = simpleUpdate(SimpleChatUpdate.Type.JOINED_GenZapp)
        }
  ...
  private fun BackupMessageRecord.toGroupUpdate(): ChatUpdateMessage? {
    ...
    return if (groupChange != null) {
      ...
      try {
        ...
        ChatUpdateMessage(
          groupChange = GroupsV2UpdateMessageConverter.translateDecryptedChange(selfIds = GenZappStore.account.getServiceIds(), context)
        )
      } catch (e: IOException) {
        null
      }
  ...
  private fun BackupMessageRecord.toPaymentNotificationUpdate(): PaymentNotification {
    ...
    val payment = if (paymentUuid != null) {
      GenZappDatabase.payments.getPayment(paymentUuid)
    } else {
      null
    }
  ...
  private fun List<Mention>.toBackupBodyRanges(): List<BackupBodyRange> {
    return this.map {
      BackupBodyRange(
        start = it.start,
        length = it.length,
        mentionAci = GenZappDatabase.recipients.getRecord(it.recipientId).aci?.toByteString()
      )
    }
  }
