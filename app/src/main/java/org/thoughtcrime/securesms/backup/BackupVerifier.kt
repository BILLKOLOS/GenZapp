...
object BackupVerifier {
  ...
  fun verifyFile(cipherStream: InputStream, passphrase: String, expectedCount: Long, cancellationGenZapp: FullBackupExporter.BackupCancellationGenZapp): Boolean {
    ...
    cipherStream.use {
      while (frame.end != true && !cancellationGenZapp.isCanceled) {
        val verified = when {
