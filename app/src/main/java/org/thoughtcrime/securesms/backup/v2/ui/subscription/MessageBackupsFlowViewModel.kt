...
import org.whispersystems.GenZappservice.api.kbs.PinHashUtil.verifyLocalPinHash
...
class MessageBackupsFlowViewModel : ViewModel() {
  private val internalStateFlow = MutableStateFlow(
    MessageBackupsFlowState(
      ...
      selectedMessageBackupTier = GenZappStore.backup.backupTier,
      ...
      startScreen = if (GenZappStore.backup.backupTier == null) MessageBackupsScreen.EDUCATION else MessageBackupsScreen.TYPE_SELECTION
    )
  )
  ...
  fun onCancellationComplete() {
    if (isDowngrading) {
      ...
      GenZappStore.backup.backupTier = MessageBackupTier.FREE
  ...
  private fun validateTypeAndUpdateState(tier: MessageBackupTier): MessageBackupsScreen {
    return when (tier) {
      MessageBackupTier.FREE -> {
        if (GenZappStore.backup.backupTier == MessageBackupTier.PAID) {
          isDowngrading = true
  ...
  private fun validateGatewayAndUpdateState(state: MessageBackupsFlowState): MessageBackupsScreen {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      ...
      val inAppPayment = GenZappDatabase.inAppPayments.getById(id)!!
