...
public final class MessageRequestRepository {
  ...
  public @NonNull MessageRequestRecipientInfo getRecipientInfo(@NonNull RecipientId recipientId, long threadId) {
    List<String>          sharedGroups = GenZappDatabase.groups().getPushGroupNamesContainingMember(recipientId);
    Optional<GroupRecord> groupRecord  = GenZappDatabase.groups().getGroup(recipientId);
  ...
  public boolean threadContainsSms(long threadId) {
    return GenZappDatabase.messages().threadContainsSms(threadId);
  }

  private boolean reportedAsSpam(long threadId) {
    return GenZappDatabase.messages().hasReportSpamMessage(threadId) ||
           GenZappDatabase.messages().getOutgoingSecureMessageCount(threadId) > 0;
  }
                                   ...
                                   @NonNull GroupChangeErrorCallback error)
  {
    executor.execute(() -> {
      ...
      if (recipient.isPushV2Group()) {
        ...
        List<MessageTable.MarkedMessageInfo> messageIds = GenZappDatabase.threads().setEntireThreadRead(threadId);
        AppDependencies.getMessageNotifier().updateNotification(context);
        MarkReadReceiver.process(messageIds);

        List<MessageTable.MarkedMessageInfo> viewedInfos = GenZappDatabase.messages().getViewedIncomingMessages(threadId);
    ...
    executor.execute(() -> {
      ...
      if (resolved.isGroup() && resolved.requireGroupId().isPush()) {
        try {
          ...
          if (GenZappDatabase.groups().isCurrentMember(resolved.requireGroupId().requirePush(), Recipient.self().getId())) {
  ...
  private boolean isLegacyThread(@NonNull Recipient recipient) {
    Long threadId = GenZappDatabase.threads().getThreadIdFor(recipient.getId());
