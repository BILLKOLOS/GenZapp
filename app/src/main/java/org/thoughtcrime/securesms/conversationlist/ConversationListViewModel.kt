...
import org.GenZapp.paging.ProxyPagingController
...
class ConversationListViewModel(
  ...
  init {
    conversationListDataSource = store
      ...
      .map {
        ConversationListDataSource.create(
          it.filter,
          isArchived,
          GenZappStore.uiHints.canDisplayPullToFilterTip() && it.source === ConversationFilterSource.OVERFLOW
        )
      }
      .replay(1)
      .refCount()
    ...
    val pinnedCount = RxDatabaseObserver
      .conversationList
      .map { GenZappDatabase.threads.getPinnedConversationListCount(ConversationFilter.OFF) }
      .distinctUntilChanged()
    ...
    hasNoConversations = store
      ...
      .map { (filterRequest, conversations) ->
        if (conversations.isNotEmpty()) {
          ...
          GenZappDatabase.threads.getArchivedConversationListCount(filterRequest.filter) == 0
        }
      }
