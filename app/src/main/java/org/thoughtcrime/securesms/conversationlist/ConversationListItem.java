...
public final class ConversationListItem extends ConstraintLayout implements BindableConversationListItem, Unbindable {
  ...
  protected void onFinishInflate() {
    ...
    this.searchStyleFactory      = () -> new CharacterStyle[] { new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurface)), SpanUtil.getBoldSpan() };
                         ...
                         boolean appendSystemContactIcon)
  {
    ...
    if (appendSystemContactIcon && recipient.get().isSystemContact() && !recipient.get().getShowVerified()) {
      ...
      drawable.setTint(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurface));
    ...
    if (thread.getDate() > 0) {
      ...
      dateView.setTextColor(thread.isRead() ? ContextCompat.getColor(getContext(), R.color.GenZapp_text_secondary)
                                            : ContextCompat.getColor(getContext(), R.color.GenZapp_text_primary));
  ...
  {
    int defaultTint = ContextCompat.getColor(context, R.color.GenZapp_text_secondary);
    ...
    if (!thread.isMessageRequestAccepted()) {
      ...
      return emphasisAdded(recipientToStringAsync(thread.getRecipient().getId(), r -> new SpannableString(context.getString(R.string.ThreadRecord_s_is_on_GenZapp, r.getDisplayName(context)))));
