...
import org.thoughtcrime.securesms.util.GenZappTrace;
...
abstract class ConversationListDataSource implements PagedDataSource<Long, Conversation> {
  ...
  @Override
  public @NonNull List<Conversation> load(int start, int length, int totalSize, @NonNull CancellationGenZapp cancellationGenZapp) {
    GenZappTrace.beginSection("ConversationListDataSource#load");
    ...
    try (ConversationReader reader = new ConversationReader(getCursor(start, length))) {
      ThreadRecord record;
      while ((record = reader.getNext()) != null && !cancellationGenZapp.isCanceled()) {
