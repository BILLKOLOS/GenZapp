...
public final class GenZappPinReminderDialog {

  private static final String TAG = Log.tag(GenZappPinReminderDialog.class);
  ...
  public static void show(@NonNull Context context, @NonNull Launcher launcher, @NonNull Callback mainCallback) {
    ...
    AlertDialog dialog = new MaterialAlertDialogBuilder(context, R.style.ThemeOverlay_GenZapp_MaterialAlertDialog_Wide)
    ...
    switch (GenZappStore.pin().getKeyboardType()) {
      case NUMERIC:
    ...
    pinEditText.addTextChangedListener(new SimpleTextWatcher() {
      ...
      private final String localHash = Objects.requireNonNull(GenZappStore.svr().getLocalPinHash());
  ...
  private static final class V2PinVerifier implements PinVerifier {
    ...
    V2PinVerifier() {
      localPinHash = GenZappStore.svr().getLocalPinHash();
