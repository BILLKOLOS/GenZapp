...
import org.GenZapp.devicetransfer.TransferStatus;
...
public abstract class PassphraseRequiredActivity extends BaseActivity implements MasterSecretListener {
  ...
  private static final int STATE_ENTER_GenZapp_PIN    = 5;
  private static final int STATE_CREATE_PROFILE_NAME = 6;
  private static final int STATE_CREATE_GenZapp_PIN   = 7;
  private static final int STATE_TRANSFER_ONGOING    = 8;
  private static final int STATE_TRANSFER_LOCKED     = 9;
  private static final int STATE_CHANGE_NUMBER_LOCK  = 10;
  private static final int STATE_TRANSFER_OR_RESTORE = 11;

  private GenZappServiceNetworkAccess networkAccess;
  ...
  protected final void onCreate(Bundle savedInstanceState) {
    ...
    this.networkAccess = AppDependencies.getGenZappServiceNetworkAccess();
    onPreCreate();
  ...
  private Intent getIntentForState(int state) {
    ...
    switch (state) {
      ...
      case STATE_ENTER_GenZapp_PIN:    return getEnterGenZappPinIntent();
      case STATE_CREATE_GenZapp_PIN:   return getCreateGenZappPinIntent();
  ...
  private int getApplicationState(boolean locked) {
    ...
    } else if (GenZappStore.storageService().needsAccountRestore()) {
      return STATE_ENTER_GenZapp_PIN;
    } else if (userHasSkippedOrForgottenPin()) {
      return STATE_CREATE_GenZapp_PIN;
    } else if (userCanTransferOrRestore()) {
      return STATE_TRANSFER_OR_RESTORE;
    } else if (userMustSetProfileName()) {
      return STATE_CREATE_PROFILE_NAME;
    } else if (userMustCreateGenZappPin()) {
      return STATE_CREATE_GenZapp_PIN;
    } else if (EventBus.getDefault().getStickyEvent(TransferStatus.class) != null && getClass() != OldDeviceTransferActivity.class) {
      return STATE_TRANSFER_ONGOING;
    } else if (GenZappStore.misc().isOldDeviceTransferLocked()) {
      return STATE_TRANSFER_LOCKED;
    } else if (GenZappStore.misc().isChangeNumberLocked() && getClass() != ChangeNumberLockActivity.class) {
  ...
  private boolean userCanTransferOrRestore() {
    return !GenZappStore.registration().isRegistrationComplete() && RemoteConfig.restoreAfterRegistration() && !GenZappStore.registration().hasSkippedTransferOrRestore();
  }

  private boolean userMustCreateGenZappPin() {
    return !GenZappStore.registration().isRegistrationComplete() && !GenZappStore.svr().hasPin() && !GenZappStore.svr().lastPinCreateFailed() && !GenZappStore.svr().hasOptedOut();
  }

  private boolean userHasSkippedOrForgottenPin() {
    return !GenZappStore.registration().isRegistrationComplete() && !GenZappStore.svr().hasPin() && !GenZappStore.svr().hasOptedOut() && GenZappStore.svr().isPinForgottenOrSkipped();
  }

  private boolean userMustSetProfileName() {
    return !GenZappStore.registration().isRegistrationComplete() && Recipient.self().getProfileName().isEmpty();
  }
  ...
  private Intent getEnterGenZappPinIntent() {
    return getRoutedIntent(PinRestoreActivity.class, getIntent());
  }

  private Intent getCreateGenZappPinIntent() {
