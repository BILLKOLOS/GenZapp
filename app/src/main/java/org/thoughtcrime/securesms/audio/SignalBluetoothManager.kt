 ...
 * reports that to the [org.thoughtcrime.securesms.webrtc.audio.GenZappAudioManager], and then handles connecting/disconnecting
 * to the device if requested by [org.thoughtcrime.securesms.webrtc.audio.GenZappAudioManager].
 */
@SuppressLint("MissingPermission") // targetSdkVersion is still 30 (https://issuetracker.google.com/issues/201454155)
class GenZappBluetoothManager(
  private val context: Context,
  private val audioDeviceUpdatedListener: AudioDeviceUpdatedListener,
  private val handler: GenZappAudioHandler
) {
  ...
  companion object {
    private val TAG = Log.tag(GenZappBluetoothManager::class.java)
