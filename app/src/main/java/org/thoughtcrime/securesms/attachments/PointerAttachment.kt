...
import org.GenZapp.core.util.Base64.encodeWithPadding
...
class PointerAttachment : Attachment {
  ...
  companion object {
    @JvmStatic
    fun forPointers(pointers: Optional<List<GenZappServiceAttachment>>): List<Attachment> {
    ...
    @JvmStatic
    @JvmOverloads
    fun forPointer(pointer: Optional<GenZappServiceAttachment>, stickerLocator: StickerLocator? = null, fastPreflightId: String? = null, transferState: Int = AttachmentTable.TRANSFER_PROGRESS_PENDING): Optional<Attachment> {
    ...
    fun forPointer(quotedAttachment: DataMessage.Quote.QuotedAttachment): Optional<Attachment> {
      val thumbnail: GenZappServiceAttachment? = try {
        if (quotedAttachment.thumbnail != null) {
          AttachmentPointerUtil.createGenZappAttachmentPointer(quotedAttachment.thumbnail)
        } else {
          null
        }
