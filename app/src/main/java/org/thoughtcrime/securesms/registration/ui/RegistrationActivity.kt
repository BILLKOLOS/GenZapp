...
class RegistrationActivity : BaseActivity() {
  ...
  private fun handleSuccessfulVerify() {
    if (GenZappStore.misc.hasLinkedDevices) {
      GenZappStore.misc.shouldShowLinkedDevicesReminder = sharedViewModel.isReregister
    }

    if (GenZappStore.storageService.needsAccountRestore()) {
      ...
      val startIntent = MainActivity.clearTop(this).apply {
        ...
        } else if (!GenZappStore.registration.hasSkippedTransferOrRestore() && RemoteConfig.messageBackups) {
