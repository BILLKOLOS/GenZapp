  ...
  private fun handleNonNormalizedNumberError(originalNumber: String, normalizedNumber: String, mode: RegistrationRepository.Mode) {
    try {
      ...
      MaterialAlertDialogBuilder(requireContext()).apply {
        ...
        setNeutralButton(R.string.RegistrationActivity_contact_GenZapp_support) { dialogInterface, _ ->
          val subject = getString(R.string.RegistrationActivity_GenZapp_android_phone_number_format)
          val body = SupportEmailUtil.generateSupportEmailBody(requireContext(), R.string.RegistrationActivity_GenZapp_android_phone_number_format, null, null)
