...
class RegistrationViewModel : ViewModel() {
  ...
  fun onBackupSuccessfullyRestored() {
    ...
    store.update {
      it.copy(registrationCheckpoint = RegistrationCheckpoint.BACKUP_RESTORED_OR_SKIPPED, recoveryPassword = GenZappStore.svr.recoveryPassword, canSkipSms = recoveryPassword != null, isReRegister = true)
    }
  }
  ...
  fun verifyReRegisterWithPin(context: Context, pin: String, wrongPinHandler: () -> Unit) {
    ...
    if (RegistrationRepository.canUseLocalRecoveryPassword()) {
      if (RegistrationRepository.doesPinMatchLocalHash(pin)) {
        ...
        viewModelScope.launch(context = coroutineExceptionHandler) {
          verifyReRegisterInternal(context, pin, GenZappStore.svr.getOrCreateMasterKey())
          setInProgress(false)
        }
      } else {
  ...
  private suspend fun verifyCodeInternal(context: Context, registrationLocked: Boolean, pin: String?) {
    ...
    if (result is RegisterAccountResult.RegistrationLocked) {
      ...
      reglock = true
      if (pin == null && GenZappStore.svr.registrationLockToken != null) {
        Log.d(TAG, "Retrying registration with stored credentials.")
        result = RegistrationRepository.registerAccount(context, sessionId, registrationData, GenZappStore.svr.pin) { GenZappStore.svr.getOrCreateMasterKey() }
  ...
  private fun matchesSavedE164(e164: String?): Boolean {
    return if (e164 == null) {
      ...
      e164 == GenZappStore.account.e164
    }
  }
  ...
  private suspend fun getRegistrationData(): RegistrationData {
    ...
    val recoveryPassword = if (currentState.sessionId == null) GenZappStore.svr.getRecoveryPassword() else null
