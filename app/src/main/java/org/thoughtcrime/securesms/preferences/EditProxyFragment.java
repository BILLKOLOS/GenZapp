...
public class EditProxyFragment extends Fragment {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    this.proxyText.setText(Optional.ofNullable(GenZappStore.proxy().getProxy()).map(GenZappProxy::getHost).orElse(""));
    this.proxySwitch.setChecked(GenZappStore.proxy().isProxyEnabled());
    ...
    description.setOnLinkClickListener(v -> CommunicationActions.openBrowserLink(requireContext(), "https://support.GenZapp.org/hc/articles/360056052052"));
  ...
  private void presentProxyState(@NonNull WebSocketConnectionState proxyState) {
    if (GenZappStore.proxy().getProxy() != null) {
      switch (proxyState) {
        case DISCONNECTED:
        case DISCONNECTING:
        case CONNECTING:
          proxyStatus.setText(R.string.preferences_connecting_to_proxy);
          proxyStatus.setTextColor(getResources().getColor(R.color.GenZapp_text_secondary));
          break;
        case CONNECTED:
          proxyStatus.setText(R.string.preferences_connected_to_proxy);
          proxyStatus.setTextColor(getResources().getColor(R.color.GenZapp_accent_green));
          break;
        case AUTHENTICATION_FAILED:
        case FAILED:
          proxyStatus.setText(R.string.preferences_connection_failed);
          proxyStatus.setTextColor(getResources().getColor(R.color.GenZapp_alert_primary));
          break;
      }
    } else {
      proxyStatus.setText("");
  ...
  private void presentEvent(@NonNull EditProxyViewModel.Event event) {
    switch (event) {
      case PROXY_SUCCESS:
        ...
        proxyText.setText(Optional.ofNullable(GenZappStore.proxy().getProxy()).map(GenZappProxy::getHost).orElse(""));
  ...
  private void onShareClicked() {
    String link = GenZappProxyUtil.generateProxyUrl(proxyText.getText().toString());
  ...
  private void onProxyTextChanged(@NonNull String text) {
    if (Util.isEmpty(text)) {
      ...
      String trueHost = GenZappProxyUtil.convertUserEnteredAddressToHost(proxyText.getText().toString());
      if (GenZappStore.proxy().isProxyEnabled() && trueHost.equals(GenZappStore.proxy().getProxyHost())) {
        proxyStatus.setVisibility(View.VISIBLE);
      } else {
        proxyStatus.setVisibility(View.INVISIBLE);
      }
