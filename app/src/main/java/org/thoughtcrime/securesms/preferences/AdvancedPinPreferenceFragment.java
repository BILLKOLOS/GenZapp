...
public class AdvancedPinPreferenceFragment extends ListSummaryPreferenceFragment {
  ...
  private void onPreferenceChanged(boolean enabled) {
    boolean hasRegistrationLock = GenZappStore.svr().isRegistrationLockEnabled();
    ...
    if (!enabled && hasRegistrationLock) {
      new MaterialAlertDialogBuilder(requireContext())
                     ...
                     .show();
    } else if (!enabled && GenZappStore.payments().mobileCoinPaymentsEnabled() && !GenZappStore.payments().getUserConfirmedMnemonic()) {
      new MaterialAlertDialogBuilder(requireContext())
