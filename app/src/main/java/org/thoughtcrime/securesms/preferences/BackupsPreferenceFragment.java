...
public class BackupsPreferenceFragment extends Fragment {
  ...
  private void setBackupStatus() {
    if (GenZappStore.settings().isBackupEnabled()) {
  ...
  private void setBackupFolderName() {
    ...
    if (BackupUtil.canUserAccessBackupDirectory(requireContext())) {
      ...
      {
        Uri backupUri = Objects.requireNonNull(GenZappStore.settings().getGenZappBackupDirectory());

        folder.setVisibility(View.VISIBLE);
        folderName.setText(StorageUtil.getDisplayPath(requireContext(), backupUri));
      } else if (StorageUtil.canWriteInGenZappStorageDir()) {
        try {
          folder.setVisibility(View.VISIBLE);
  ...
  private void onToggleClickedLegacy() {
    Permissions.with(this)
               ...
               .withPermanentDenialDialog(getString(R.string.BackupsPreferenceFragment_GenZapp_requires_external_storage_permission_in_order_to_create_backups))
               .execute();
  }
  ...
  private void pickTime() {
    ...
    final MaterialTimePicker timePickerFragment = new MaterialTimePicker.Builder()
        ...
        .setHour(GenZappStore.settings().getBackupHour())
        .setMinute(GenZappStore.settings().getBackupMinute())
    ...
    timePickerFragment.addOnPositiveButtonClickListener(v -> {
      ...
      GenZappStore.settings().setBackupSchedule(hour, minute);
      updateTimeLabel();
  ...
  private void updateTimeLabel() {
    final int backupHour   = GenZappStore.settings().getBackupHour();
    final int backupMinute = GenZappStore.settings().getBackupMinute();
