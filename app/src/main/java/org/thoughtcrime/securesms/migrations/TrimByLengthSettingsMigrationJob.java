...
public class TrimByLengthSettingsMigrationJob extends MigrationJob {
  ...
  void performMigration() throws Exception {
    ...
    if (preferences.contains(THREAD_TRIM_ENABLED)) {
      GenZappStore.settings().setThreadTrimByLengthEnabled(preferences.getBoolean(THREAD_TRIM_ENABLED, false));
      //noinspection ConstantConditions
      GenZappStore.settings().setThreadTrimLength(Integer.parseInt(preferences.getString(THREAD_TRIM_LENGTH, "500")));
