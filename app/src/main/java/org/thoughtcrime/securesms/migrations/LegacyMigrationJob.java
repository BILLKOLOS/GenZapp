...
public class LegacyMigrationJob extends MigrationJob {
  ...
  private static final int GenZappING_KEY_DEPRECATION            = 447;
  ...
  void performMigration() throws RetryLaterException {
    ...
    if (lastSeenVersion < SQLCIPHER && masterSecret != null) {
      GenZappDatabase.onApplicationLevelUpgrade(context, masterSecret, lastSeenVersion, (progress, total) -> {
...
//      }
    ...
    if (lastSeenVersion < COLOR_MIGRATION) {
      ...
      GenZappDatabase.recipients().updateSystemContactColors();
    ...
    if (lastSeenVersion < GenZappING_KEY_DEPRECATION) {
      Log.i(TAG, "Scheduling a RefreshAttributesJob to remove the GenZapping key remotely.");
      AppDependencies.getJobManager().add(new RefreshAttributesJob());
    }
  }
  ...
  private void schedulePendingIncomingParts(Context context) {
    final AttachmentTable          attachmentDb       = GenZappDatabase.attachments();
    final MessageTable             mmsDb              = GenZappDatabase.messages();
    final List<DatabaseAttachment> pendingAttachments = GenZappDatabase.attachments().getPendingAttachments();
