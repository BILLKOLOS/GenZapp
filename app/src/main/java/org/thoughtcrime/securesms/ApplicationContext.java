...
import org.conscrypt.ConscryptGenZapp;
import org.greenrobot.eventbus.EventBus;
import org.GenZapp.aesgcmprovider.AesGcmProvider;
import org.GenZapp.core.util.MemoryTracker;
import org.GenZapp.core.util.concurrent.AnrDetector;
import org.GenZapp.core.util.concurrent.GenZappExecutors;
import org.GenZapp.core.util.logging.AndroidLogger;
import org.GenZapp.core.util.logging.Log;
import org.GenZapp.core.util.logging.Scrubber;
import org.GenZapp.core.util.tracing.Tracer;
import org.GenZapp.glide.GenZappGlideCodecs;
...
import org.thoughtcrime.securesms.mms.GenZappGlideComponents;
import org.thoughtcrime.securesms.mms.GenZappGlideModule;
...
import org.thoughtcrime.securesms.util.GenZappUncaughtExceptionHandler;
...
public class ApplicationContext extends MultiDexApplication implements AppForegroundObserver.Listener {
  ...
  public void onCreate() {
    ...
    GenZappLocalMetrics.ColdStart.start();
    ...
    AppStartup.getInstance().addBlocking("sqlcipher-init", () -> {
                            ...
                            .addBlocking("scrubber", () -> Scrubber.setIdentifierHmacKeyProvider(() -> GenZappStore.svr().getOrCreateMasterKey().deriveLoggingKey()))
                            ...
                            .addBlocking("proxy-init", () -> {
                              if (GenZappStore.proxy().isProxyEnabled()) {
                                ...
                                ConscryptGenZapp.setUseEngineSocketByDefault(true);
                              }
                            })
                            .addBlocking("blob-provider", this::initializeBlobProvider)
                            .addBlocking("remote-config", RemoteConfig::init)
                            .addBlocking("ring-rtc", this::initializeRingRtc)
                            .addBlocking("glide", () -> GenZappGlideModule.setRegisterGlideComponents(new GenZappGlideComponents()))
                            ...
                            .addPostRender(() -> GenZappDatabase.messageLog().trimOldMessages(System.currentTimeMillis(), RemoteConfig.retryRespondMaxAge()))
    ...
    GenZappLocalMetrics.ColdStart.onApplicationCreateFinished();
  ...
  public void onForeground() {
    ...
    GenZappExecutors.BOUNDED.execute(() -> {
      ...
      long lastForegroundTime = GenZappStore.misc().getLastForegroundTime();
      ...
      GenZappStore.misc().setLastForegroundTime(currentTime);
    });
  ...
  public void checkBuildExpiration() {
    if (Util.getTimeUntilBuildExpiry(GenZappStore.misc().getEstimatedServerTime()) <= 0 && !GenZappStore.misc().isClientDeprecated()) {
  ...
  private void initializeSecurityProvider() {
    ...
    int conscryptPosition = Security.insertProviderAt(ConscryptGenZapp.newProvider(), 2);
  ...
  protected void initializeLogging() {
    ...
    GenZappProtocolLoggerProvider.setProvider(new CustomGenZappProtocolLogger());
    GenZappProtocolLoggerProvider.initializeLogging(BuildConfig.LIBGenZapp_LOG_LEVEL);
  ...
  private void initializeCrashHandling() {
    ...
    Thread.setDefaultUncaughtExceptionHandler(new GenZappUncaughtExceptionHandler(originalHandler));
  }

  private void initializeRx() {
    RxDogTag.install();
    RxJavaPlugins.setInitIoSchedulerHandler(schedulerSupplier -> Schedulers.from(GenZappExecutors.BOUNDED_IO, true, false));
    RxJavaPlugins.setInitComputationSchedulerHandler(schedulerSupplier -> Schedulers.from(GenZappExecutors.BOUNDED, true, false));
  ...
  private void initializeFirstEverAppLaunch() {
    if (TextSecurePreferences.getFirstInstallVersion(this) == -1) {
      if (!GenZappDatabase.databaseFileExists(this) || VersionTracker.getDaysSinceFirstInstalled(this) < 365) {
  ...
  private void initializeFcmCheck() {
    if (GenZappStore.account().isRegistered()) {
      long lastSetTime = GenZappStore.account().getFcmTokenLastSetTime();
      long nextSetTime = lastSetTime + TimeUnit.HOURS.toMillis(6);
      long now         = System.currentTimeMillis();

      if (GenZappStore.account().getFcmToken() == null || nextSetTime <= now || lastSetTime > now) {
  ...
  private void initializeCircumvention() {
    if (AppDependencies.getGenZappServiceNetworkAccess().isCensored()) {
      try {
  ...
  private void ensureProfileUploaded() {
    if (GenZappStore.account().isRegistered() && !GenZappStore.registration().hasUploadedProfile() && !Recipient.self().getProfileName().isEmpty()) {
  ...
  private void initializeCleanup() {
    int deleted = GenZappDatabase.attachments().deleteAbandonedPreuploadedAttachments();
    Log.i(TAG, "Deleted " + deleted + " abandoned attachments.");
  }

  private void initializeGlideCodecs() {
    GenZappGlideCodecs.setLogProvider(new org.GenZapp.glide.Log.Provider() {
      @Override
