...
class MediaSelectionRepository(context: Context) {
  ...
  fun send(
    ...
    return Maybe.create { emitter ->
      ...
      val storyType: StoryType = if (singleRecipient?.isDistributionList == true) {
        GenZappDatabase.distributionLists.getStoryType(singleRecipient.requireDistributionListId())
      ...
      if (MessageSender.isLocalSelfSend(context, singleRecipient, SendType.GenZapp)) {
  ...
  fun isLocalSelfSend(recipient: Recipient?): Boolean {
    return MessageSender.isLocalSelfSend(context, recipient, SendType.GenZapp)
  }
  ...
  private fun scheduleMessages(
    ...
    for (recipientId in recipientIds) {
      ...
      val thread = GenZappDatabase.threads.getOrCreateThreadIdFor(recipient)
      ...
      MessageSender.send(context, outgoingMessage, thread, SendType.GenZapp, null, null)
    }
  }
  ...
  private fun sendMessages(
    ...
    for (contact in contacts) {
      ...
      if (isStory && !recipient.isMyStory) {
        GenZappStore.story.setLatestStorySend(StorySend.newSend(recipient))
      }

      val storyType: StoryType = when {
        recipient.isDistributionList -> GenZappDatabase.distributionLists.getStoryType(recipient.requireDistributionListId())
