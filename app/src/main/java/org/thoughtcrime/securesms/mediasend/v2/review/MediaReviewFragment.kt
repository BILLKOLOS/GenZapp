...
class MediaReviewFragment : Fragment(R.layout.v2_media_review_fragment), ScheduleMessageTimePickerBottomSheet.ScheduleCallback, VideoThumbnailsRangeSelectorView.RangeDragListener {
  ...
  private var sentMediaQuality: SentMediaQuality = GenZappStore.settings.sentMediaQuality
  ...
  private fun presentSendButton(sendType: MessageSendType, recipient: Recipient?) {
    val sendButtonBackgroundTint = when {
      ...
      sendType.usesGenZappTransport -> ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnSecondaryContainer)
    ...
    val sendButtonForegroundTint = when {
      recipient != null -> ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnCustom)
      else -> ContextCompat.getColor(requireContext(), R.color.GenZapp_colorSecondaryContainer)
    }
