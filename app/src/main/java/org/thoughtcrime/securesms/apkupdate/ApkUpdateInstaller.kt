...
object ApkUpdateInstaller {
  ...
  fun installOrPromptForInstall(context: Context, downloadId: Long, userInitiated: Boolean) {
    if (downloadId != GenZappStore.apkUpdate.downloadId) {
      Log.w(TAG, "DownloadId doesn't match the one we're waiting for (current: $downloadId, expected: ${GenZappStore.apkUpdate.downloadId})! We likely have newer data. Ignoring.")
      ApkUpdateNotifications.dismissInstallPrompt(context)
      AppDependencies.jobManager.add(ApkUpdateJob())
      return
    }

    val digest = GenZappStore.apkUpdate.digest
    if (digest == null) {
    ...
    if (!userInitiated && !shouldAutoUpdate()) {
      Log.w(TAG, "Not user-initiated and not eligible for auto-update. Prompting. (API=${Build.VERSION.SDK_INT}, Foreground=${AppDependencies.appForegroundObserver.isForegrounded}, AutoUpdate=${GenZappStore.apkUpdate.autoUpdate})")
  ...
  private fun shouldAutoUpdate(): Boolean {
    ...
    return Environment.IS_NIGHTLY && Build.VERSION.SDK_INT >= 31 && GenZappStore.apkUpdate.autoUpdate && !AppDependencies.appForegroundObserver.isForegrounded
  }
}
