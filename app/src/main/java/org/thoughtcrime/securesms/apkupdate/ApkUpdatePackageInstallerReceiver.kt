...
class ApkUpdatePackageInstallerReceiver : BroadcastReceiver() {
  ...
  companion object {
    ...
    const val EXTRA_USER_INITIATED = "GenZapp.user_initiated"
    const val EXTRA_DOWNLOAD_ID = "GenZapp.download_id"
  }
  ...
  override fun onReceive(context: Context, intent: Intent?) {
    ...
    when (statusCode) {
      PackageInstaller.STATUS_SUCCESS -> {
        if (GenZappStore.apkUpdate.lastApkUploadTime != GenZappStore.apkUpdate.pendingApkUploadTime) {
          Log.i(TAG, "Update installed successfully! Updating our lastApkUploadTime to ${GenZappStore.apkUpdate.pendingApkUploadTime}")
          GenZappStore.apkUpdate.lastApkUploadTime = GenZappStore.apkUpdate.pendingApkUploadTime
