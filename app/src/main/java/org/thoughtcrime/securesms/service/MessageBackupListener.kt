...
class MessageBackupListener : PersistentAlarmManagerListener() {
  ...
  override fun getNextScheduledExecutionTime(context: Context): Long {
    return GenZappStore.backup.nextBackupTime
  }

  override fun onAlarm(context: Context, scheduledTime: Long): Long {
    if (GenZappStore.backup.areBackupsEnabled) {
      val timeSinceLastSync = System.currentTimeMillis() - GenZappStore.backup.lastMediaSyncTime
  ...
  companion object {
    ...
    fun schedule(context: Context?) {
      if (RemoteConfig.messageBackups && GenZappStore.backup.areBackupsEnabled) {
    ...
    fun setNextBackupTimeToIntervalFromNow(maxJitterSeconds: Int = BACKUP_JITTER_WINDOW_SECONDS): Long {
      ...
      next = when (GenZappStore.backup.backupFrequency) {
      ...
      GenZappStore.backup.nextBackupTime = nextTime
      return nextTime
    }
  }
}
