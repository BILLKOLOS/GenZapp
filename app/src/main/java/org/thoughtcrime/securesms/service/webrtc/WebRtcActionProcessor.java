...
import org.whispersystems.GenZappservice.api.messages.calls.AnswerMessage;
import org.whispersystems.GenZappservice.api.messages.calls.BusyMessage;
import org.whispersystems.GenZappservice.api.messages.calls.HangupMessage;
import org.whispersystems.GenZappservice.api.messages.calls.IceUpdateMessage;
 ...
 * <p>
 * Processing of the actions occur in by calls from {@link GenZappCallManager} and
...
public abstract class WebRtcActionProcessor {
  ...
  protected final @NonNull WebRtcServiceState handleSendOffer(@NonNull WebRtcServiceState currentState, @NonNull CallMetadata callMetadata, @NonNull OfferMetadata offerMetadata, boolean broadcast) {
    ...
    GenZappServiceCallMessage callMessage         = GenZappServiceCallMessage.forOffer(offerMessage, destinationDeviceId);
                                                            ...
                                                            @NonNull ReceivedOfferMetadata receivedOfferMetadata)
  {
    ...
    NotificationProfile activeProfile = NotificationProfiles.getActiveProfile(GenZappDatabase.notificationProfiles().getProfiles());
    ...
    try {
      ...
      byte[] localIdentityKey  = WebRtcUtil.getPublicKeyBytes(GenZappStore.account().getAciIdentityKey().getPublicKey().serialize());
      ...
      webRtcInteractor.getCallManager().receivedOffer(callMetadata.getCallId(),
                                                      ...
                                                      GenZappStore.account().getDeviceId(),
                                                      GenZappStore.account().isPrimaryDevice(),
                                                      remoteIdentityKey,
                                                      localIdentityKey);
                                                         ...
                                                         boolean broadcast)
  {
    ...
    Integer       destinationDeviceId    = broadcast ? null : callMetadata.getRemoteDevice();
    GenZappServiceCallMessage callMessage = GenZappServiceCallMessage.forAnswer(answerMessage, destinationDeviceId);
  ...
  protected final @NonNull WebRtcServiceState handleSendBusy(@NonNull WebRtcServiceState currentState, @NonNull CallMetadata callMetadata, boolean broadcast) {
    ...
    Integer                  destinationDeviceId = broadcast ? null : callMetadata.getRemoteDevice();
    GenZappServiceCallMessage callMessage         = GenZappServiceCallMessage.forBusy(busyMessage, destinationDeviceId);
                                                               ...
                                                               boolean broadcast)
  {
    ...
    Integer                  destinationDeviceId = broadcast ? null : callMetadata.getRemoteDevice();
    GenZappServiceCallMessage callMessage         = GenZappServiceCallMessage.forHangup(hangupMessage, destinationDeviceId);
  ...
  protected final @NonNull WebRtcServiceState handleSendIceCandidates(@NonNull WebRtcServiceState currentState, @NonNull CallMetadata callMetadata, boolean broadcast, @NonNull List<byte[]> iceCandidates) {
    ...
    Integer                  destinationDeviceId = broadcast ? null : callMetadata.getRemoteDevice();
    GenZappServiceCallMessage callMessage         = GenZappServiceCallMessage.forIceUpdates(iceUpdateMessages, destinationDeviceId);
                                                                  ...
                                                                  @NonNull RingUpdate ringUpdate)
  {
    ...
    try {
      ...
      GenZappDatabase.calls().insertOrUpdateGroupCallFromRingState(ringId,
