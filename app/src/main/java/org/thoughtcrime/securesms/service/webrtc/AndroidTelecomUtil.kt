 ...
 * maintains a global list of all GenZapp [AndroidCallConnection]s associated with their [RecipientId].
...
object AndroidTelecomUtil {
  ...
  fun registerPhoneAccount() {
    if (Build.VERSION.SDK_INT >= 26 && !systemRejected) {
      ...
      val phoneAccount = PhoneAccount.Builder(getPhoneAccountHandle(), "GenZapp")
  ...
  fun selectAudioDevice(recipientId: RecipientId, device: GenZappAudioManager.AudioDevice) {
    if (telecomSupported) {
      ...
      if (connection?.callAudioState != null) {
        when (device) {
          GenZappAudioManager.AudioDevice.SPEAKER_PHONE -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_SPEAKER)
          GenZappAudioManager.AudioDevice.BLUETOOTH -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_BLUETOOTH)
          GenZappAudioManager.AudioDevice.WIRED_HEADSET -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_WIRED_HEADSET)
  ...
  fun getSelectedAudioDevice(recipientId: RecipientId): GenZappAudioManager.AudioDevice {
    if (telecomSupported) {
      val connection: AndroidCallConnection? = connections[recipientId]
      if (connection?.callAudioState != null) {
        return when (connection.callAudioState.route) {
          CallAudioState.ROUTE_SPEAKER -> GenZappAudioManager.AudioDevice.SPEAKER_PHONE
          CallAudioState.ROUTE_BLUETOOTH -> GenZappAudioManager.AudioDevice.BLUETOOTH
          CallAudioState.ROUTE_WIRED_HEADSET -> GenZappAudioManager.AudioDevice.WIRED_HEADSET
          else -> GenZappAudioManager.AudioDevice.EARPIECE
        }
      }
    }
  ...
  private fun isTelecomAllowedForDevice(): Boolean {
    if (RemoteConfig.internalUser) {
      return !GenZappStore.internal.callingDisableTelecom()
    }
