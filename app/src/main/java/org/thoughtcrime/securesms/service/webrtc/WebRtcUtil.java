...
public final class WebRtcUtil {
  ...
  public static void enableSpeakerPhoneIfNeeded(@NonNull WebRtcInteractor webRtcInteractor, WebRtcServiceState currentState) {
    ...
    if (currentState.getLocalDeviceState().getActiveDevice() == GenZappAudioManager.AudioDevice.EARPIECE ||
        currentState.getLocalDeviceState().getActiveDevice() == GenZappAudioManager.AudioDevice.NONE &&
        currentState.getCallInfoState().getActivePeer() != null)
    {
      webRtcInteractor.setDefaultAudioDevice(currentState.getCallInfoState().requireActivePeer().getId(), GenZappAudioManager.AudioDevice.SPEAKER_PHONE, true);
    }
  }
