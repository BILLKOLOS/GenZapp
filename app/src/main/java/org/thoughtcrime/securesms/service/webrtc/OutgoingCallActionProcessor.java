...
public class OutgoingCallActionProcessor extends DeviceAwareActionProcessor {
  ...
  protected @NonNull WebRtcServiceState handleStartOutgoingCall(@NonNull WebRtcServiceState currentState, @NonNull RemotePeer remotePeer, @NonNull OfferMessage.Type offerType) {
    ...
    webRtcInteractor.setDefaultAudioDevice(remotePeer.getId(),
                                           isVideoCall ? GenZappAudioManager.AudioDevice.SPEAKER_PHONE : GenZappAudioManager.AudioDevice.EARPIECE,
                                           false);
    ...
    RecipientUtil.setAndSendUniversalExpireTimerIfNecessary(context, Recipient.resolved(remotePeer.getId()), GenZappDatabase.threads().getThreadIdIfExistsFor(remotePeer.getId()));

    GenZappDatabase.calls().insertOneToOneCall(remotePeer.getCallId().longValue(),
