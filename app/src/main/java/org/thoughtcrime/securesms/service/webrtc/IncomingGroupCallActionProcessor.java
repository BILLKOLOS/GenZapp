...
public final class IncomingGroupCallActionProcessor extends DeviceAwareActionProcessor {
                                                                  ...
                                                                  @NonNull CallManager.RingUpdate ringUpdate)
  {
    ...
    if (GenZappDatabase.calls().isRingCancelled(ringId, remotePeerGroup.getId()) && !updateForCurrentRingId) {
      try {
    ...
    if (shouldDisturbUserWithCall && GenZappStore.settings().isCallNotificationsEnabled()) {
      Uri                         ringtone     = recipient.resolve().getCallRingtone();
      RecipientTable.VibrateState vibrateState = recipient.resolve().getCallVibrate();

      if (ringtone == null) {
        ringtone = GenZappStore.settings().getCallRingtone();
      }

      webRtcInteractor.startIncomingRinger(ringtone, vibrateState == RecipientTable.VibrateState.ENABLED || (vibrateState == RecipientTable.VibrateState.DEFAULT && GenZappStore.settings().isCallVibrateEnabled()));
    }
