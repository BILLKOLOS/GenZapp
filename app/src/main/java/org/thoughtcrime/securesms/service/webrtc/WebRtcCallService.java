...
/**
 * Provide a foreground service for {@link GenZappCallManager} to leverage to run in the background when necessary. Also
 * provides devices listeners needed for during a call (i.e., bluetooth, power button).
 */
public final class WebRtcCallService extends Service implements GenZappAudioManager.EventListener {
  ...
  private final Executor               singleThreadExecutor   = GenZappExecutors.newCachedSingleThreadExecutor("GenZapp-webrtc-in-call", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD);

  private GenZappCallManager callManager;

  private NetworkReceiver                 networkReceiver;
  private PowerButtonReceiver             powerButtonReceiver;
  private UncaughtExceptionHandlerManager uncaughtExceptionHandlerManager;
  private PhoneStateListener              hangUpRtcOnDeviceCallAnswered;
  private GenZappAudioManager              GenZappAudioManager;
  ...
  public void onCreate() {
    Log.v(TAG, "onCreate");
    super.onCreate();
    this.callManager                   = AppDependencies.getGenZappCallManager();
  ...
  public void onDestroy() {
    ...
    if (GenZappAudioManager != null) {
      GenZappAudioManager.shutdown();
    }
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    switch (intent.getAction()) {
      ...
      case ACTION_SEND_AUDIO_COMMAND:
        setCallNotification();
        if (GenZappAudioManager == null) {
          GenZappAudioManager = GenZappAudioManager.create(this, this);
        }
        AudioManagerCommand audioCommand = Objects.requireNonNull(intent.getParcelableExtra(EXTRA_AUDIO_COMMAND));
        Log.i(TAG, "Sending audio command [" + audioCommand.getClass().getSimpleName() + "] to " + GenZappAudioManager.getClass().getSimpleName());
        GenZappAudioManager.handleCommand(audioCommand);
        return START_STICKY;
  ...
  private class HangUpRtcOnPstnCallAnsweredListener extends PhoneStateListener {
    ...
    public void onCallStateChanged(int state, @NonNull String phoneNumber) {
      ...
      if (state == TelephonyManager.CALL_STATE_OFFHOOK) {
        hangup();
        Log.i(TAG, "Device phone call ended GenZapp call.");
      }
    }
  ...
  private static class NetworkReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      AppDependencies.getGenZappCallManager().networkChange(activeNetworkInfo != null && activeNetworkInfo.isConnected());
      AppDependencies.getGenZappCallManager().dataModeUpdate();
    }
  }
  ...
  private static class PowerButtonReceiver extends BroadcastReceiver {
    ...
    public void onReceive(@NonNull Context context, @NonNull Intent intent) {
      if (Intent.ACTION_SCREEN_OFF.equals(intent.getAction())) {
        AppDependencies.getGenZappCallManager().screenOff();
      }
    }
  }
