...
import org.thoughtcrime.securesms.webrtc.audio.GenZappAudioManager.Companion.create
...
/**
 * Entry point for [GenZappCallManager] and friends to interact with the Android system.
 *
...
class ActiveCallManager(
...
) : GenZappAudioManager.EventListener {
  ...
  companion object {
    ...
    @JvmStatic
    fun denyCall() {
      AppDependencies.GenZappCallManager.denyCall()
    }

    @JvmStatic
    fun hangup() {
      AppDependencies.GenZappCallManager.localHangup()
    }
  ...
  private val callManager = AppDependencies.GenZappCallManager
  ...
  private var GenZappAudioManager: GenZappAudioManager? = null
  ...
  fun sendAudioCommand(audioCommand: AudioManagerCommand) {
    if (GenZappAudioManager == null) {
      GenZappAudioManager = create(application, this)
    }

    Log.i(TAG, "Sending audio command [" + audioCommand.javaClass.simpleName + "] to " + GenZappAudioManager?.javaClass?.simpleName)
  ...
  override fun onAudioDeviceChanged(activeDevice: GenZappAudioManager.AudioDevice, devices: Set<GenZappAudioManager.AudioDevice>) {
  ...
  class ActiveCallServiceReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
      ...
      when (intent?.action) {
        ACTION_DENY -> AppDependencies.GenZappCallManager.denyCall()
        ACTION_HANGUP -> AppDependencies.GenZappCallManager.localHangup()
      }
    }
  }
  ...
  private class NetworkReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      ...
      AppDependencies.GenZappCallManager.apply {
  ...
  private class PowerButtonReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      if (Intent.ACTION_SCREEN_OFF == intent.action) {
        AppDependencies.GenZappCallManager.screenOff()
      }
    }
  }
