...
public class ContactRepository {
  ...
  public ContactRepository(@NonNull Context context, @NonNull String noteToSelfTitle) {
    this.recipientTable  = GenZappDatabase.recipients();
    this.noteToSelfTitle = noteToSelfTitle;
    this.context           = context.getApplicationContext();
  }

  @WorkerThread
  public @NonNull Cursor queryGenZappContacts(@NonNull String query) {
    return queryGenZappContacts(new RecipientTable.ContactSearchQuery(query, true, ContactSearchSortOrder.NATURAL));
  }

  @WorkerThread
  public @NonNull Cursor queryGenZappContacts(@NonNull RecipientTable.ContactSearchQuery contactSearchQuery) {
    Cursor cursor = TextUtils.isEmpty(contactSearchQuery.getQuery()) ? recipientTable.getGenZappContacts(contactSearchQuery.getIncludeSelf())
                                                                     : recipientTable.queryGenZappContacts(contactSearchQuery);
  ...
  @WorkerThread
  public Cursor queryNonGenZappContacts(@NonNull String query) {
    Cursor cursor = TextUtils.isEmpty(query) ? recipientTable.getNonGenZappContacts()
                                             : recipientTable.queryNonGenZappContacts(query);
    return new SearchCursorWrapper(cursor, SEARCH_CURSOR_MAPPERS);
  }
