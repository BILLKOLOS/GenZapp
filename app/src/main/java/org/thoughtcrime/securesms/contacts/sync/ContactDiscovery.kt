...
import org.GenZapp.contacts.SystemContactsRepository.ContactIterator
import org.GenZapp.contacts.SystemContactsRepository.ContactPhoneDetails
...
object ContactDiscovery {
  ...
  fun refreshAll(context: Context, notifyOfNewUsers: Boolean) {
    if (TextUtils.isEmpty(GenZappStore.account.e164)) {
    ...
    if (!GenZappStore.registration.isRegistrationComplete) {
      if (GenZappStore.account.isRegistered && GenZappStore.svr.lastPinCreateFailed()) {
  ...
  private fun refreshRecipients(
    ...
    val preExistingRegisteredIds: Set<RecipientId> = GenZappDatabase.recipients.getRegistered()
    ...
    if (hasContactsPermissions(context)) {
      ...
      if (TextSecurePreferences.hasSuccessfullyRetrievedDirectory(context) && notifyOfNewUsers) {
        val systemContacts: Set<RecipientId> = GenZappDatabase.recipients.getSystemContacts().toSet()
  ...
  private fun notifyNewUsers(context: Context, newUserIds: Collection<RecipientId>) {
    if (!GenZappStore.settings.isNotifyWhenContactJoinsGenZapp) return
    ...
    Recipient.resolvedList(newUserIds)
      ...
      .map {
        ...
        GenZappDatabase.messages.insertMessageInbox(message)
      }
      .filter { it.isPresent }
      .map { it.get() }
      .forEach { result ->
  ...
  private fun syncRecipientsWithSystemContacts(
    ...
    val localNumber: String = GenZappStore.account.e164 ?: ""
    val handle = GenZappDatabase.recipients.beginBulkSystemContactUpdate(clearInfoForMissingContacts)
    try {
    ...
    if (NotificationChannels.supported()) {
      GenZappDatabase.recipients.getRecipientsWithNotificationChannels().use { reader ->
  ...
  private fun hasSession(id: RecipientId): Boolean {
    ...
    val protocolAddress = Recipient.resolved(id).requireServiceId().toProtocolAddress(GenZappServiceAddress.DEFAULT_DEVICE_ID)
