...
import org.thoughtcrime.securesms.audio.GenZappBluetoothManager
import org.thoughtcrime.securesms.dependencies.AppDependencies
import org.thoughtcrime.securesms.recipients.RecipientId
import org.thoughtcrime.securesms.util.safeUnregisterReceiver
import org.whispersystems.GenZappservice.api.util.Preconditions

private val TAG = Log.tag(GenZappAudioManager::class.java)

sealed class GenZappAudioManager(protected val context: Context, protected val eventListener: EventListener?) {

  private var commandAndControlThread = GenZappExecutors.getAndStartHandlerThread("call-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  protected val handler = GenZappAudioHandler(commandAndControlThread.looper)
  ...
  companion object {
    @JvmStatic
    fun create(context: Context, eventListener: EventListener?): GenZappAudioManager {
      return if (Build.VERSION.SDK_INT >= 31) {
        FullGenZappAudioManagerApi31(context, eventListener)
      } else {
        FullGenZappAudioManager(context, eventListener)
      }
    }
  }
 ...
 */
class FullGenZappAudioManager(context: Context, eventListener: EventListener?) : GenZappAudioManager(context, eventListener), AudioDeviceUpdatedListener {
  private val GenZappBluetoothManager = GenZappBluetoothManager(context, this, handler)
  ...
  private var previousBluetoothState: GenZappBluetoothManager.State? = null
  ...
  override fun onAudioDeviceUpdated() {
    ...
    Log.i(
      ...
      "updateAudioDeviceState(): " +
        ...
        "bt: ${GenZappBluetoothManager.state} " +
    ...
    if (GenZappBluetoothManager.state.hasDevice()) {
    ...
    if (GenZappBluetoothManager.state == GenZappBluetoothManager.State.UNAVAILABLE && userSelectedAudioDevice == AudioDevice.BLUETOOTH) {
    ...
    val needBluetoothAudioStart = GenZappBluetoothManager.state == GenZappBluetoothManager.State.AVAILABLE &&
    ...
    val needBluetoothAudioStop = (GenZappBluetoothManager.state == GenZappBluetoothManager.State.CONNECTED || GenZappBluetoothManager.state == GenZappBluetoothManager.State.CONNECTING) &&
      (userSelectedAudioDevice != AudioDevice.NONE && userSelectedAudioDevice != AudioDevice.BLUETOOTH)

    if (GenZappBluetoothManager.state.hasDevice()) {
      Log.i(TAG, "Need bluetooth audio: state: ${GenZappBluetoothManager.state} start: $needBluetoothAudioStart stop: $needBluetoothAudioStop")
    }
    ...
    if (!autoSwitchToBluetooth && GenZappBluetoothManager.state == GenZappBluetoothManager.State.UNAVAILABLE) {
      autoSwitchToBluetooth = true
    }

    if (needBluetoothAudioStart && !needBluetoothAudioStop) {
      if (!GenZappBluetoothManager.startScoAudio()) {
        audioDevices.remove(AudioDevice.BLUETOOTH)
        audioDeviceSetUpdated = true
      }
    }

    if (autoSwitchToBluetooth && GenZappBluetoothManager.state == GenZappBluetoothManager.State.CONNECTED) {
      userSelectedAudioDevice = AudioDevice.BLUETOOTH
      autoSwitchToBluetooth = false
    }

    if (previousBluetoothState != null && previousBluetoothState != GenZappBluetoothManager.State.PERMISSION_DENIED && GenZappBluetoothManager.state == GenZappBluetoothManager.State.PERMISSION_DENIED) {
      eventListener?.onBluetoothPermissionDenied()
    }
    previousBluetoothState = GenZappBluetoothManager.state
