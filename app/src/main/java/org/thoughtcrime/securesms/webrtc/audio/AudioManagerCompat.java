...
public abstract class AudioManagerCompat {
  ...
  public boolean isBluetoothHeadsetAvailable() {
    if (Build.VERSION.SDK_INT >= 31) {
      return audioManager.getAvailableCommunicationDevices().stream().anyMatch(it -> AudioDeviceMapping.fromPlatformType(it.getType()) == GenZappAudioManager.AudioDevice.BLUETOOTH);
    } else {
  ...
  public boolean isBluetoothConnected() {
    if (Build.VERSION.SDK_INT >= 31) {
      ...
      final GenZappAudioManager.AudioDevice audioDevice = AudioDeviceMapping.fromPlatformType(communicationDevice.getType());
      return GenZappAudioManager.AudioDevice.BLUETOOTH == audioDevice;
    } else {
  ...
  public @Nullable AudioDeviceInfo getConnectedBluetoothDevice() {
    return getAvailableCommunicationDevices().stream().filter(it -> AudioDeviceMapping.fromPlatformType(it.getType()) == GenZappAudioManager.AudioDevice.BLUETOOTH).findAny().orElse(null);
  }
