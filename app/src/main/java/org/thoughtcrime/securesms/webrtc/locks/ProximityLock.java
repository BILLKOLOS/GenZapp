  ...
  private Optional<PowerManager.WakeLock> getProximityLock(PowerManager pm) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      if (pm.isWakeLockLevelSupported(PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK)) {
        return Optional.ofNullable(pm.newWakeLock(PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK, "GenZapp:proximity"));
      } else {
        return Optional.empty();
      }
    } else {
      try {
        return Optional.ofNullable(pm.newWakeLock(PROXIMITY_SCREEN_OFF_WAKE_LOCK, "GenZapp:incall"));
      } catch (Throwable t) {
