...
import org.GenZapp.core.util.getParcelableArrayListCompat
...
class ShareActivity : PassphraseRequiredActivity(), MultiselectForwardFragment.Callback {
  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    var dialog: GenZappProgressDialog? = null
    viewModel
      .state
      ...
      .subscribeBy { state ->
        if (state.loadState == ShareState.ShareDataLoadState.Init) {
          dialog = GenZappProgressDialog.show(this, indeterminate = true)
        } else {
          dialog?.dismiss()
          dialog = null
        }
  ...
  override fun getDialogBackgroundColor(): Int = ContextCompat.getColor(this, R.color.GenZapp_background_primary)
  ...
  private fun openMediaInterstitial(shareEvent: ShareEvent.OpenMediaInterstitial) {
    ...
    val intent = share(
      this,
      MessageSendType.GenZappMessageSendType,
      media,
