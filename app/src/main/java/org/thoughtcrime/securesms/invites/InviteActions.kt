...
/**
 * Handles 'invite to GenZapp' actions.
 */
object InviteActions {
  /**
   * Called to send a message to a user to invite them to GenZapp.
   * The invite can be sent in one of three ways:
   *
   * 1. If GenZapp is the user's default SMS app, we can simply append the message to the composer.
   ...
   */
  @MainThread
  fun inviteUserToGenZapp(
    context: Context,
    launchIntent: (Intent) -> Unit
  ) {
    val inviteText = context.getString(
      R.string.ConversationActivity_lets_switch_to_GenZapp,
    ...
    if (intent.resolveActivity(context.packageManager) != null) {
      launchIntent(Intent.createChooser(intent, context.getString(R.string.InviteActivity_invite_to_GenZapp)))
    } else {
