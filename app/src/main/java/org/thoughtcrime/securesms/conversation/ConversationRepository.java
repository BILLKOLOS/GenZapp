...
public class ConversationRepository {
  ...
  public @NonNull ConversationData getConversationData(long threadId, @NonNull Recipient conversationRecipient, int jumpToPosition) {
    ThreadTable.ConversationMetadata    metadata                       = GenZappDatabase.threads().getConversationMetadata(threadId);
    int                                 threadSize                     = GenZappDatabase.messages().getMessageCountForThread(threadId);
    ...
    if (lastSeen > 0) {
      lastSeenPosition = GenZappDatabase.messages().getMessagePositionOnOrAfterTimestamp(threadId, lastSeen);
    }
    ...
    if (lastSeen == 0 && lastScrolled > 0) {
      lastScrolledPosition = GenZappDatabase.messages().getMessagePositionOnOrAfterTimestamp(threadId, lastScrolled);
    }
    ...
    if (!isMessageRequestAccepted) {
      ...
      if (conversationRecipient.isGroup()) {
        Optional<GroupRecord> group = GenZappDatabase.groups().getGroup(conversationRecipient.getId());
        if (group.isPresent()) {
  ...
  public void markGiftBadgeRevealed(long messageId) {
    GenZappExecutors.BOUNDED_IO.execute(() -> {
      List<MessageTable.MarkedMessageInfo> markedMessageInfo = GenZappDatabase.messages().setOutgoingGiftsRevealed(Collections.singletonList(messageId));
