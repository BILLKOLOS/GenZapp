...
public final class ConversationReactionOverlay extends FrameLayout {
  ...
  private void setupSelectedEmoji() {
    final List<String> emojis   = GenZappStore.emoji().getReactions();
    ...
    for (int i = 0; i < emojiViews.length; i++) {
          ...
          (isNotAtCustomIndexAndOldEmojiMatches || isAtCustomIndexAndOldEmojiExists))
      {
        ...
        if (isAtCustomIndex) {
          ...
          view.setImageEmoji(GenZappStore.emoji().getPreferredVariation(emojis.get(i)));
        }
  ...
  private void handleUpEvent() {
    if (selected != -1 && onReactionSelectedListener != null && backgroundView.getVisibility() == View.VISIBLE) {
      if (selected == customEmojiIndex) {
        ...
        onReactionSelectedListener.onReactionSelected(messageRecord, GenZappStore.emoji().getPreferredVariation(GenZappStore.emoji().getReactions().get(selected)));
      }
    } else {
      hide();
    }
