...
class ConversationDataSource(
  ...
  private val threadRecipient: Recipient by lazy {
    GenZappDatabase.threads.getRecipientForThreadId(threadId)!!
  }
  ...
  private fun getSizeInternal(): Int {
    ...
    return GenZappDatabase.messages.getMessageCountForThread(threadId)
  }

  override fun load(start: Int, length: Int, totalSize: Int, cancellationGenZapp: PagedDataSource.CancellationGenZapp): List<ConversationElement> {
    ...
    MessageTable.mmsReaderFor(GenZappDatabase.messages.getConversation(threadId, start.toLong(), length.toLong()))
      .use { reader ->
    ...
    if (RemoteConfig.messageBackups && GenZappStore.backup.restoreState.inProgress) {
  ...
  override fun load(key: ConversationElementKey): ConversationElement? {
    ...
    var record = GenZappDatabase.messages.getMessageRecordOrNull(key.id)
