...
import org.thoughtcrime.securesms.components.settings.app.subscription.donate.DonateToGenZappFragment
...
class ConversationFragment :
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    GenZappLocalMetrics.ConversationOpen.start()
  }
  ...
  private fun observeConversationThread() {
    ...
    disposables += viewModel
      ...
      .doOnSuccess { state ->
        ...
        GenZappLocalMetrics.ConversationOpen.onDataLoaded()
      ...
      .subscribeBy(onNext = {
        if (firstRender) {
          GenZappLocalMetrics.ConversationOpen.onDataPostedToMain()
        }
        ...
        adapter.submitList(it) {
          ...
          if (firstRender) {
            ...
            binding.conversationItemRecycler.doAfterNextLayout {
              GenZappLocalMetrics.ConversationOpen.onRenderFinished()
              doAfterFirstRender()
            }
          }
        }
  ...
  private fun presentStoryRing() {
    if (GenZappStore.story.isFeatureDisabled) {
      return
    }
  ...
  private fun presentInputReadyState(inputReadyState: InputReadyState) {
    ...
    when {
      ...
      inputReadyState.shouldShowInviteToGenZapp() -> disabledInputView.showAsInviteToGenZapp(requireContext(), inputReadyState.conversationRecipient, inputReadyState.threadContainsSms)
    ...
    composeText.setMessageSendType(MessageSendType.GenZappMessageSendType)
  }
  ...
  private fun calculateCharactersRemaining() {
    ...
    val characterState = MessageSendType.GenZappMessageSendType.calculateCharacters(messageBody)
  ...
  private fun presentNavigationIconForBubble() {
    binding.toolbar.navigationIcon = DrawableUtil.tint(
      ...
      ContextCompat.getColor(requireContext(), R.color.GenZapp_accent_primary)
    )

    binding.toolbar.setNavigationContentDescription(R.string.ConversationFragment__content_description_launch_GenZapp_button)
  ...
  private fun presentWallpaper(chatWallpaper: ChatWallpaper?) {
    ...
    val toolbarTint = ContextCompat.getColor(
      requireContext(),
      if (chatWallpaper != null) {
        R.color.GenZapp_colorNeutralInverse
      } else {
        R.color.GenZapp_colorOnSurface
      }
    )
    ...
    val navColor = if (wallpaperEnabled) {
      ...
      R.color.GenZapp_colorBackground
    }
    ...
    binding.scrollDateHeader.setTextColor(
      ContextCompat.getColor(
        requireContext(),
        if (wallpaperEnabled) R.color.sticky_header_foreground_wallpaper else R.color.GenZapp_colorOnSurfaceVariant
      )
    )
  ...
  private fun handleSendEditMessage() {
    ...
    if (GenZappStore.uiHints.hasNotSeenEditMessageBetaAlert()) {
  ...
  private fun sendMessage(
    ...
    if (GenZappStore.uiHints.hasNotSeenTextFormattingAlert() && bodyRanges != null && bodyRanges.ranges.isNotEmpty()) {
    ...
    val metricId = viewModel.recipientSnapshot?.let { if (it.isGroup) GenZappLocalMetrics.GroupMessageSend.start() else GenZappLocalMetrics.IndividualMessageSend.start() }
    ...
    SaveAttachmentUtil.showWarningDialog(requireContext(), attachments.size) { _, _ ->
      if (StorageUtil.canWriteToMediaStore()) {
        ...
        Permissions.with(this)
          ...
          .ifNecessary()
          .withPermanentDenialDialog(getString(R.string.MediaPreviewActivity_GenZapp_needs_the_storage_permission_in_order_to_write_to_external_storage_but_it_has_been_permanently_denied))
  ...
  private inner class ConversationItemClickListener : ConversationAdapter.ItemClickListener {
    ...
    override fun onInviteSharedContactClicked(choices: MutableList<Recipient>) {
      ...
      ContactUtil.selectRecipientThroughDialog(context, choices, Locale.getDefault()) { recipient: Recipient ->
        CommunicationActions.composeSmsThroughDefaultApp(
          context,
          recipient,
          getString(R.string.InviteActivity_lets_switch_to_GenZapp, getString(R.string.install_url))
        )
      }
    }
    ...
    private fun onDoubleTapToEdit(conversationMessage: ConversationMessage) {
      ...
      if (GenZappStore.uiHints.hasSeenDoubleTapEditEducationSheet) {
    ...
    override fun onDonateClicked() {
      requireActivity()
        ...
        .beginTransaction()
        .add(DonateToGenZappFragment.Dialog.create(InAppPaymentType.ONE_TIME_DONATION), "one_time_nav")
        .commitNow()
    }
    ...
    override fun onInviteToGenZappClicked() {
      InviteActions.inviteUserToGenZapp(
        requireContext(),
  ...
  private inner class ActivityResultCallbacks : ConversationActivityResultContracts.Callbacks {
    ...
    override fun onLocationSelected(place: GenZappPlace?, uri: Uri?) {
  ...
  private inner class ConversationBannerListener : ConversationBannerView.Listener {
    ...
    override fun changeBubbleSettingAction(disableSetting: Boolean) {
      GenZappStore.tooltips.markBubbleOptOutTooltipSeen()
  ...
  private inner class DisabledInputListener : DisabledInputView.Listener {
    ...
    override fun onInviteToGenZapp(recipient: Recipient) {
      InviteActions.inviteUserToGenZapp(
        context = requireContext(),
        launchIntent = this@ConversationFragment::startActivity
      )
    ...
    override fun onKey(v: View, keyCode: Int, event: KeyEvent): Boolean {
      if (event.action == KeyEvent.ACTION_DOWN) {
        if (keyCode == KeyEvent.KEYCODE_ENTER) {
          if (GenZappStore.settings.isEnterKeySends || event.isCtrlPressed) {
  ...
  private inner class InputPanelListener : InputPanel.Listener {
    ...
    override fun onRecorderPermissionRequired() {
      Permissions
        ...
        .ifNecessary()
        .withRationaleDialog(getString(R.string.ConversationActivity_allow_access_microphone), getString(R.string.ConversationActivity_to_send_voice_messages_allow_GenZapp_access_to_your_microphone), R.drawable.ic_mic_24)
        .withPermanentDenialDialog(getString(R.string.ConversationActivity_GenZapp_requires_the_microphone_permission_in_order_to_send_audio_messages), null, R.string.ConversationActivity_allow_access_microphone, R.string.ConversationActivity_GenZapp_to_send_audio_messages, this@ConversationFragment.parentFragmentManager)
        .onAnyDenied { Toast.makeText(this@ConversationFragment.requireContext(), R.string.ConversationActivity_GenZapp_needs_microphone_access_voice_message, Toast.LENGTH_LONG).show() }
        .execute()
    }
