...
class DraftRepository(
  ...
  private val threadTable: ThreadTable = GenZappDatabase.threads,
  private val draftTable: DraftTable = GenZappDatabase.drafts,
  private val saveDraftsExecutor: Executor = SerialMonoLifoExecutor(GenZappExecutors.BOUNDED),
  ...
  private fun getShareOrDraftDataInternal(lastShareDataTimestamp: Long): Pair<ShareOrDraftData?, Drafts?>? {
    ...
    if (conversationArguments?.canInitializeFromDatabase() == true) {
      ...
      val location: GenZappPlace? = drafts.firstOrNull { it.type == DraftTable.Draft.LOCATION }?.let { GenZappPlace.deserialize(it.value) }
      if (location != null) {
  ...
  private fun loadDraftQuoteInternal(serialized: String): ConversationMessage? {
    ...
    val messageRecord: MessageRecord = GenZappDatabase.messages.getMessageFor(quoteId.id, quoteId.author)?.let {
      if (it is MmsMessageRecord) {
        it.withAttachments(GenZappDatabase.attachments.getAttachmentsForMessage(it.id))
      } else {
        it
      }
    } ?: return null

    val threadRecipient = requireNotNull(GenZappDatabase.threads.getRecipientForThreadId(messageRecord.threadId))
  ...
  private fun loadDraftMessageEditInternal(serialized: String): ConversationMessage? {
    ...
    val messageRecord: MessageRecord = GenZappDatabase.messages.getMessageRecordOrNull(messageId.id) ?: return null
    val threadRecipient: Recipient = requireNotNull(GenZappDatabase.threads.getRecipientForThreadId(messageRecord.threadId))
  ...
  sealed interface ShareOrDraftData {
    ...
    data class SetLocation(val location: GenZappPlace, val draftText: CharSequence?) : ShareOrDraftData
