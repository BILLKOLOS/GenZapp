...
public final class SafetyNumberChangeRepository {
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipients(@NonNull List<ChangedRecipient> changedRecipients) {
    ...
    GenZappExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsInternal(changedRecipients)));
    return liveData;
  }
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipientsAndResend(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    MutableLiveData<TrustAndVerifyResult> liveData = new MutableLiveData<>();
    GenZappExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsAndResendInternal(changedRecipients, messageRecord)));
    return liveData;
  }
  ...
  private @Nullable MessageRecord getMessageRecord(Long messageId, String messageType) {
    try {
      return GenZappDatabase.messages().getMessageRecord(messageId);
  ...
  private TrustAndVerifyResult trustOrVerifyChangedRecipientsInternal(@NonNull List<ChangedRecipient> changedRecipients) {
    GenZappIdentityKeyStore identityStore = AppDependencies.getProtocolStore().aci().identities();
                                                                               ...
                                                                               @NonNull MessageRecord messageRecord)
  {
    ...
    try (GenZappSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      for (ChangedRecipient changedRecipient : changedRecipients) {
        GenZappProtocolAddress mismatchAddress = changedRecipient.getRecipient().requireServiceId().toProtocolAddress(GenZappServiceAddress.DEFAULT_DEVICE_ID);
        ...
        GenZappIdentityKeyStore.SaveResult result = AppDependencies.getProtocolStore().aci().identities().saveIdentity(mismatchAddress, newIdentityKey, true);

        Log.d(TAG, "Saving identity result: " + result);
        if (result == GenZappIdentityKeyStore.SaveResult.NO_CHANGE) {
          Log.i(TAG, "Archiving sessions explicitly as they appear to be out of sync.");
          AppDependencies.getProtocolStore().aci().sessions().archiveSessions(changedRecipient.getRecipient().getId(), GenZappServiceAddress.DEFAULT_DEVICE_ID);
          AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(mismatchAddress);
          GenZappDatabase.senderKeyShared().deleteAllFor(changedRecipient.getRecipient().getId());
        }
      }
    }
  ...
  private void processOutgoingMessageRecord(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    for (ChangedRecipient changedRecipient : changedRecipients) {
      ...
      if (messageRecord.isMms()) {
        GenZappDatabase.messages().removeMismatchedIdentity(messageRecord.getId(), id, identityKey);
