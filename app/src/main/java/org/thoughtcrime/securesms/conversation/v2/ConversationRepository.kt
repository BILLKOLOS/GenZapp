...
class ConversationRepository(
  ...
  fun getConversationThreadState(threadId: Long, requestedStartPosition: Int): Single<ConversationThreadState> {
    return Single.fromCallable {
      val recipient = GenZappDatabase.threads.getRecipientForThreadId(threadId)!!

      GenZappLocalMetrics.ConversationOpen.onMetadataLoadStarted()
      val metadata = oldConversationRepository.getConversationData(threadId, recipient, requestedStartPosition)
      GenZappLocalMetrics.ConversationOpen.onMetadataLoaded()
  ...
  fun sendMessage(
    ...
    val sendCompletable = Completable.create { emitter ->
      val splitMessage: MessageUtil.SplitResult = MessageUtil.getSplitMessage(
        applicationContext,
        body,
        MessageSendType.GenZappMessageSendType.calculateCharacters(body).maxPrimaryMessageSize
      )
      ...
      if (preUploadResults.isEmpty()) {
        MessageSender.send(
          ...
          message,
          threadId,
          MessageSender.SendType.GenZapp,
          metricId
        ) {
          emitter.onComplete()
        }
  ...
  fun setLastVisibleMessageTimestamp(threadId: Long, lastVisibleMessageTimestamp: Long) {
    GenZappExecutors.BOUNDED_IO.execute { GenZappDatabase.threads.setLastScrolled(threadId, lastVisibleMessageTimestamp) }
  }
  ...
  fun getQuotedMessagePosition(threadId: Long, quote: Quote): Single<Int> {
    return Single.fromCallable {
      GenZappDatabase.messages.getQuotedMessagePosition(threadId, quote.id, quote.author)
    }.subscribeOn(Schedulers.io())
  }

  fun getMessageResultPosition(threadId: Long, receivedTimestamp: Long): Single<Int> {
    return Single.fromCallable {
      GenZappDatabase.messages.getMessagePositionInConversation(threadId, receivedTimestamp)
    }.subscribeOn(Schedulers.io())
  }

  fun getNextMentionPosition(threadId: Long): Single<Int> {
    return Single.fromCallable {
      val details = GenZappDatabase.messages.getOldestUnreadMentionDetails(threadId)
      if (details == null) {
        -1
      } else {
        GenZappDatabase.messages.getMessagePositionInConversation(threadId, details.second(), details.first())
      }
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun getMessagePosition(threadId: Long, dateReceived: Long, authorId: RecipientId): Single<Int> {
    return Single.fromCallable {
      GenZappDatabase.messages.getMessagePositionInConversation(threadId, dateReceived, authorId)
    }.subscribeOn(Schedulers.io())
  }
  ...
  private fun getUnreadCount(threadId: Long): Int {
    return GenZappDatabase.messages.getUnreadCount(threadId)
  }

  private fun getUnreadMentionsCount(threadId: Long): Int {
    return GenZappDatabase.messages.getUnreadMentionCount(threadId)
  }
  ...
  fun getReminder(groupRecord: GroupRecord?): Maybe<Optional<Reminder>> {
    return Maybe.fromCallable {
      val reminder: Reminder? = when {
        ...
        isInBubble && !GenZappStore.tooltips.hasSeenBubbleOptOutTooltip() && Build.VERSION.SDK_INT > 29 -> {
  ...
  fun dismissRequestReviewState(threadRecipientId: RecipientId) {
    GenZappExecutors.BOUNDED_IO.execute {
      GenZappDatabase.nameCollisions.markCollisionsForThreadRecipientDismissed(threadRecipientId)
    }
  }
  ...
  fun getRequestReviewState(recipient: Recipient, group: GroupRecord?, messageRequest: MessageRequestState): Single<RequestReviewState> {
    return Single.fromCallable {
      ...
      if (group == null) {
        val recipientsToReview = GenZappDatabase.nameCollisions.getCollisionsForThreadRecipientId(recipient.id)
      ...
      if (group != null && group.isV2Group) {
        ...
        val duplicateRecipients: List<ReviewRecipient> = GenZappDatabase.nameCollisions.getCollisionsForThreadRecipientId(group.recipientId)
  ...
  fun setConversationMuted(recipientId: RecipientId, until: Long) {
    GenZappExecutors.BOUNDED_IO.execute { recipients.setMuted(recipientId, until) }
  }
  ...
  fun updateStickerLastUsedTime(stickerRecord: StickerRecord, timestamp: Duration) {
    GenZappExecutors.BOUNDED_IO.execute {
      GenZappDatabase.stickers.updateStickerLastUsedTime(stickerRecord.rowId, timestamp.inWholeMilliseconds)
    }
  }

  fun startExpirationTimeout(expirationInfos: List<MessageTable.ExpirationInfo>) {
    GenZappDatabase.messages.markExpireStarted(expirationInfos.map { it.id to it.expireStarted })
    AppDependencies.expiringMessageManager.scheduleDeletion(expirationInfos)
  }

  fun markLastSeen(threadId: Long) {
    GenZappExecutors.BOUNDED_IO.execute {
      GenZappDatabase.threads.setLastSeen(threadId)
    }
  }

  fun getEarliestMessageSentDate(threadId: Long): Single<Long> {
    return Single
      .fromCallable { GenZappDatabase.messages.getEarliestMessageSentDate(threadId) }
      .subscribeOn(Schedulers.io())
  }
