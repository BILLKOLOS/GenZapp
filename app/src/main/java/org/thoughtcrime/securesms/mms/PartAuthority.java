...
public class PartAuthority {
      ...
      throws IOException
  {
    ...
    try {
      switch (match) {
      case PART_ROW:          return GenZappDatabase.attachments().getAttachmentStream(new PartUriParser(uri).getPartId(), 0);
      case STICKER_ROW:       return GenZappDatabase.stickers().getStickerStream(ContentUris.parseId(uri));
      ...
      case THUMBNAIL_ROW:     return GenZappDatabase.attachments().getAttachmentThumbnailStream(new PartUriParser(uri).getPartId(), 0);
  ...
  public static @Nullable String getAttachmentFileName(@NonNull Context context, @NonNull Uri uri) {
    ...
    switch (match) {
    case PART_ROW:
      Attachment attachment = GenZappDatabase.attachments().getAttachment(new PartUriParser(uri).getPartId());
  ...
  public static @Nullable AttachmentTable.TransformProperties getAttachmentTransformProperties(@NonNull Uri uri) {
    ...
    switch (match) {
      case PART_ROW:
        return GenZappDatabase.attachments().getTransformProperties(new PartUriParser(uri).getPartId());
      default:
        return null;
    }
  }
