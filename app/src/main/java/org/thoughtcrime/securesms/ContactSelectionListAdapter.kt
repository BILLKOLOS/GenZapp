...
class ContactSelectionListAdapter(
  ...
  init {
    ...
    registerFactory(InviteToGenZappModel::class.java, LayoutFactory({ InviteToGenZappViewHolder(it, onClickCallbacks::onInviteToGenZappClicked) }, R.layout.contact_selection_invite_action_item))
  ...
  class InviteToGenZappModel : MappingModel<InviteToGenZappModel> {
    override fun areItemsTheSame(newItem: InviteToGenZappModel): Boolean = true
    override fun areContentsTheSame(newItem: InviteToGenZappModel): Boolean = true
  }
  ...
  private class InviteToGenZappViewHolder(itemView: View, onClickListener: () -> Unit) : MappingViewHolder<InviteToGenZappModel>(itemView) {
    init {
      itemView.setOnClickListener { onClickListener() }
    }

    override fun bind(model: InviteToGenZappModel) = Unit
  }
  ...
  class ArbitraryRepository : org.thoughtcrime.securesms.contacts.paged.ArbitraryRepository {
    ...
    enum class ArbitraryRow(val code: String) {
      NEW_GROUP("new-group"),
      INVITE_TO_GenZapp("invite-to-GenZapp"),
    ...
    override fun getMappingModel(arbitrary: ContactSearchData.Arbitrary): MappingModel<*> {
      return when (ArbitraryRow.fromCode(arbitrary.type)) {
        ...
        ArbitraryRow.INVITE_TO_GenZapp -> InviteToGenZappModel()
