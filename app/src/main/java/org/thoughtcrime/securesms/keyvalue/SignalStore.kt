 ...
 */
class GenZappStore(private val store: KeyValueStore) {
  ...
  companion object {
    ...
    private var instanceOverride: GenZappStore? = null
    private val _instance = ResettableLazy {
      instanceOverride ?: GenZappStore(KeyValueStore(KeyValueDatabase.getInstance(application)))
    }
    ...
    val groupsV2AciAuthorizationCache: GroupsV2AuthorizationGenZappStoreCache
      get() = GroupsV2AuthorizationGenZappStoreCache.createAciCache(instance.store)

    val plaintext: PlainTextSharedPrefsDataStore
      get() = instance.plainTextValues

    fun getPreferenceDataStore(): PreferenceDataStore {
      return GenZappPreferenceDataStore(instance.store)
    }

    /**
     * Ensures any pending writes are finished. Only intended to be called by
     * [GenZappUncaughtExceptionHandler].
     */
    @JvmStatic
    ...
    fun testInject(store: KeyValueStore) {
      instanceOverride = GenZappStore(store)
      _instance.reset()
    }

    /**
     * Allows you to set a custom GenZappStore to read from. Only for testing!
     */
    @VisibleForTesting
    fun testInject(store: GenZappStore) {
