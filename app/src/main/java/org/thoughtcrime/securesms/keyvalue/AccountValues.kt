...
class AccountValues internal constructor(store: KeyValueStore) : GenZappStoreValues(store) {
  ...
  /** The password for communicating with the GenZapp service. */
  ...
  /** Whether or not the user is registered with the GenZapp service. */
  ...
  var deviceId: Int by integerValue(KEY_DEVICE_ID, GenZappServiceAddress.DEFAULT_DEVICE_ID)

  val isPrimaryDevice: Boolean
    get() = deviceId == GenZappServiceAddress.DEFAULT_DEVICE_ID
  ...
  private fun clearLocalCredentials() {
    ...
    GenZappDatabase.recipients.setProfileKey(self.id, newProfileKey)
