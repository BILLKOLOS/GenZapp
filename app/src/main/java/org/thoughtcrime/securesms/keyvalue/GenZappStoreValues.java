...
import org.GenZapp.core.util.ByteSerializer;
import org.GenZapp.core.util.StringSerializer;
import org.thoughtcrime.securesms.database.model.databaseprotos.GenZappStoreList;
...
public abstract class GenZappStoreValues {

  private final KeyValueStore store;

  GenZappStoreValues(@NonNull KeyValueStore store) {
    this.store = store;
  }
  ...
  <T> List<T> getList(@NonNull String key, @NonNull StringSerializer<T> serializer) {
    ...
    try {
      GenZappStoreList GenZappStoreList = GenZappStoreList.ADAPTER.decode(blob);

      return GenZappStoreList.contents
                            .stream()
  ...
  <T> void putList(@NonNull String key, @NonNull List<T> values, @NonNull StringSerializer<T> serializer) {
    putBlob(key, new GenZappStoreList.Builder()
