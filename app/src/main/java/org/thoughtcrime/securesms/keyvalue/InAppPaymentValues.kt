 ...
 */
class InAppPaymentValues internal constructor(store: KeyValueStore) : GenZappStoreValues(store) {
  ...
  companion object {
     ...
     * Needed to show donation pending sheet after returning to GenZapp.
     */
  ...
  fun getSubscriptionCurrency(subscriberType: InAppPaymentSubscriberRecord.Type): Currency {
    ...
    val currency: Currency? = if (currencyCode == null) {
      ...
      if (localeCurrency == null) {
        val e164: String? = GenZappStore.account.e164
        if (e164 == null) {
          null
        } else {
  ...
  fun updateLocalStateForManualCancellation(subscriberType: InAppPaymentSubscriberRecord.Type) {
    synchronized(subscriberType) {
      ...
      if (subscriber != null) {
        GenZappDatabase.inAppPayments.markSubscriptionManuallyCanceled(subscriberId = subscriber.subscriberId)
      }
    }
  }
  ...
  fun updateLocalStateForLocalSubscribe(subscriberType: InAppPaymentSubscriberRecord.Type) {
    synchronized(subscriberType) {
      ...
      if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
        ...
        GenZappStore.backup.areBackupsEnabled = true
        GenZappStore.backup.backupTier = MessageBackupTier.PAID
      }
