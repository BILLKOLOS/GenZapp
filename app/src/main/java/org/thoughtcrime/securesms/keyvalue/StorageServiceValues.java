...
public class StorageServiceValues extends GenZappStoreValues {
  ...
  public synchronized StorageKey getOrCreateStorageKey() {
    ...
    return GenZappStore.svr().getOrCreateMasterKey().deriveStorageServiceKey();
  }
  ...
  public void setManifest(@NonNull GenZappStorageManifest manifest) {
  ...
  public @NonNull GenZappStorageManifest getManifest() {
    ...
    if (data != null) {
      return GenZappStorageManifest.deserialize(data);
    } else {
      return GenZappStorageManifest.EMPTY;
    }
  }

  public synchronized void setStorageKeyFromPrimary(@NonNull StorageKey storageKey) {
    Preconditions.checkState(GenZappStore.account().isLinkedDevice(), "Can only set storage key directly on linked devices");
    putBlob(SYNC_STORAGE_KEY, storageKey.serialize());
  }

  public void clearStorageKeyFromPrimary() {
    Preconditions.checkState(GenZappStore.account().isLinkedDevice(), "Can only clear storage key directly on linked devices");
