...
public final class ProxyValues extends GenZappStoreValues {
  ...
  public void enableProxy(@NonNull GenZappProxy proxy) {
   ...
   */
  public void setProxy(@Nullable GenZappProxy proxy) {
    if (proxy != null) {
      getStore().beginWrite()
                .putString(KEY_HOST, proxy.getHost())
                .putInteger(KEY_PORT, proxy.getPort())
  ...
  public @Nullable GenZappProxy getProxy() {
    String host = getString(KEY_HOST, null);
    int    port = getInteger(KEY_PORT, 0);

    if (host != null) {
      return new GenZappProxy(host, port);
    } else {
      return null;
    }
  }
