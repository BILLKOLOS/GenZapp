...
import org.whispersystems.GenZappservice.api.messages.GenZappServiceReceiptMessage;
...
public class SendViewedReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException {
    ...
    boolean canSendStoryReceipts    = GenZappStore.story().getViewedReceiptsEnabled();

    List<MessageId> foundMessageIds       = new LinkedList<>();
    List<Long>      messageSentTimestamps = new LinkedList<>();
    List<StoryType> storyTypes            = GenZappDatabase.messages().getStoryTypes(this.messageIds);
    ...
    GenZappServiceAddress        remoteAddress  = RecipientUtil.toGenZappServiceAddress(context, recipient);
    GenZappServiceReceiptMessage receiptMessage = new GenZappServiceReceiptMessage(GenZappServiceReceiptMessage.Type.VIEWED,
    ...
    SendMessageResult result = messageSender.sendReceipt(remoteAddress,
                                                         SealedSenderAccessUtil.getSealedSenderAccessFor(recipient,
                                                                                                         () -> GenZappDatabase.groups().getGroupSendFullToken(threadId, recipientId)),
                                                         receiptMessage,
                                                         recipient.getNeedsPniSignature());

    if (Util.hasItems(foundMessageIds)) {
      GenZappDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, foundMessageIds, false);
    }
  }
