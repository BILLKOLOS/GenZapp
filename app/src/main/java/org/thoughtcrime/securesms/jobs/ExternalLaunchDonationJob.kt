...
class ExternalLaunchDonationJob private constructor(
    ...
    if (donationError != null) {
      when (stripe3DSData.inAppPayment.type) {
        InAppPaymentType.ONE_TIME_DONATION -> {
          GenZappStore.inAppPayments.setPendingOneTimeDonation(
        ...
        InAppPaymentType.RECURRING_DONATION -> {
          GenZappStore.inAppPayments.appendToTerminalDonationQueue(
  ...
  private fun runForPaymentIntent() {
    ...
    GenZappDatabase.donationReceipts.addReceipt(inAppPaymentReceiptRecord)
  ...
  private fun runForSetupIntent() {
    ...
    Log.i(TAG, "Set default payment method via GenZapp service!", true)
    Log.i(TAG, "Storing the subscription payment source type locally.", true)
    GenZappStore.inAppPayments.setSubscriptionPaymentSourceType(stripe3DSData.paymentSourceType)
    ...
    try {
      ...
      if (updateSubscriptionLevelResponse.status in listOf(200, 204)) {
        ...
        GenZappStore.inAppPayments.updateLocalStateForLocalSubscribe(subscriber.type)
        GenZappStore.inAppPayments.setVerifiedSubscription3DSData(stripe3DSData)
