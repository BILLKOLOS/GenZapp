...
class InAppPaymentKeepAliveJob private constructor(
  ...
  companion object {
    ...
    fun enqueueAndTrackTimeIfNecessary() {
      ...
      val lastKeepAliveTime = GenZappStore.inAppPayments.getLastKeepAliveLaunchTime().milliseconds
    ...
    fun enqueueAndTrackTime(now: Duration) {
      ...
      GenZappStore.inAppPayments.setLastKeepAliveLaunchTime(now.inWholeMilliseconds)
    }
  }
  ...
  private fun doRun() {
    ...
    if (GenZappDatabase.inAppPayments.hasPending(type.inAppPaymentType)) {
    ...
    when (activeInAppPayment.data.redemption?.stage) {
      InAppPaymentData.RedemptionState.Stage.INIT -> {
        ...
        GenZappDatabase.inAppPayments.update(payment)
  ...
  private fun getActiveInAppPayment(
    ...
    val current: InAppPaymentTable.InAppPayment? = GenZappDatabase.inAppPayments.getByEndOfPeriod(type.inAppPaymentType, endOfCurrentPeriod)

    return if (current == null) {
      val oldInAppPayment = GenZappDatabase.inAppPayments.getByLatestEndOfPeriod(type.inAppPaymentType)
      ...
      if (type == InAppPaymentSubscriberRecord.Type.DONATION) {
        GenZappStore.inAppPayments.setLastEndOfPeriod(endOfCurrentPeriod.inWholeSeconds)
      }

      val inAppPaymentId = GenZappDatabase.inAppPayments.insert(
