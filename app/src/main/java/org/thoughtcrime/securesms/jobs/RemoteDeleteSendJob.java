...
public class RemoteDeleteSendJob extends BaseJob {
  ...
  {
    MessageRecord message = GenZappDatabase.messages().getMessageRecord(messageId);
    ...
    if (conversationRecipient.isDistributionList()) {
      recipients = GenZappDatabase.storySends().getRemoteDeleteRecipients(message.getId(), message.getTimestamp());
  ...
  protected void onRun() throws Exception {
    ...
    if (totalSkips.size() > 0 && message.getToRecipient().isGroup()) {
      GenZappDatabase.groupReceipts().setSkipped(totalSkips, messageId);
    }
      ...
      throws IOException, UntrustedIdentityException
  {
    GenZappServiceDataMessage.Builder dataMessageBuilder = GenZappServiceDataMessage.newBuilder()
                                                                                  ...
                                                                                  .withRemoteDelete(new GenZappServiceDataMessage.RemoteDelete(targetSentTimestamp));
    ...
    if (conversationRecipient.isSelf()) {
      AppDependencies.getGenZappServiceMessageSender().sendSyncMessage(dataMessage);
    }
