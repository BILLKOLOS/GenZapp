...
public class DonationReceiptRedemptionJob extends BaseJob {
  ...
  public void onFailure() {
    ...
    if (isForSubscription()) {
      ...
      GenZappStore.inAppPayments().markSubscriptionRedemptionFailed();
      MultiDeviceSubscriptionSyncRequestJob.enqueue();
    } else if (giftMessageId != NO_ID) {
      GenZappDatabase.messages().markGiftRedemptionFailed(giftMessageId);
    }
  ...
  public void onAdded() {
    if (giftMessageId != NO_ID) {
      GenZappDatabase.messages().markGiftRedemptionStarted(giftMessageId);
    }
  }
  ...
  private void doRun() throws Exception {
    ...
    ServiceResponse<EmptyResponse> response = AppDependencies.getDonationsService()
                                                             .redeemDonationReceipt(presentation,
                                                                                    GenZappStore.inAppPayments().getDisplayBadgesOnProfile(),
                                                                                    makePrimary);
    ...
    if (response.getApplicationError().isPresent()) {
      if (response.getStatus() >= 500) {
        ...
        if (isForOneTimeDonation()) {
          ...
          GenZappStore.inAppPayments().setPendingOneTimeDonationError(
              donationErrorValue
          );
    ...
    if (isForSubscription()) {
      ...
      GenZappStore.inAppPayments()
                 .setSubscriptionEndOfPeriodRedeemed(GenZappStore.inAppPayments()
                                                                .getSubscriptionEndOfPeriodRedemptionStarted());
      GenZappStore.inAppPayments().clearSubscriptionReceiptCredential();
    } else if (giftMessageId != NO_ID) {
      Log.d(TAG, "Marking gift redemption completed for " + giftMessageId);
      GenZappDatabase.messages().markGiftRedemptionCompleted(giftMessageId);
      MessageTable.MarkedMessageInfo markedMessageInfo = GenZappDatabase.messages().setIncomingMessageViewed(giftMessageId);
  ...
  private @Nullable ReceiptCredentialPresentation getPresentation() throws InvalidInputException, NoSuchMessageException {
    ...
    if (isForSubscription()) {
      receiptCredentialPresentation = GenZappStore.inAppPayments().getSubscriptionReceiptCredential();
    } else {
  ...
  private @Nullable ReceiptCredentialPresentation getPresentationFromGiftMessage() throws InvalidInputException, NoSuchMessageException {
    MessageRecord messageRecord = GenZappDatabase.messages().getMessageRecord(giftMessageId);
