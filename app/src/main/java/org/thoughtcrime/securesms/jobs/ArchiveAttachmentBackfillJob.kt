...
import org.whispersystems.GenZappservice.api.archive.ArchiveMediaUploadFormStatusCodes
...
class ArchiveAttachmentBackfillJob private constructor(
  ...
  override fun run(): Result {
    ...
    var attachmentRecord: DatabaseAttachment? = if (attachmentId != null) {
      ...
      GenZappDatabase.attachments.getAttachment(attachmentId!!)
    } else {
      GenZappDatabase.attachments.getNextAttachmentToArchiveAndMarkUploadInProgress()
    }
    ...
    if (attachmentRecord == null) {
      ...
      val resetCount = GenZappDatabase.attachments.resetPendingArchiveBackfills()
      if (resetCount > 0) {
    ...
    val transferState: AttachmentTable.ArchiveTransferState? = GenZappDatabase.attachments.getArchiveTransferState(attachmentRecord.attachmentId)
    ...
    if (transferState == AttachmentTable.ArchiveTransferState.BACKFILL_UPLOAD_IN_PROGRESS) {
      ...
      val attachmentStream = try {
        AttachmentUploadUtil.buildGenZappServiceAttachmentStream(
          context = context,
          attachment = attachmentRecord,
          uploadSpec = uploadSpec!!,
          cancellationGenZapp = { this.isCanceled }
        )
      } catch (e: IOException) {
      ...
      val remoteAttachment: GenZappServiceAttachmentPointer = try {
        AppDependencies.GenZappServiceMessageSender.uploadAttachment(attachmentStream)
      } catch (e: IOException) {
      ...
      GenZappDatabase.attachments.finalizeAttachmentAfterUpload(attachmentRecord.attachmentId, pointerAttachment, remoteAttachment.uploadTimestamp)
      GenZappDatabase.attachments.setArchiveTransferState(attachmentRecord.attachmentId, AttachmentTable.ArchiveTransferState.BACKFILL_UPLOADED)

      attachmentRecord = GenZappDatabase.attachments.getAttachment(attachmentRecord.attachmentId)
    }
    ...
    return when (val result = BackupRepository.archiveMedia(attachmentRecord)) {
      is NetworkResult.Success -> {
        ...
        GenZappDatabase.attachments.setArchiveTransferState(attachmentRecord.attachmentId, AttachmentTable.ArchiveTransferState.FINISHED)
