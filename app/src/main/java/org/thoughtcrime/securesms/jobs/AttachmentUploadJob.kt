...
import org.GenZapp.core.util.inRoundedDays
...
import org.whispersystems.GenZappservice.api.push.exceptions.NonSuccessfulResumableUploadResponseCodeException
import org.whispersystems.GenZappservice.api.push.exceptions.ResumeLocationInvalidException
...
class AttachmentUploadJob private constructor(
  ...
  override fun onAdded() {
    ...
    val database = GenZappDatabase.attachments
  ...
  public override fun onRun() {
    ...
    val databaseAttachment = GenZappDatabase.attachments.getAttachment(attachmentId) ?: throw InvalidAttachmentException("Cannot find the specified attachment.")
    ...
    if (uploadSpec == null) {
      ...
      uploadSpec = AppDependencies.GenZappServiceMessageSender.getResumableUploadSpec().toProto()
    } else {
    ...
    try {
      getAttachmentNotificationIfNeeded(databaseAttachment).use { notification ->
        buildAttachmentStream(databaseAttachment, notification, uploadSpec!!).use { localAttachment ->
          ...
          GenZappDatabase.attachments.finalizeAttachmentAfterUpload(databaseAttachment.attachmentId, attachment, remoteAttachment.uploadTimestamp)
          ArchiveThumbnailUploadJob.enqueueIfNecessary(databaseAttachment.attachmentId)
        }
      }
    } catch (e: StreamResetException) {
      val lastReset = GenZappStore.misc.lastNetworkResetDueToStreamResets
      ...
      if (lastReset > now || lastReset + NETWORK_RESET_THRESHOLD > now) {
        ...
        GenZappStore.misc.lastNetworkResetDueToStreamResets = now
      } else {
  ...
  private fun buildAttachmentStream(attachment: Attachment, notification: AttachmentProgressService.Controller?, resumableUploadSpec: ResumableUpload): GenZappServiceAttachmentStream {
    ...
    return try {
      AttachmentUploadUtil.buildGenZappServiceAttachmentStream(
        ...
        cancellationGenZapp = { isCanceled },
        progressListener = object : GenZappServiceAttachment.ProgressListener {
