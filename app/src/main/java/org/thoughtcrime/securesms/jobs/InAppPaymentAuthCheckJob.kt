...
import org.GenZapp.donations.json.StripePaymentIntent
import org.GenZapp.donations.json.StripeSetupIntent
...
class InAppPaymentAuthCheckJob private constructor(parameters: Parameters) : BaseJob(parameters), StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (GenZappDatabase.inAppPayments.hasWaitingForAuth()) {
  ...
  override fun onRun() {
    ...
    val unauthorizedInAppPayments = GenZappDatabase.inAppPayments.getAllWaitingForAuth()
  ...
  private fun migrateLegacyData() {
    val pending3DSData = GenZappStore.inAppPayments.consumePending3DSData()
  ...
  private fun checkOneTimePayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    GenZappDatabase.donationReceipts.addReceipt(receipt)
    ...
    val updatedPayment = GenZappDatabase.inAppPayments.getById(inAppPayment.id)
  ...
  private fun checkRecurringPayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    Log.d(TAG, "Set default payment method via GenZapp service.", true)
    ...
    try {
      ...
      if (updateLevelResponse.status in listOf(200, 204)) {
        ...
        with(GenZappDatabase.inAppPayments.getById(inAppPayment.id)!!) {
  ...
  private fun markChecked(inAppPayment: InAppPaymentTable.InAppPayment) {
    val fresh = GenZappDatabase.inAppPayments.getById(inAppPayment.id)
    GenZappDatabase.inAppPayments.update(
