...
class BuildExpirationConfirmationJob private constructor(params: Parameters) : Job(params) {
  ...
  override fun run(): Result {
    if (Util.getTimeUntilBuildExpiry(GenZappStore.misc.estimatedServerTime) > 0) {
      Log.i(TAG, "Build not expired.", true)
      return Result.success()
    }

    if (GenZappStore.misc.isClientDeprecated) {
    ...
    if (!GenZappStore.account.isRegistered) {
      ...
      GenZappStore.misc.isClientDeprecated = true
      return Result.success()
    }

    val result: NetworkResult<RemoteConfigResult> = NetworkResult.fromFetch {
      AppDependencies.GenZappServiceAccountManager.remoteConfig
    }

    return when (result) {
      is NetworkResult.Success -> {
        val serverTimeMs = result.result.serverEpochTimeSeconds.seconds.inWholeMilliseconds
        GenZappStore.misc.setLastKnownServerTime(serverTimeMs, System.currentTimeMillis())
