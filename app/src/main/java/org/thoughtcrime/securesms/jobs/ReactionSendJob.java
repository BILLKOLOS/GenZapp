...
public class ReactionSendJob extends BaseJob {
  ...
  {
    MessageRecord message = GenZappDatabase.messages().getMessageRecord(messageId.getId());

    Recipient conversationRecipient = GenZappDatabase.threads().getRecipientForThreadId(message.getThreadId());
  ...
  protected void onRun() throws Exception {
    ...
    ReactionTable reactionTable = GenZappDatabase.reactions();
    MessageRecord message       = GenZappDatabase.messages().getMessageRecord(messageId.getId());

    Recipient targetAuthor        = message.getFromRecipient();
    long      targetSentTimestamp = message.getDateSent();

    if (targetAuthor.getId().equals(GenZappStore.releaseChannel().getReleaseChannelRecipientId())) {
      return;
    }
  ...
  {
    GenZappServiceDataMessage.Builder dataMessageBuilder = GenZappServiceDataMessage.newBuilder()
    ...
    GenZappServiceDataMessage dataMessage         = dataMessageBuilder.build();
    ...
    if (includesSelf) {
      results.add(AppDependencies.getGenZappServiceMessageSender().sendSyncMessage(dataMessage));
    }
    ...
    for (RecipientId unregistered : groupResult.unregistered) {
      GenZappDatabase.recipients().markUnregistered(unregistered);
    }

    return groupResult.completed;
  }

  private static GenZappServiceDataMessage.Reaction buildReaction(@NonNull Context context,
                                                                 @NonNull ReactionRecord reaction,
                                                                 boolean remove,
      ...
      throws IOException
  {
    return new GenZappServiceDataMessage.Reaction(reaction.getEmoji(),
                                                 remove,
