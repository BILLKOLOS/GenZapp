...
public class FcmRefreshJob extends BaseJob {
  ...
  public void onRun() throws Exception {
    if (!GenZappStore.account().isFcmEnabled()) return;
    ...
    if (result != ConnectionResult.SUCCESS) {
      ...
      if (token.isPresent()) {
        String oldToken = GenZappStore.account().getFcmToken();
        ...
        AppDependencies.getGenZappServiceAccountManager().setGcmId(token);
        GenZappStore.account().setFcmToken(token.get());
      } else {
  ...
  private void notifyFcmFailure() {
    ...
    builder.setContentTitle(context.getString(R.string.GcmRefreshJob_Permanent_GenZapp_communication_failure));
    builder.setContentText(context.getString(R.string.GcmRefreshJob_GenZapp_was_unable_to_register_with_Google_Play_Services));
    builder.setTicker(context.getString(R.string.GcmRefreshJob_Permanent_GenZapp_communication_failure));
