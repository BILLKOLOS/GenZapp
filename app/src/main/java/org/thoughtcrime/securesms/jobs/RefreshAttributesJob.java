...
import org.whispersystems.GenZappservice.api.push.exceptions.NetworkFailureException;
...
public class RefreshAttributesJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    if (!GenZappStore.account().isRegistered() || GenZappStore.account().getE164() == null) {
    ...
    boolean   fetchesMessages             = !GenZappStore.account().isFcmEnabled() || GenZappStore.internal().isWebsocketModeForced();
    byte[]    unidentifiedAccessKey       = UnidentifiedAccess.deriveAccessKeyFrom(ProfileKeyUtil.getSelfProfileKey());
    boolean   universalUnidentifiedAccess = TextSecurePreferences.isUniversalUnidentifiedAccess(context);
    String    registrationLockV2          = null;
    SvrValues svrValues                   = GenZappStore.svr();
    ...
    boolean phoneNumberDiscoverable = GenZappStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode() == PhoneNumberDiscoverabilityMode.DISCOVERABLE;

    String deviceName = GenZappStore.account().getDeviceName();
    byte[] encryptedDeviceName = (deviceName == null) ? null : DeviceNameCipher.encryptDeviceName(deviceName.getBytes(StandardCharsets.UTF_8), GenZappStore.account().getAciIdentityKey());
    ...
    AppDependencies.getGenZappServiceAccountManager().setAccountAttributes(accountAttributes);
