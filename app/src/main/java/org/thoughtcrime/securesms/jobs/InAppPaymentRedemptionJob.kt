...
class InAppPaymentRedemptionJob private constructor(
  ...
  override fun onAdded() {
    if (jobData.giftMessageId != null) {
      ...
      GenZappDatabase.messages.markGiftRedemptionStarted(jobData.giftMessageId)
    }
  }
  ...
  override fun onFailure() {
    if (jobData.giftMessageId != null) {
      ...
      GenZappDatabase.messages.markGiftRedemptionFailed(jobData.giftMessageId)
    }

    if (jobData.inAppPaymentId != null) {
      Log.w(TAG, "A permanent failure occurred.")

      val inAppPayment = GenZappDatabase.inAppPayments.getById(InAppPaymentTable.InAppPaymentId(jobData.inAppPaymentId))
  ...
  private fun onRunForGiftMessageId(messageId: Long) {
    val giftBadge = GenZappDatabase.messages.getMessageRecord(messageId)
    ...
    GenZappDatabase.messages.markGiftRedemptionCompleted(messageId)
  }
