...
import org.GenZapp.core.util.getDownloadManager
...
class ApkUpdateJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    if (shouldUpdate(getCurrentAppVersionCode(), updateDescriptor, GenZappStore.apkUpdate.lastApkUploadTime, Environment.IS_WEBSITE)) {
      Log.i(TAG, "Newer version code available. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${GenZappStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
      ...
      Log.d(TAG, "Version code is the same or older than our own. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${GenZappStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
    }

    GenZappStore.apkUpdate.lastSuccessfulCheck = System.currentTimeMillis()
  }
  ...
  private fun getDownloadStatus(uri: String, remoteDigest: ByteArray): DownloadStatus {
    val pendingDownloadId: Long = GenZappStore.apkUpdate.downloadId
    val pendingDigest: ByteArray? = GenZappStore.apkUpdate.digest

    if (pendingDownloadId == -1L || pendingDigest == null || !MessageDigest.isEqual(pendingDigest, remoteDigest)) {
      GenZappStore.apkUpdate.clearDownloadAttributes()
  ...
  private fun handleDownloadStart(uri: String?, versionName: String?, digest: ByteArray, uploadTimestamp: Long) {
    ...
    val downloadRequest = DownloadManager.Request(Uri.parse(uri)).apply {
      ...
      setTitle("Downloading GenZapp update")
      setDescription("Downloading GenZapp $versionName")
      setDestinationInExternalFilesDir(context, null, "GenZapp-update.apk")
    ...
    GenZappStore.apkUpdate.setDownloadAttributes(downloadId, digest, uploadTimestamp)
  }
  ...
  private fun deleteExistingDownloadedApks(context: Context) {
    ...
    for (file in directory.listFiles() ?: emptyArray()) {
      if (file.name.startsWith("GenZapp-update")) {
        if (file.delete()) {
