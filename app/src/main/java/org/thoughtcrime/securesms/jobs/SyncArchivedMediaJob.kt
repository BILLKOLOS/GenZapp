...
class SyncArchivedMediaJob private constructor(
  ...
  override fun onRun() {
    ...
    GenZappStore.backup.lastMediaSyncTime = System.currentTimeMillis()
  }
  ...
  private fun syncPage(archivedItemPage: ArchiveGetMediaItemsResponse): Set<ArchivedMediaObject> {
    ...
    GenZappDatabase.rawDatabase.withinTransaction {
      archivedItemPage.storedMediaObjects.forEach { storedMediaObject ->
        val rows = GenZappDatabase.attachments.updateArchiveCdnByMediaId(archiveMediaId = storedMediaObject.mediaId, archiveCdn = storedMediaObject.cdn)
        if (rows == 0) {
