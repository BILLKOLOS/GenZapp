...
class CallSyncEventJob private constructor(
  ...
  private fun processEvent(callSyncEvent: CallSyncEventJobRecord): CallSyncEventJobRecord? {
    val call = GenZappDatabase.calls.getCallById(callSyncEvent.callId, callSyncEvent.deserializeRecipientId())
    if (call == null) {
    ...
    return try {
      AppDependencies.GenZappServiceMessageSender.sendSyncMessage(GenZappServiceSyncMessage.forCallEvent(syncMessage))
      null
    } catch (e: Exception) {
