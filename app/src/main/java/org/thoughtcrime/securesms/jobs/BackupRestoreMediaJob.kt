...
class BackupRestoreMediaJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    do {
      val attachmentBatch = GenZappDatabase.attachments.getRestorableAttachments(batchSize)
      val messageIds = attachmentBatch.map { it.mmsId }.toSet()
      val messageMap = GenZappDatabase.messages.getMessages(messageIds).associate { it.id to (it as MmsMessageRecord) }
      restoreJobBatch = GenZappDatabase.attachments.getRestorableAttachments(batchSize).map { attachment ->
        val message = messageMap[attachment.mmsId]!!
        if (shouldRestoreFullSize(message, restoreTime, GenZappStore.backup.optimizeStorage)) {
          ...
          GenZappDatabase.attachments.setTransferState(
            messageId = attachment.mmsId,
            attachmentId = attachment.attachmentId,
