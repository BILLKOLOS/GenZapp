...
class GroupRingCleanupJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    GenZappDatabase.calls.getLatestRingingCalls().forEach {
      AppDependencies.GenZappCallManager.peekGroupCall(it.peer)
    }

    GenZappDatabase.calls.markRingingCallsAsMissed()
  }
