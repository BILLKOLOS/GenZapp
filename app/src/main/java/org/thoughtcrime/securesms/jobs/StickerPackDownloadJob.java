...
import org.whispersystems.GenZappservice.api.messages.GenZappServiceStickerManifest;
import org.whispersystems.GenZappservice.api.messages.GenZappServiceStickerManifest.StickerInfo;
...
public class StickerPackDownloadJob extends BaseJob {
  ...
  protected void onRun() throws IOException, InvalidMessageException {
    if (isReferencePack && !GenZappDatabase.attachments().containsStickerPackId(packId) && !BlessedPacks.contains(packId)) {
      Log.w(TAG, "There are no attachments with the requested packId present for this reference pack. Skipping.");
      return;
    }

    if (isReferencePack && GenZappDatabase.stickers().isPackAvailableAsReference(packId)) {
      Log.i(TAG, "Sticker pack already available for reference. Skipping.");
      return;
    }

    GenZappServiceMessageReceiver receiver        = AppDependencies.getGenZappServiceMessageReceiver();
    JobManager   jobManager      = AppDependencies.getJobManager();
    StickerTable stickerDatabase = GenZappDatabase.stickers();
    byte[]       packIdBytes     = Hex.fromStringCondensed(packId);
    byte[]                       packKeyBytes    = Hex.fromStringCondensed(packKey);
    GenZappServiceStickerManifest manifest        = receiver.retrieveStickerManifest(packIdBytes, packKeyBytes);
  ...
  public void onFailure() {
    ...
    GenZappDatabase.stickers().uninstallPack(packId);
    GenZappDatabase.stickers().deleteOrphanedPacks();
  }
