...
class LeaveGroupV2WorkerJob(parameters: Parameters, private val groupId: GroupId.V2) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    val threadId = GenZappDatabase.threads.getThreadIdIfExistsFor(groupRecipient.id)
    if (threadId != -1L) {
      GenZappDatabase.recipients.setProfileSharing(groupRecipient.id, enabled = false)
      GenZappDatabase.threads.setEntireThreadRead(threadId)
