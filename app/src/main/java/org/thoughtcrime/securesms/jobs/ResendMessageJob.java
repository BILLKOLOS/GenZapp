...
public class ResendMessageJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    if (GenZappStore.internal().delayResends()) {
    ...
    if (distributionId != null) {
      if (groupId != null) {
        ...
        Optional<GroupRecord> groupRecord = GenZappDatabase.groups().getGroupByDistributionId(distributionId);
        ...
        createGroupSendToken = () -> GenZappDatabase.groups().getGroupSendFullToken(groupId, recipientId);
      } else {
        Log.d(TAG, "GroupId is not present. Assuming this is a message for a distribution list.");
        DistributionListRecord listRecord = GenZappDatabase.distributionLists().getListByDistributionId(distributionId);
    ...
    try {
      ...
      AppDependencies.getProtocolStore().aci().sessions().archiveSessions(recipientId, GenZappServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(GenZappServiceAddress.DEFAULT_DEVICE_ID));
      AppDependencies.getProtocolStore().pni().sessions().archiveSessions(recipientId, GenZappServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().pni().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(GenZappServiceAddress.DEFAULT_DEVICE_ID));
