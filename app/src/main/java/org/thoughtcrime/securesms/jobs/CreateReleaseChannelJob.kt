...
/**
 * Creates the Release Channel (GenZapp) recipient.
 */
class CreateReleaseChannelJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    if (GenZappStore.releaseChannel.releaseChannelRecipientId != null) {
      Log.i(TAG, "Already created Release Channel recipient ${GenZappStore.releaseChannel.releaseChannelRecipientId}")

      val recipient = Recipient.resolved(GenZappStore.releaseChannel.releaseChannelRecipientId!!)
      ...
      recipients.setProfileName(releaseChannelId, ProfileName.asGiven("GenZapp"))
  ...
  private fun setAvatar(id: RecipientId) {
    ...
    AvatarRenderer.renderAvatar(
      context,
      Avatar.Resource(
        R.drawable.ic_GenZapp_logo_large,
      ...
      onAvatarRendered = { media ->
        ...
        GenZappDatabase.recipients.setProfileAvatar(id, "local")
        latch.countDown()
      },
      onRenderFailed = { t ->
        Log.w(TAG, t)
