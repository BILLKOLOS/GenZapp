...
public class AutomaticSessionResetJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (RemoteConfig.automaticSessionReset()) {
      ...
      DeviceLastResetTime resetTimes         = GenZappDatabase.recipients().getLastSessionResetTimes(recipientId);
      ...
      if (timeSinceLastReset > resetInterval) {
        ...
        GenZappDatabase.recipients().setLastSessionResetTime(recipientId, setLastResetTime(resetTimes, deviceId, System.currentTimeMillis()));
  ...
  private void insertLocalMessage() {
    MessageTable.InsertResult result = GenZappDatabase.messages().insertChatSessionRefreshedMessage(recipientId, deviceId, sentTimestamp - 1);
