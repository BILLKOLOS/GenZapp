...
public class SendDeliveryReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException, UndeliverableMessageException {
    ...
    GenZappServiceReceiptMessage receiptMessage = new GenZappServiceReceiptMessage(GenZappServiceReceiptMessage.Type.DELIVERY,
    ...
    if (messageId != null) {
      GenZappDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, messageId, false);
    }
  }
  ...
  private @Nullable GroupSendFullToken getGroupSendFullToken() {
    ...
    long threadId = GenZappDatabase.messages().getThreadIdForMessage(messageId.getId());
    if (threadId == -1) {
      return null;
    }

    return GenZappDatabase.groups().getGroupSendFullToken(threadId, recipientId);
  }
