...
import org.whispersystems.GenZappservice.api.svr.SecureValueRecoveryV2
...
class Svr2MirrorJob private constructor(parameters: Parameters, private var serializedChangeSession: String?) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    SvrRepository.operationLock.withLock {
      ...
      val svr2: SecureValueRecoveryV2 = AppDependencies.GenZappServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE)

      val session: PinChangeSession = serializedChangeSession?.let { session ->
        svr2.resumePinChangeSession(pin, GenZappStore.svr.getOrCreateMasterKey(), session)
      } ?: svr2.setPin(pin, GenZappStore.svr.getOrCreateMasterKey())
      ...
      return when (val response: BackupResponse = session.execute()) {
        is BackupResponse.Success -> {
          ...
          GenZappStore.svr.appendSvr2AuthTokenToList(response.authorization.asBasic())
