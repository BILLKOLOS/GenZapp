...
import org.GenZapp.libGenZapp.zkgroup.receipts.ReceiptCredential;
import org.GenZapp.libGenZapp.zkgroup.receipts.ReceiptCredentialPresentation;
import org.GenZapp.libGenZapp.zkgroup.receipts.ReceiptCredentialRequestContext;
import org.GenZapp.libGenZapp.zkgroup.receipts.ReceiptCredentialResponse;
...
public class SubscriptionReceiptRequestResponseJob extends BaseJob {
  ...
  public void onFailure() {
    if (terminalDonation.error != null) {
      GenZappStore.inAppPayments().appendToTerminalDonationQueue(terminalDonation);
    } else {
  ...
  private void doRun() throws Exception {
    ReceiptCredentialRequestContext requestContext     = GenZappStore.inAppPayments().getSubscriptionRequestCredential();
    ...
    if (subscription == null) {
      ...
      long storedEndOfPeriod = GenZappStore.inAppPayments().getLastEndOfPeriod();
      if (storedEndOfPeriod < subscription.getEndOfCurrentPeriod()) {
        ...
        GenZappStore.inAppPayments().setLastEndOfPeriod(subscription.getEndOfCurrentPeriod());
        MultiDeviceSubscriptionSyncRequestJob.enqueue();
      }

      if (GenZappStore.inAppPayments().getSubscriptionEndOfPeriodConversionStarted() == 0L) {
        Log.i(TAG, "Marking the start of initial conversion.", true);
        GenZappStore.inAppPayments().setSubscriptionEndOfPeriodConversionStarted(subscription.getEndOfCurrentPeriod());
      }
    }
    ...
    if (response.getApplicationError().isPresent()) {
      ...
      GenZappDatabase.donationReceipts().addReceipt(InAppPaymentReceiptRecord.createForSubscription(subscription));

      GenZappStore.inAppPayments().clearSubscriptionRequestCredential();
      GenZappStore.inAppPayments().setSubscriptionReceiptCredential(receiptCredentialPresentation);
      GenZappStore.inAppPayments().setSubscriptionEndOfPeriodRedemptionStarted(subscription.getEndOfCurrentPeriod());
  ...
  private void handleApplicationError(ServiceResponse<ReceiptCredentialResponse> response) throws Exception {
    switch (response.getStatus()) {
      case 204:
        ...
        GenZappStore.inAppPayments().clearSubscriptionRedemptionFailed();
        throw new RetryableException();
      case 400:
  ...
  private void onPaymentFailure(@NonNull ActiveSubscription.Subscription subscription, @Nullable ActiveSubscription.ChargeFailure chargeFailure, boolean isForKeepAlive) {
    ...
    if (isForKeepAlive) {
      ...
      GenZappStore.inAppPayments().setUnexpectedSubscriptionCancelationChargeFailure(chargeFailure);
      GenZappStore.inAppPayments().setUnexpectedSubscriptionCancelationReason(subscription.getStatus());
      GenZappStore.inAppPayments().setUnexpectedSubscriptionCancelationTimestamp(subscription.getEndOfCurrentPeriod());
      GenZappStore.inAppPayments().setShowMonthlyDonationCanceledDialog(true);

      AppDependencies.getDonationsService().getDonationsConfiguration(Locale.getDefault()).getResult().ifPresent(config -> {
        GenZappStore.inAppPayments().setExpiredBadge(DonationsConfigurationExtensionsKt.getBadge(config, subscription.getLevel()));
      });
      ...
      PaymentSourceType               paymentSourceType = GenZappStore.inAppPayments().getSubscriptionPaymentSourceType();
  ...
  public static class Factory implements Job.Factory<SubscriptionReceiptRequestResponseJob> {
    ...
    public @NonNull SubscriptionReceiptRequestResponseJob create(@NonNull Parameters parameters, @Nullable byte[] serializedData) {
      ...
      if (requestContextBytes != null && GenZappStore.inAppPayments().getSubscriptionRequestCredential() == null) {
        try {
          requestContext = new ReceiptCredentialRequestContext(requestContextBytes);
          GenZappStore.inAppPayments().setSubscriptionRequestCredential(requestContext);
