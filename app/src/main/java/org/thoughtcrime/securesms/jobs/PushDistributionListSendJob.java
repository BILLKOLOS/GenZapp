...
public final class PushDistributionListSendJob extends PushSendJob {
                             ...
                             @NonNull Set<RecipientId> filterRecipientIds)
  {
    try {
      ...
      if (!message.getStoryType().isTextStory()) {
        ...
        GenZappDatabase.attachments().updateAttachmentCaption(storyAttachment.attachmentId, message.getBody());
      }
      ...
      throws IOException, UntrustedIdentityException, UndeliverableMessageException
  {
    try {
      ...
      if (message.getStoryType().isTextStory()) {
        storyMessage = GenZappServiceStoryMessage.forTextAttachment(Recipient.self().getProfileKey(), null, StorySendUtil.deserializeBodyToStoryTextAttachment(message, this::getPreviewsFor), message.getStoryType().isStoryWithReplies(), bodyRanges);
      } else if (!attachmentPointers.isEmpty()) {
        storyMessage = GenZappServiceStoryMessage.forFileAttachment(Recipient.self().getProfileKey(), null, attachmentPointers.get(0), message.getStoryType().isStoryWithReplies(), bodyRanges);
      } else {
        throw new UndeliverableMessageException("No attachment on non-text story.");
      }

      SentStorySyncManifest                   manifest           = GenZappDatabase.storySends().getFullSentStorySyncManifest(messageId, message.getSentTimeMillis());
      Set<GenZappServiceStoryMessageRecipient> manifestCollection = manifest != null ? manifest.toRecipientsSet() : Collections.emptySet();
