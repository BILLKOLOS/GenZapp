...
public class SubscriptionKeepAliveJob extends BaseJob {
  ...
  private void doRun() throws Exception {
    ...
    if (endOfCurrentPeriod > GenZappStore.inAppPayments().getLastEndOfPeriod()) {
      Log.i(TAG,
            String.format(Locale.US,
                          "Last end of period change. Requesting receipt refresh. (old: %d to new: %d)",
                          GenZappStore.inAppPayments().getLastEndOfPeriod(),
                          activeSubscription.getActiveSubscription().getEndOfCurrentPeriod()),
            true);

      GenZappStore.inAppPayments().setLastEndOfPeriod(endOfCurrentPeriod);
    ...
    if (endOfCurrentPeriod > GenZappStore.inAppPayments().getSubscriptionEndOfPeriodConversionStarted()) {
      ...
      GenZappStore.inAppPayments().setSubscriptionEndOfPeriodConversionStarted(endOfCurrentPeriod);
      GenZappStore.inAppPayments().refreshSubscriptionRequestCredential();

      SubscriptionReceiptRequestResponseJob.createSubscriptionContinuationJobChain(true, -1L, terminalDonation).enqueue();
    } else if (endOfCurrentPeriod > GenZappStore.inAppPayments().getSubscriptionEndOfPeriodRedemptionStarted()) {
      if (GenZappStore.inAppPayments().getSubscriptionRequestCredential() == null) {
      ...
      SubscriptionReceiptRequestResponseJob.createSubscriptionContinuationJobChain(true, -1L, terminalDonation).enqueue();
    } else if (endOfCurrentPeriod > GenZappStore.inAppPayments().getSubscriptionEndOfPeriodRedeemed()) {
      if (GenZappStore.inAppPayments().getSubscriptionReceiptCredential() == null) {
