...
import org.whispersystems.GenZappservice.api.crypto.InvalidCiphertextException
import org.whispersystems.GenZappservice.api.crypto.ProfileCipher
import org.whispersystems.GenZappservice.api.profiles.ProfileAndCredential
import org.whispersystems.GenZappservice.api.profiles.GenZappServiceProfile
import org.whispersystems.GenZappservice.api.services.ProfileService.ProfileResponseProcessor
import org.whispersystems.GenZappservice.api.util.ExpiringProfileCredentialUtil
...
class RetrieveProfileJob private constructor(parameters: Parameters, private val recipientIds: MutableSet<RecipientId>) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    val localRecords = GenZappDatabase.recipients.getRecords(recipientIds)
    ...
    updatedProfiles.chunked(150).forEach { list: List<Pair<Recipient, ProfileAndCredential>> ->
      GenZappDatabase.runInTransaction {
        for (profile in list) {
    ...
    GenZappDatabase.recipients.markProfilesFetched(successIds, System.currentTimeMillis())
    ...
    if (newlyRegisteredIds.isNotEmpty()) {
      ...
      GenZappDatabase.recipients.bulkUpdatedRegisteredStatus(newlyRegisteredIds, emptySet())
    }
    if (operationState.unregistered.isNotEmpty()) {
      ...
      for (recipientId in operationState.unregistered) {
        GenZappDatabase.recipients.markUnregistered(recipientId)
      }
    }
  ...
  private fun isUpdated(localRecipientRecord: RecipientRecord, remoteProfile: GenZappServiceProfile, remoteExpiringProfileKeyCredential: Optional<ExpiringProfileKeyCredential>): Boolean {
    if (localRecipientRecord.GenZappProfileAvatar != remoteProfile.avatar) {
      return true
    }
    ...
    if (localRecipientRecord.GenZappProfileName != newProfileName) {
      return true
    }
  ...
  private fun setProfileBadges(recipient: Recipient, serviceBadges: List<GenZappServiceProfile.Badge>?) {
    ...
    GenZappDatabase.recipients.setBadges(recipient.id, badges)
  }
  ...
  private fun setExpiringProfileKeyCredential(
    ...
    GenZappDatabase.recipients.setProfileKeyCredential(recipient.id, recipientProfileKey, credential)
  }
  ...
  private fun setUnidentifiedAccessMode(recipient: Recipient, unidentifiedAccessVerifier: String?, unrestrictedUnidentifiedAccess: Boolean) {
    ...
    if (recipient.sealedSenderAccessMode !== newMode) {
      ...
      GenZappDatabase.recipients.setSealedSenderAccessMode(recipient.id, newMode)
    }
  }
  ...
  private fun setProfileName(recipient: Recipient, profileName: String?): Boolean {
    try {
      ...
      if (localProfileName.isEmpty &&
        ...
        val username = GenZappDatabase.recipients.getUsername(recipient.id)
        val e164 = if (username == null) GenZappDatabase.recipients.getE164sForIds(listOf(recipient.id)).firstOrNull() else null

        if (username != null || e164 != null) {
          Log.i(TAG, "Learned profile name for first time, inserting event")
          GenZappDatabase.messages.insertLearnedProfileNameChangeMessage(recipient, e164, username)
        } else {
      ...
      if (remoteProfileName != localProfileName) {
        ...
        GenZappDatabase.recipients.setProfileName(recipient.id, remoteProfileName)
        ...
        if (writeChangeEvent) {
          ...
          GenZappDatabase.messages.insertProfileNameChangeMessages(recipient, remoteDisplayName, localDisplayName)
        } else {
        ...
        if (recipient.isIndividual &&
          ...
          val threadId = GenZappDatabase.threads.getThreadIdFor(recipient.id)
          if (threadId != null && !RecipientUtil.isMessageRequestAccepted(threadId, recipient)) {
            GenZappDatabase.nameCollisions.handleIndividualNameCollision(recipient.id)
          }
        }
  ...
  private fun setProfileAbout(recipient: Recipient, encryptedAbout: String?, encryptedEmoji: String?) {
    try {
      ...
      GenZappDatabase.recipients.setAbout(recipient.id, plaintextAbout, plaintextEmoji)
    } catch (e: InvalidCiphertextException) {
      Log.w(TAG, e)
    } catch (e: IOException) {
      Log.w(TAG, e)
  ...
  private fun clearUsername(recipient: Recipient) {
    GenZappDatabase.recipients.setUsername(recipient.id, null)
  }

  private fun setProfileCapabilities(recipient: Recipient, capabilities: GenZappServiceProfile.Capabilities?) {
    if (capabilities == null) {
      return
    }

    GenZappDatabase.recipients.setCapabilities(recipient.id, capabilities)
  }
  ...
  private fun setPhoneNumberSharingMode(recipient: Recipient, phoneNumberSharing: String?) {
    ...
    try {
      ...
      if (recipient.phoneNumberSharing !== remotePhoneNumberSharing) {
        ...
        GenZappDatabase.recipients.setPhoneNumberSharing(recipient.id, remotePhoneNumberSharing)
      }
    } catch (e: InvalidCiphertextException) {
  ...
  private fun setProfileAvatar(recipient: Recipient, profileAvatar: String?) {
    ...
    if (profileAvatar != recipient.profileAvatar) {
      GenZappDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_RETRIEVE_AVATAR + recipient.id) {
  ...
  private fun getRequestType(recipient: Recipient): GenZappServiceProfile.RequestType {
    return if (ExpiringProfileCredentialUtil.isValid(recipient.expiringProfileKeyCredential)) GenZappServiceProfile.RequestType.PROFILE else GenZappServiceProfile.RequestType.PROFILE_AND_CREDENTIAL
  }
  ...
  companion object {
    ...
    fun forRecipients(recipientIds: Set<RecipientId>): List<Job> {
      ...
      for (recipientId in recipientIds) {
        ...
        when {
          ...
          recipient.isGroup -> combined += GenZappDatabase.groups.getGroupMemberIds(recipient.requireGroupId(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
    ...
    fun enqueueRoutineFetchIfNecessary() {
      if (!GenZappStore.registration.isRegistrationComplete || !GenZappStore.account.isRegistered || GenZappStore.account.aci == null) {
        Log.i(TAG, "Registration not complete. Skipping.")
        return
      }

      val timeSinceRefresh = System.currentTimeMillis() - GenZappStore.misc.lastProfileRefreshTime
      ...
      GenZappExecutors.BOUNDED.execute {
        ...
        val ids: List<RecipientId> = GenZappDatabase.recipients.getRecipientsForRoutineProfileFetch(
        ...
        GenZappStore.misc.lastProfileRefreshTime = System.currentTimeMillis()
      }
    }
  }
}
