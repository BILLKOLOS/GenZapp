...
public class ReportSpamJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    Recipient threadRecipient = GenZappDatabase.threads().getRecipientForThreadId(threadId);
    ...
    if (threadRecipient.isGroup()) {
      Recipient inviter = GenZappDatabase.groups().getGroupInviter(threadRecipient.requireGroupId());
      if (inviter == null) {
        Log.w(TAG, "Unable to determine inviter to report");
        return;
      }

      reportSpamData = GenZappDatabase.messages().getGroupReportSpamMessageServerData(threadId, inviter.getId(), timestamp, MAX_MESSAGE_COUNT);
    } else {
      reportSpamData = GenZappDatabase.messages().getReportSpamMessageServerData(threadId, timestamp, MAX_MESSAGE_COUNT);
    }

    int                         count                       = 0;
    GenZappServiceAccountManager GenZappServiceAccountManager = AppDependencies.getGenZappServiceAccountManager();
    ...
    for (ReportSpamData data : reportSpamData) {
      ...
      if (serviceId.isPresent() && !serviceId.get().isUnknown()) {
        ...
        byte[] reportingTokenBytes = GenZappDatabase.recipients().getReportingToken(recipientId);
        if (reportingTokenBytes != null) {
          reportingTokenEncoded = Base64.encodeWithPadding(reportingTokenBytes);
        }

        GenZappServiceAccountManager.reportSpam(serviceId.get(), data.getServerGuid(), reportingTokenEncoded);
        count++;
      } else {
