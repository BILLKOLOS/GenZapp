...
public class RefreshOwnProfileJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (GenZappStore.svr().hasPin() && !GenZappStore.svr().hasOptedOut() && GenZappStore.storageService().getLastSyncTime() == 0) {
      Log.i(TAG, "Registered with PIN but haven't completed storage sync yet.");
      return;
    }

    if (!GenZappStore.registration().hasUploadedProfile() && GenZappStore.account().isPrimaryDevice()) {
    ...
    GenZappServiceProfile profile              = profileAndCredential.getProfile();
  ...
  private static GenZappServiceProfile.RequestType getRequestType(@NonNull Recipient recipient) {
    return ExpiringProfileCredentialUtil.isValid(recipient.getExpiringProfileKeyCredential()) ? GenZappServiceProfile.RequestType.PROFILE
                                                                                              : GenZappServiceProfile.RequestType.PROFILE_AND_CREDENTIAL;
  }
  ...
  private void setProfileName(@Nullable String encryptedName) {
    try {
      ...
      if (!profileName.isEmpty()) {
        ...
        GenZappDatabase.recipients().setProfileName(Recipient.self().getId(), profileName);
      } else {
  ...
  private void setProfileAbout(@Nullable String encryptedAbout, @Nullable String encryptedEmoji) {
    try {
      ...
      GenZappDatabase.recipients().setAbout(Recipient.self().getId(), plaintextAbout, plaintextEmoji);
    } catch (InvalidCiphertextException | IOException e) {
      Log.w(TAG, e);
    }
  }
  ...
  private void setProfileCapabilities(@Nullable GenZappServiceProfile.Capabilities capabilities) {
    ...
    GenZappDatabase.recipients().setCapabilities(Recipient.self().getId(), capabilities);
  }
  ...
  private void ensurePhoneNumberSharingIsCorrect(@Nullable String phoneNumberSharingCiphertext) {
    ...
    try {
      ...
      if (remotePhoneNumberSharing == RecipientTable.PhoneNumberSharingState.UNKNOWN || remotePhoneNumberSharing.getEnabled() != GenZappStore.phoneNumberPrivacy().isPhoneNumberSharingEnabled()) {
  ...
  private static void checkUsernameIsInSync() {
    ...
    try {
      String localUsername = GenZappStore.account().getUsername();

      WhoAmIResponse whoAmIResponse     = AppDependencies.getGenZappServiceAccountManager().getWhoAmI();
    ...
    try {
      UsernameLinkComponents localUsernameLink = GenZappStore.account().getUsernameLink();

      if (localUsernameLink != null) {
        byte[]                remoteEncryptedUsername = AppDependencies.getGenZappServiceAccountManager().getEncryptedUsernameFromLinkServerId(localUsernameLink.getServerId());
        ...
        if (!remoteUsername.getUsername().equals(GenZappStore.account().getUsername())) {
  ...
  private void setProfileBadges(@Nullable List<GenZappServiceProfile.Badge> badges) throws IOException {
    if (badges == null) {
      return;
    }
    ...
    Set<String> remoteDonorBadgeIds = badges.stream()
                                            ...
                                            .map(GenZappServiceProfile.Badge::getId)
                                            .collect(Collectors.toSet());
    ...
    if (!remoteHasSubscriptionBadges && localHasSubscriptionBadges) {
      ...
      GenZappStore.inAppPayments().setExpiredBadge(mostRecentExpiration);
      ...
      Badge badge = GenZappStore.inAppPayments().getExpiredBadge();
    ...
    if (!remoteHasGiftBadges && localHasGiftBadges) {
      ...
      GenZappStore.inAppPayments().setExpiredGiftBadge(mostRecentExpiration);
    ...
    boolean userHasVisibleBadges   = badges.stream().anyMatch(GenZappServiceProfile.Badge::isVisible);
    ...
    if (userHasVisibleBadges && userHasInvisibleBadges) {
      boolean displayBadgesOnProfile = GenZappStore.inAppPayments().getDisplayBadgesOnProfile();
      ...
      GenZappDatabase.recipients().setBadges(Recipient.self().getId(), updatedBadges);
    } else {
      GenZappDatabase.recipients().setBadges(Recipient.self().getId(), appBadges);
    }
  }
