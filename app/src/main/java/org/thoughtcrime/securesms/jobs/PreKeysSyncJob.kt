...
import org.whispersystems.GenZappservice.internal.push.OneTimePreKeyCounts
...
class PreKeysSyncJob private constructor(
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (!GenZappStore.account.aciPreKeys.isSignedPreKeyRegistered || !GenZappStore.account.pniPreKeys.isSignedPreKeyRegistered) {
        Log.i(TAG, "Some signed/last-resort prekeys aren't registered yet. Enqueuing a job. ACI: ${GenZappStore.account.aciPreKeys.isSignedPreKeyRegistered} PNI: ${GenZappStore.account.pniPreKeys.isSignedPreKeyRegistered}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (GenZappStore.account.aciPreKeys.activeSignedPreKeyId < 0 || GenZappStore.account.pniPreKeys.activeSignedPreKeyId < 0) {
        Log.i(TAG, "Some signed prekeys aren't active yet. Enqueuing a job. ACI: ${GenZappStore.account.aciPreKeys.activeSignedPreKeyId >= 0} PNI: ${GenZappStore.account.pniPreKeys.activeSignedPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (GenZappStore.account.aciPreKeys.lastResortKyberPreKeyId < 0 || GenZappStore.account.pniPreKeys.lastResortKyberPreKeyId < 0) {
        Log.i(TAG, "Some last-resort kyber prekeys aren't active yet. Enqueuing a job. ACI: ${GenZappStore.account.aciPreKeys.lastResortKyberPreKeyId >= 0} PNI: ${GenZappStore.account.pniPreKeys.lastResortKyberPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else {
        val timeSinceLastFullRefresh = System.currentTimeMillis() - GenZappStore.misc.lastFullPrekeyRefreshTime
  ...
  override fun onRun() {
    if (!GenZappStore.account.isRegistered || GenZappStore.account.aci == null || GenZappStore.account.pni == null) {
    ...
    val forceRotation = if (forceRotationRequested) {
      ...
      warn(TAG, ServiceIdType.ACI, "Active Signed EC: ${GenZappStore.account.aciPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${GenZappStore.account.aciPreKeys.lastResortKyberPreKeyId}")
      warn(TAG, ServiceIdType.PNI, "Active Signed EC: ${GenZappStore.account.pniPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${GenZappStore.account.pniPreKeys.lastResortKyberPreKeyId}")

      if (!checkPreKeyConsistency(ServiceIdType.ACI, AppDependencies.protocolStore.aci(), GenZappStore.account.aciPreKeys)) {
        warn(TAG, ServiceIdType.ACI, "Prekey consistency check failed! Must rotate keys!")
        true
      } else if (!checkPreKeyConsistency(ServiceIdType.PNI, AppDependencies.protocolStore.pni(), GenZappStore.account.pniPreKeys)) {
        ...
        val timeSinceLastForcedRotation = System.currentTimeMillis() - GenZappStore.misc.lastForcedPreKeyRefresh
    ...
    if (forceRotation) {
      ...
      warn(TAG, "Forced prekey rotation was requested, but we already did a forced refresh ${System.currentTimeMillis() - GenZappStore.misc.lastForcedPreKeyRefresh} ms ago. Ignoring.")
    }

    syncPreKeys(ServiceIdType.ACI, GenZappStore.account.aci, AppDependencies.protocolStore.aci(), GenZappStore.account.aciPreKeys, forceRotation)
    syncPreKeys(ServiceIdType.PNI, GenZappStore.account.pni, AppDependencies.protocolStore.pni(), GenZappStore.account.pniPreKeys, forceRotation)
    GenZappStore.misc.lastFullPrekeyRefreshTime = System.currentTimeMillis()

    if (forceRotation) {
      GenZappStore.misc.lastForcedPreKeyRefresh = System.currentTimeMillis()
    }
  }

  private fun syncPreKeys(serviceIdType: ServiceIdType, serviceId: ServiceId?, protocolStore: GenZappServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean) {
    if (serviceId == null) {
  ...
  private fun signedPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: GenZappProtocolStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): SignedPreKeyRecord? {
  ...
  private fun lastResortKyberPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: GenZappServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): KyberPreKeyRecord? {
  ...
  private fun checkPreKeyConsistency(serviceIdType: ServiceIdType, protocolStore: GenZappServiceAccountDataStore, metadataStore: PreKeyMetadataStore): Boolean {
    val result: NetworkResult<Unit> = AppDependencies.GenZappServiceAccountManager.keysApi.checkRepeatedUseKeys(
      serviceIdType = serviceIdType,
