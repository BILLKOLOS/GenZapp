...
class StoryOnboardingDownloadJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueueIfNeeded() {
      if (GenZappStore.story.hasDownloadedOnboardingStory) {
        return
      }
  ...
  override fun onRun() {
    ...
    val releaseChannelRecipientId = GenZappStore.releaseChannel.releaseChannelRecipientId
    ...
    GenZappDatabase.messages.getAllStoriesFor(releaseChannelRecipientId, -1).use { reader ->
      reader.forEach { messageRecord ->
        GenZappDatabase.messages.deleteMessage(messageRecord.id)
      }
    }
    ...
    val threadId = GenZappDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(releaseChannelRecipientId))
    ...
    if (insertResults.size != ONBOARDING_IMAGE_COUNT) {
      ...
      insertResults.forEach {
        GenZappDatabase.messages.deleteMessage(it.messageId)
      }
    ...
    GenZappStore.story.hasDownloadedOnboardingStory = true

    Log.i(TAG, "Enqueueing download jobs...")
    insertResults.forEach { insertResult ->
      GenZappDatabase.attachments.getAttachmentsForMessage(insertResult.messageId).forEach {
  ...
  private fun getLocaleCodes(): List<String> {
    ...
    if (GenZappStore.settings.language != "zz") {
      potentialOnboardingUrlLanguages += GenZappStore.settings.language
    }
