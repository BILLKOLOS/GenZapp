...
public final class EmojiSearchIndexDownloadJob extends BaseJob {
  ...
  public static void scheduleIfNecessary() {
    long    timeSinceCheck = System.currentTimeMillis() - GenZappStore.emoji().getLastSearchIndexCheck();
    boolean needsCheck     = false;

    if (GenZappStore.emoji().hasSearchIndex()) {
  ...
  protected void onRun() throws Exception {
    ...
    if (manifest.getVersion() == GenZappStore.emoji().getSearchVersion() &&
        remoteLanguage.equals(GenZappStore.emoji().getSearchLanguage()))
    {
      Log.i(TAG, "Already using the latest version of " + manifest.getVersion() + " with the correct language " + remoteLanguage);
      GenZappStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
      return;
    }
    ...
    GenZappDatabase.emojiSearch().setSearchIndex(searchIndex);
    GenZappStore.emoji().onSearchIndexUpdated(manifest.getVersion(), remoteLanguage);
    GenZappStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
