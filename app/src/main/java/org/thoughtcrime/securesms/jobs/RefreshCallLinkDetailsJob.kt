...
class RefreshCallLinkDetailsJob private constructor(
  ...
  override fun onRun() {
    val manager: GenZappCallLinkManager = AppDependencies.GenZappCallManager.callLinkManager
    ...
    when (val result = manager.readCallLink(credentials).safeBlockingGet()) {
      is ReadCallLinkResult.Success -> {
        GenZappDatabase.callLinks.updateCallLinkState(credentials.roomId, result.callLinkState)
      }
      else -> Unit
    }
  }
