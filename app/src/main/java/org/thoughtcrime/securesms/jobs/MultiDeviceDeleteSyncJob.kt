...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.pad
...
import org.whispersystems.GenZappservice.internal.push.SyncMessage.DeleteForMe
...
class MultiDeviceDeleteSyncJob private constructor(
  ...
  companion object {
    ...
    private fun createMessageDeletes(messageRecords: Collection<MessageRecord>): List<AddressableMessage> {
      return messageRecords.mapNotNull { message ->
        val threadRecipient = GenZappDatabase.threads.getRecipientForThreadId(message.threadId)
  ...
  private fun syncDelete(deleteForMe: DeleteForMe): Boolean {
    ...
    return try {
      ...
      AppDependencies.GenZappServiceMessageSender.sendSyncMessage(syncMessageContent, true, Optional.empty()).isSuccess
    } catch (e: IOException) {
