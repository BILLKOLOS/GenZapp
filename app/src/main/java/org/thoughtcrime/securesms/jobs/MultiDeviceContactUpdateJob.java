...
import org.whispersystems.GenZappservice.api.util.InvalidNumberException;
...
public class MultiDeviceContactUpdateJob extends BaseJob {
      ...
      throws IOException, UntrustedIdentityException, NetworkException
  {
    ...
    try {
      ...
      Map<RecipientId, Integer> inboxPositions  = GenZappDatabase.threads().getInboxPositions();
      Set<RecipientId>          archived        = GenZappDatabase.threads().getArchivedRecipients();
      ...
      sendUpdate(AppDependencies.getGenZappServiceMessageSender(),
      ...
      throws IOException, UntrustedIdentityException, NetworkException
  {
    ...
    try {
      DeviceContactsOutputStream out            = new DeviceContactsOutputStream(writeDetails.outputStream);
      List<Recipient>            recipients     = GenZappDatabase.recipients().getRecipientsForMultiDeviceSync();
      Map<RecipientId, Integer>  inboxPositions = GenZappDatabase.threads().getInboxPositions();
      Set<RecipientId>           archived       = GenZappDatabase.threads().getArchivedRecipients();
      ...
      if (profileKey != null) {
        out.write(new DeviceContact(Optional.of(GenZappStore.account().getAci()),
                                    Optional.of(GenZappStore.account().getE164()),
                                    Optional.empty(),
                                    Optional.empty(),
  ...
  private void sendUpdate(GenZappServiceMessageSender messageSender, InputStream stream, long length, boolean complete)
      throws UntrustedIdentityException, NetworkException
  {
    if (length > 0) {
      try {
        GenZappServiceAttachmentStream.Builder attachmentStream = GenZappServiceAttachment.newStreamBuilder()
                                                                                        .withStream(stream)
                                                                                        .withContentType("application/octet-stream")
                                                                                        .withLength(length)
                                                                                        .withResumableUploadSpec(messageSender.getResumableUploadSpec());

        messageSender.sendSyncMessage(GenZappServiceSyncMessage.forContacts(new ContactsMessage(attachmentStream.build(), complete))
        );
  ...
  private Optional<GenZappServiceAttachmentStream> getSystemAvatar(@Nullable Uri uri) throws IOException {
    if (uri == null) {
    ...
    try {
      ...
      return Optional.of(GenZappServiceAttachment.newStreamBuilder()
      ...
      throws InvalidNumberException, IOException
  {
    ...
    GenZappServiceAddress destination = RecipientUtil.toGenZappServiceAddress(context, recipient);
