...
import org.whispersystems.GenZappservice.api.messages.multidevice.DeviceContactsInputStream
import org.whispersystems.GenZappservice.api.messages.multidevice.VerifiedMessage.VerifiedState
...
class MultiDeviceContactSyncJob(parameters: Parameters, private val attachmentPointer: ByteArray) : BaseJob(parameters) {
  ...
  constructor(contactsAttachment: GenZappServiceAttachmentPointer) : this(
    Parameters.Builder()
  ...
  override fun onRun() {
    ...
    val contactAttachment: GenZappServiceAttachmentPointer = AttachmentPointerUtil.createGenZappAttachmentPointer(attachmentPointer)

    try {
      val contactsFile: File = BlobProvider.getInstance().forNonAutoEncryptingSingleSessionOnDisk(context)
      AppDependencies.GenZappServiceMessageReceiver
  ...
  private fun processContactFile(inputStream: InputStream) {
    ...
    val recipients = GenZappDatabase.recipients
    val threads = GenZappDatabase.threads

    var contact: DeviceContact? = deviceContacts.read()
    while (contact != null) {
      val recipient = if (contact.aci.isPresent) {
        Recipient.externalPush(GenZappServiceAddress(contact.aci.get(), contact.e164.orElse(null)))
      } else {
