...
class AttachmentHashBackfillJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    val (file: File?, attachmentId: AttachmentId?) = GenZappDatabase.attachments.getUnhashedDataFile() ?: (null to null)
    ...
    if (!file.exists()) {
      ...
      GenZappDatabase.attachments.clearUsagesOfDataFile(file)
      AppDependencies.jobManager.add(AttachmentHashBackfillJob())
      return Result.success()
    }

    try {
      val inputStream = GenZappDatabase.attachments.getAttachmentStream(attachmentId, 0)
      ...
      GenZappDatabase.attachments.setHashForDataFile(file, hash)
