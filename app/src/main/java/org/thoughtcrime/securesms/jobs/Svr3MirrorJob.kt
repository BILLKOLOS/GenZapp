...
import org.whispersystems.GenZappservice.api.svr.SecureValueRecoveryV3
...
class Svr3MirrorJob private constructor(parameters: Parameters, private var serializedChangeSession: String?) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    SvrRepository.operationLock.withLock {
      ...
      val svr3: SecureValueRecoveryV3 = AppDependencies.GenZappServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libGenZappNetwork)

      val session: PinChangeSession = serializedChangeSession?.let { session ->
        svr3.resumePinChangeSession(pin, GenZappStore.svr.getOrCreateMasterKey(), session)
      } ?: svr3.setPin(pin, GenZappStore.svr.getOrCreateMasterKey())
      ...
      return when (val response: BackupResponse = session.execute()) {
        is BackupResponse.Success -> {
          ...
          GenZappStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
