...
class RetrieveRemoteAnnouncementsJob private constructor(private val force: Boolean, parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueue(force: Boolean = false) {
      ...
      if (!force && System.currentTimeMillis() < GenZappStore.releaseChannel.nextScheduledCheck) {
  ...
  override fun onRun() {
    ...
    val values = GenZappStore.releaseChannel
  ...
  private fun updateReleaseNotes(announcements: List<ReleaseNote>) {
    ...
    if (!values.hasMetConversationRequirement) {
      if ((GenZappDatabase.threads.getArchivedConversationListCount(ConversationFilter.OFF) + GenZappDatabase.threads.getUnarchivedConversationListCount(ConversationFilter.OFF)) < 6) {
    ...
    val threadId = GenZappDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(values.releaseChannelRecipientId!!))
    ...
    if (addedNewNotes) {
      ThreadUtil.sleep(5)
      GenZappDatabase.messages.insertBoostRequestMessage(values.releaseChannelRecipientId!!, threadId)
    }
  ...
  private fun updateMegaphones(megaphones: List<RemoteMegaphone>) {
    ...
    val existingMegaphones: Map<String, RemoteMegaphoneRecord> = GenZappDatabase.remoteMegaphones.getAll().associateBy { it.uuid }
    ...
    resolvedMegaphones
      ...
      .forEach { megaphone ->
        ...
        if (existingMegaphones.contains(uuid)) {
          GenZappDatabase.remoteMegaphones.update(
            uuid = uuid,
    ...
    if (megaphonesToDelete.isNotEmpty()) {
      ...
      for ((uuid, megaphone) in megaphonesToDelete) {
        ...
        GenZappDatabase.remoteMegaphones.clear(uuid)
      }
    }
  }
  ...
  private fun String.getLocaleUrls(): List<String> {
    ...
    if (GenZappStore.settings.language != "zz") {
      potentialNoteUrls += "$this/${GenZappStore.settings.language}.json"
    }
