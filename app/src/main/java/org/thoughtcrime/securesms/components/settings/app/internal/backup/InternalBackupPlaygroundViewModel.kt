...
class InternalBackupPlaygroundViewModel : ViewModel() {
  ...
  fun loadMedia() {
    disposables += Single
      .fromCallable { GenZappDatabase.attachments.debugGetLatestAttachments() }
      .subscribeOn(Schedulers.io())
  ...
  fun restoreArchivedMedia(attachment: BackupAttachment) {
    disposables += Completable
      .fromCallable {
        ...
        val insertMessage = GenZappDatabase.messages.insertMessageInbox(message, threadId).get()

        GenZappDatabase.attachments.debugCopyAttachmentForArchiveRestore(
          insertMessage.messageId,
          attachment.dbAttachment
        )

        val archivedAttachment = GenZappDatabase.attachments.getAttachmentsForMessage(insertMessage.messageId).first()
