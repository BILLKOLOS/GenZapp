...
import org.GenZapp.donations.json.StripeIntentStatus
...
import org.whispersystems.GenZappservice.api.subscriptions.StripeClientSecret
 ...
 * 1. Generate and send a SubscriberId, which is a 32 byte ID representing this user, to GenZapp Service, which creates a Stripe Customer
 ...
 * 1. Set the default PaymentMethod for the customer, using the PaymentMethod id, via the GenZapp service
 *
...
class StripeRepository(
  ...
  override fun fetchPaymentIntent(price: FiatMoney, level: Long, sourceType: PaymentSourceType.Stripe): Single<StripeIntentAccessor> {
    Log.d(TAG, "Fetching payment intent from GenZapp service for $price... (Locale.US minimum precision: ${price.minimumUnitPrecisionString})")
    return Single
      .fromCallable {
        AppDependencies
          .donationsService
      ...
      .map {
        ...
        Log.d(TAG, "Got payment intent from GenZapp service!")
      }
  }

  /**
   * Creates the PaymentMethod via the GenZapp Service. Note that if the operation fails with a 409,
  ...
  override fun fetchSetupIntent(inAppPaymentType: InAppPaymentType, sourceType: PaymentSourceType.Stripe): Single<StripeIntentAccessor> {
    Log.d(TAG, "Fetching setup intent from GenZapp service...")
    return createPaymentMethod(inAppPaymentType.requireSubscriberType(), sourceType)
      ...
      .doOnSuccess {
        Log.d(TAG, "Got setup intent from GenZapp service!")
      }
  }
  ...
  fun setDefaultPaymentMethod(
    ...
    return Single.fromCallable {
      ...
      Log.d(TAG, "Setting default payment method via GenZapp service...")
      Single.fromCallable {
        ...
        Log.d(TAG, "Set default payment method via GenZapp service!")
        Log.d(TAG, "Storing the subscription payment source type locally.")
        GenZappDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriberRecord.subscriberId, paymentSourceType.toPaymentMethodType())
      }
    }
  }
