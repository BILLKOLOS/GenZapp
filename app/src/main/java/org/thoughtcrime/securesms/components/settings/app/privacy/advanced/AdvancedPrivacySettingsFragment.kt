...
class AdvancedPrivacySettingsFragment : DSLSettingsFragment(R.string.preferences__advanced) {
  ...
  private val sealedSenderSummary: CharSequence by lazy {
    SpanUtil.learnMore(
      requireContext(),
      ContextCompat.getColor(requireContext(), R.color.GenZapp_text_primary)
    ) {
  ...
  var progressDialog: GenZappProgressDialog? = null
  ...
  val statusIcon: CharSequence by lazy {
    ...
    val iconTint = ContextCompat.getColor(requireContext(), R.color.GenZapp_text_primary_dialog)
    ...
    viewModel.state.observe(viewLifecycleOwner) {
      if (it.showProgressSpinner) {
        if (progressDialog?.isShowing == false) {
          progressDialog = GenZappProgressDialog.show(requireContext(), null, null, true)
        }
      } else {
        progressDialog?.hide()
      }
  ...
  private fun getConfiguration(state: AdvancedPrivacySettingsState): DSLConfiguration {
    return configure {
      switchPref(
        ...
        summary = DSLSettingsText.from(R.string.preferences_advanced__relay_all_calls_through_the_GenZapp_server_to_avoid_revealing_your_ip_address),
      ...
      val censorshipSummaryResId: Int = when (state.censorshipCircumventionState) {
        CensorshipCircumventionState.AVAILABLE -> R.string.preferences_communication__censorship_circumvention_if_enabled_GenZapp_will_attempt_to_circumvent_censorship
  ...
  private fun getPushToggleSummary(isPushEnabled: Boolean): String {
    return if (isPushEnabled) {
      PhoneNumberFormatter.prettyPrint(GenZappStore.account.e164!!)
    } else {
