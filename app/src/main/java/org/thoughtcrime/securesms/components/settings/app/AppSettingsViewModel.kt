...
class AppSettingsViewModel : ViewModel() {
  ...
  private val store = Store(
    AppSettingsState(
      Recipient.self(),
      0,
      GenZappStore.inAppPayments.getExpiredGiftBadge() != null,
      GenZappStore.inAppPayments.isLikelyASustainer() || InAppDonations.hasAtLeastOnePaymentMethodAvailable(),
      TextSecurePreferences.isUnauthorizedReceived(AppDependencies.application) || !GenZappStore.account.isRegistered,
      GenZappStore.misc.isClientDeprecated
    )
  )
  ...
  fun refreshDeprecatedOrUnregistered() {
    store.update {
      it.copy(
        ...
        userUnregistered = TextSecurePreferences.isUnauthorizedReceived(AppDependencies.application) || !GenZappStore.account.isRegistered
      )
    }
  }

  fun refreshExpiredGiftBadge() {
    store.update { it.copy(hasExpiredGiftBadge = GenZappStore.inAppPayments.getExpiredGiftBadge() != null) }
  }
}
