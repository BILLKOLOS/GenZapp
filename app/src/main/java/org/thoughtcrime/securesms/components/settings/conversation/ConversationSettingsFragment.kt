...
import org.GenZapp.core.util.getParcelableArrayListExtraCompat
...
class ConversationSettingsFragment : DSLSettingsFragment(
  ...
  private val alertTint by lazy { ContextCompat.getColor(requireContext(), R.color.GenZapp_alert_primary) }
  private val alertDisabledTint by lazy { ContextCompat.getColor(requireContext(), R.color.GenZapp_alert_primary_50) }
  ...
  override fun getMaterial3OnScrollHelper(toolbar: Toolbar?): Material3OnScrollHelper {
    return object : Material3OnScrollHelper(requireActivity(), toolbar!!, viewLifecycleOwner) {
      override val inactiveColorSet = ColorSet(
        toolbarColorRes = R.color.GenZapp_colorBackground_0,
        statusBarColorRes = R.color.GenZapp_colorBackground
      )
    }
  }
  ...
  private fun getConfiguration(state: ConversationSettingsState): DSLConfiguration {
    return configure {
      ...
      if (state.canModifyBlockedState) {
        ...
        val reportSpamTint = if (state.isDeprecatedOrUnregistered) R.color.GenZapp_alert_primary_50 else R.color.GenZapp_alert_primary
        clickPref(
