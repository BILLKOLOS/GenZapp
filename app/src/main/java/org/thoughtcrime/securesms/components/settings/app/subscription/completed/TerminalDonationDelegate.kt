...
class TerminalDonationDelegate(
  ...
  override fun onResume(owner: LifecycleOwner) {
    val donations = GenZappStore.inAppPayments.consumeTerminalDonations()
    ...
    val verifiedMonthlyDonation: Stripe3DSData? = GenZappStore.inAppPayments.consumeVerifiedSubscription3DSData()
  ...
  private fun handleInAppPaymentSheets() {
    lifecycleDisposable += Single.fromCallable {
      GenZappDatabase.inAppPayments.consumeDonationPaymentsToNotifyUser()
      ...
      for (payment in inAppPayments) {
        ...
        } else if (payment.data.error != null && payment.data.cancellation != null && payment.data.cancellation.reason != InAppPaymentData.Cancellation.Reason.MANUAL && GenZappStore.inAppPayments.showMonthlyDonationCanceledDialog) {
