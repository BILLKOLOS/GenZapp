...
class AccountSettingsFragment : DSLSettingsFragment(R.string.AccountSettingsFragment__account) {
  ...
  private fun getConfiguration(state: AccountSettingsState): DSLConfiguration {
    return configure {
      sectionHeaderPref(R.string.preferences_app_protection__GenZapp_pin)
      ...
      switchPref(
        ...
        summary = DSLSettingsText.from(R.string.AccountSettingsFragment__require_your_GenZapp_pin),
      ...
      if (!state.isDeprecatedOrUnregistered()) {
        if (state.clientDeprecated) {
          clickPref(
            title = DSLSettingsText.from(R.string.preferences_account_update_GenZapp),
            onClick = {
        ...
        clickPref(
          title = DSLSettingsText.from(R.string.preferences_account_delete_all_data, ContextCompat.getColor(requireContext(), R.color.GenZapp_alert_primary)),
          onClick = {
      ...
      clickPref(
        title = DSLSettingsText.from(R.string.preferences__delete_account, ContextCompat.getColor(requireContext(), if (state.isDeprecatedOrUnregistered()) R.color.GenZapp_alert_primary else R.color.GenZapp_alert_primary_50)),
        isEnabled = state.isDeprecatedOrUnregistered(),
        onClick = {
  ...
  private fun setPinRemindersEnabled(enabled: Boolean) {
    if (!enabled) {
      ...
      when (GenZappStore.pin.keyboardType) {
      ...
      turnOffButton.setOnClickListener {
        ...
        val correct = PinHashUtil.verifyLocalPinHash(GenZappStore.svr.localPinHash!!, pin)
        if (correct) {
          GenZappStore.pin.setPinRemindersEnabled(false)
          viewModel.refreshState()
          dialog.dismiss()
        } else {
      ...
      GenZappStore.pin.setPinRemindersEnabled(true)
      viewModel.refreshState()
    }
  }
}
