 ...
 *
 * Overflow items are rendered in a [GenZappContextMenu].
 */
class GenZappBottomActionBar(context: Context, attributeSet: AttributeSet) : LinearLayout(context, attributeSet) {
  ...
  init {
    ...
    setBackgroundResource(R.drawable.GenZapp_bottom_action_bar_background)
    elevation = 20f
  }
  ...
  private fun present(items: List<ActionItem>) {
    ...
    renderableItems.forEach { item ->
      val view: View = LayoutInflater.from(context).inflate(R.layout.GenZapp_bottom_action_bar_item, this, false)
      addView(view)
      bindItem(view, item)
    }
    ...
    if (overflowItems.isNotEmpty()) {
      ...
      bindItem(
        ...
        ActionItem(
          ...
          title = context.getString(R.string.GenZappBottomActionBar_more),
          action = {
            GenZappContextMenu.Builder(view, parent as ViewGroup)
              .preferredHorizontalPosition(GenZappContextMenu.HorizontalPosition.END)
  ...
  private fun bindItem(view: View, item: ActionItem) {
    val icon: ImageView = view.findViewById(R.id.GenZapp_bottom_action_bar_item_icon)
    val title: TextView = view.findViewById(R.id.GenZapp_bottom_action_bar_item_title)
