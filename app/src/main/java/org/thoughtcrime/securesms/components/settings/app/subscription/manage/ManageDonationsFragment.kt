...
class ManageDonationsFragment :
  ...
  companion object {
    ...
    const val DONATE_TROUBLESHOOTING_URL = "https://support.GenZapp.org/hc/articles/360031949872#fix"
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    val expiredGiftBadge = GenZappStore.inAppPayments.getExpiredGiftBadge()
  ...
  private fun getConfiguration(state: ManageDonationsState): DSLConfiguration {
    return configure {
      ...
      primaryWrappedButton(
        text = DSLSettingsText.from(R.string.ManageDonationsFragment__donate_to_GenZapp),
        onClick = {
          findNavController().safeNavigate(ManageDonationsFragmentDirections.actionManageDonationsFragmentToDonateToGenZappFragment(InAppPaymentType.ONE_TIME_DONATION))
        }
      )
  ...
  private fun DSLConfiguration.presentNetworkFailureSettings(state: ManageDonationsState, hasReceipts: Boolean) {
    if (GenZappStore.inAppPayments.isLikelyASustainer()) {
        ...
        ActiveSubscriptionPreference.Model(
          ...
          onRowClick = {
            if (it != ManageDonationsState.RedemptionState.IN_PROGRESS) {
              findNavController().safeNavigate(ManageDonationsFragmentDirections.actionManageDonationsFragmentToDonateToGenZappFragment(InAppPaymentType.RECURRING_DONATION))
            }
          },
          onPendingClick = {
            displayPendingDialog(it)
  ...
  private fun DSLConfiguration.presentOtherWaysToGive() {
    ...
    clickPref(
      ...
      onClick = {
        findNavController().safeNavigate(ManageDonationsFragmentDirections.actionManageDonationsFragmentToDonateToGenZappFragment(InAppPaymentType.ONE_TIME_GIFT))
      }
    )
  }
