...
sealed class ConversationSettingsViewModel(
  ...
  protected val store = Store(
    ConversationSettingsState(
      specificSettingsState = specificSettingsState,
      isDeprecatedOrUnregistered = GenZappStore.misc.isClientDeprecated || TextSecurePreferences.isUnauthorizedReceived(AppDependencies.application)
    )
  )
  ...
  init {
    ...
    val sharedMedia: LiveData<List<MediaTable.MediaRecord>> = LiveDataUtil.mapAsync(GenZappExecutors.BOUNDED, updater) { tId ->
      ...
      store.update(recipientAndIsActive) { (recipient, isActive), state ->
        state.copy(
          ...
          buttonStripState = ButtonStripPreference.State(
            ...
            isSearchAvailable = callMessageIds.isEmpty(),
            isAddToStoryAvailable = recipient.isPushV2Group && !recipient.isBlocked && isActive && !GenZappStore.story.isFeatureDisabled
          ),
