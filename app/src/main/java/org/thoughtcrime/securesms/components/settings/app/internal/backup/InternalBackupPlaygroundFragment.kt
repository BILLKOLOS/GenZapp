...
import org.GenZapp.core.util.bytes
import org.GenZapp.core.util.getLength
  ...
  override fun FragmentContent() {
    ...
    Tabs(
      ...
      mediaContent = { snackbarHostState ->
        MediaList(
          enabled = GenZappStore.backup.backsUpMedia,
          state = mediaState,
    ...
    topBar = {
      Column {
        TopAppBar(
          ...
          actions = {
            if (tabIndex == 1 && GenZappStore.backup.backsUpMedia) {
...
fun MediaList(
  ...
  if (!enabled) {
    Text(
      text = "You do not have read/write to archive cdn enabled via GenZappStore.backup",
      modifier = Modifier
        .padding(16.dp)
    )
    return
