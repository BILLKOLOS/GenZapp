...
class AdvancedPrivacySettingsViewModel(
  ...
  fun setCensorshipCircumventionEnabled(enabled: Boolean) {
    GenZappStore.settings.setCensorshipCircumventionEnabled(enabled)
  ...
  private fun getState(): AdvancedPrivacySettingsState {
    ...
    return AdvancedPrivacySettingsState(
      isPushEnabled = GenZappStore.account.isRegistered,
  ...
  private fun getCensorshipCircumventionState(): CensorshipCircumventionState {
    ...
    val isCountryCodeCensoredByDefault: Boolean = AppDependencies.GenZappServiceNetworkAccess.isCountryCodeCensoredByDefault(countryCode)
    val enabledState: SettingsValues.CensorshipCircumventionEnabled = GenZappStore.settings.censorshipCircumventionEnabled
    val hasInternet: Boolean = NetworkConstraint.isMet(AppDependencies.application)
    val websocketConnected: Boolean = AppDependencies.GenZappWebSocket.webSocketState.firstOrError().blockingGet() == WebSocketConnectionState.CONNECTED

    return when {
      GenZappStore.internal.allowChangingCensorshipSetting() -> {
  ...
  private fun getCensorshipCircumventionEnabled(state: CensorshipCircumventionState): Boolean {
    return when (state) {
      ...
      CensorshipCircumventionState.AVAILABLE_AUTOMATICALLY_ENABLED -> {
        ...
        GenZappStore.settings.censorshipCircumventionEnabled == SettingsValues.CensorshipCircumventionEnabled.ENABLED
      }
    }
  }
