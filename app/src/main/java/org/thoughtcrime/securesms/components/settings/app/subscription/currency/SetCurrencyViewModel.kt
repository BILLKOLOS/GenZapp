...
class SetCurrencyViewModel(
  ...
  private val store = Store(
    SetCurrencyState(
      selectedCurrencyCode = if (inAppPaymentType.recurring) {
        GenZappStore.inAppPayments.getSubscriptionCurrency(inAppPaymentType.requireSubscriberType()).currencyCode
      } else {
        GenZappStore.inAppPayments.getOneTimeCurrency().currencyCode
      },
  ...
  fun setSelectedCurrency(selectedCurrencyCode: String) {
    ...
    if (!inAppPaymentType.recurring) {
      GenZappStore.inAppPayments.setOneTimeCurrency(Currency.getInstance(selectedCurrencyCode))
    } else {
