...
import org.GenZapp.core.util.getParcelableCompat
import org.GenZapp.core.util.getSerializableCompat
...
import org.thoughtcrime.securesms.databinding.DonateToGenZappFragmentBinding
 ...
 */
class DonateToGenZappFragment :
  DSLSettingsFragment(
    layoutId = R.layout.donate_to_GenZapp_fragment
  ),
  ThanksForYourSupportBottomSheetDialogFragment.Callback,
  InAppPaymentCheckoutDelegate.Callback {

  companion object {
    private val TAG = Log.tag(DonateToGenZappFragment::class.java)
  }
  ...
  private val args: DonateToGenZappFragmentArgs by navArgs()
  private val viewModel: DonateToGenZappViewModel by viewModels(factoryProducer = {
    DonateToGenZappViewModel.Factory(args.startType)
  })

  private val disposables = LifecycleDisposable()
  private val binding by ViewBinderDelegate(DonateToGenZappFragmentBinding::bind)

  private val supportTechSummary: CharSequence by lazy {
    SpannableStringBuilder(SpanUtil.color(ContextCompat.getColor(requireContext(), R.color.GenZapp_colorOnSurfaceVariant), requireContext().getString(R.string.DonateToGenZappFragment__private_messaging)))
      .append(" ")
      .append(
        SpanUtil.readMore(requireContext(), ContextCompat.getColor(requireContext(), R.color.GenZapp_colorPrimary)) {
          findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToSubscribeLearnMoreBottomSheetDialog())
        }
      )
  }
  ...
  override fun getMaterial3OnScrollHelper(toolbar: Toolbar?): Material3OnScrollHelper {
    return object : Material3OnScrollHelper(requireActivity(), toolbar!!, viewLifecycleOwner) {
      override val activeColorSet: ColorSet = ColorSet(R.color.transparent, R.color.GenZapp_colorBackground)
      override val inactiveColorSet: ColorSet = ColorSet(R.color.transparent, R.color.GenZapp_colorBackground)
    }
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    disposables += viewModel.actions.subscribe { action ->
      when (action) {
        is DonateToGenZappAction.DisplayCurrencySelectionDialog -> {
          val navAction = DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToSetCurrencyFragment(
        ...
        is DonateToGenZappAction.DisplayGatewaySelectorDialog -> {
          Log.d(TAG, "Presenting gateway selector for ${action.inAppPayment.id}")
          val navAction = DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToGatewaySelectorBottomSheetDialog(action.inAppPayment)

          findNavController().safeNavigate(navAction)
        }

        is DonateToGenZappAction.CancelSubscription -> {
          val navAction = DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToStripePaymentInProgressFragment(
        ...
        is DonateToGenZappAction.UpdateSubscription -> {
          if (action.inAppPayment.data.paymentMethodType == InAppPaymentData.PaymentMethodType.PAYPAL) {
            val navAction = DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToPaypalPaymentInProgressFragment(
  ...
  private fun getConfiguration(state: DonateToGenZappState): DSLConfiguration {
    return configure {
      space(36.dp)
      ...
      noPadTextPref(
        title = DSLSettingsText.from(
          R.string.DonateToGenZappFragment__privacy_over_profit,
      ...
      if (state.inAppPaymentType == InAppPaymentType.RECURRING_DONATION && state.monthlyDonationState.isSubscriptionActive) {
        ...
        primaryButton(
          text = DSLSettingsText.from(R.string.DonateToGenZappFragment__continue),
  ...
  private fun showDonationPendingDialog(state: DonateToGenZappState) {
    val message = if (state.inAppPaymentType == InAppPaymentType.ONE_TIME_DONATION) {
      if (state.oneTimeDonationState.isOneTimeDonationLongRunning) {
        R.string.DonateToGenZappFragment__bank_transfers_usually_take_1_business_day_to_process_onetime
      } else if (state.oneTimeDonationState.isNonVerifiedIdeal) {
        R.string.DonateToGenZappFragment__your_ideal_payment_is_still_processing
      } else {
        R.string.DonateToGenZappFragment__your_payment_is_still_being_processed_onetime
      }
    } else {
      if (state.monthlyDonationState.activeSubscription?.paymentMethod == ActiveSubscription.PAYMENT_METHOD_SEPA_DEBIT) {
        R.string.DonateToGenZappFragment__bank_transfers_usually_take_1_business_day_to_process_monthly
      } else if (state.monthlyDonationState.nonVerifiedMonthlyDonation != null) {
        R.string.DonateToGenZappFragment__your_ideal_payment_is_still_processing
      } else {
        R.string.DonateToGenZappFragment__your_payment_is_still_being_processed_monthly
      }
    }

    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToGenZappFragment__you_have_a_donation_pending)
      .setMessage(message)
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }

  private fun DSLConfiguration.displayOneTimeSelection(areFieldsEnabled: Boolean, state: DonateToGenZappState.OneTimeDonationState) {
    when (state.donationStage) {
      DonateToGenZappState.DonationStage.INIT -> customPref(Boost.LoadingModel())
      DonateToGenZappState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryOneTimeDonationState() })
      DonateToGenZappState.DonationStage.READY -> {
        customPref(
  ...
  private fun DSLConfiguration.displayMonthlySelection(areFieldsEnabled: Boolean, state: DonateToGenZappState.MonthlyDonationState) {
    when (state.donationStage) {
      DonateToGenZappState.DonationStage.INIT -> customPref(Subscription.LoaderModel())
      DonateToGenZappState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryMonthlyDonationState() })
      else -> {
  ...
  private fun showSepaEuroMaximumDialog(sepaEuroMaximum: FiatMoney) {
    ...
    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToGenZapp__donation_amount_too_high)
      .setMessage(getString(R.string.DonateToGenZappFragment__you_can_send_up_to_s_via_bank_transfer, max))
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }
  ...
  override fun navigateToStripePaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToStripePaymentInProgressFragment(
  ...
  override fun navigateToPayPalPaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToPaypalPaymentInProgressFragment(
        InAppPaymentProcessorAction.PROCESS_NEW_IN_APP_PAYMENT,
        inAppPayment,
        inAppPayment.type
      )
  ...
  override fun navigateToCreditCardForm(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToCreditCardFragment(inAppPayment))
  }

  override fun navigateToIdealDetailsFragment(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToIdealTransferDetailsFragment(inAppPayment))
  }

  override fun navigateToBankTransferMandate(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToBankTransferMandateFragment(inAppPayment))
  }

  override fun onPaymentComplete(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToThanksForYourSupportBottomSheetDialog(Badges.fromDatabaseBadge(inAppPayment.data.badge!!)))
  }
  ...
  override fun navigateToDonationPending(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToGenZappFragmentDirections.actionDonateToGenZappFragmentToDonationPendingBottomSheet(inAppPayment))
  }
}
