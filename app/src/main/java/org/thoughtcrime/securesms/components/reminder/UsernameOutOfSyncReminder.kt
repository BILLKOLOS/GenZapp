...
class UsernameOutOfSyncReminder : Reminder(NO_RESOURCE) {
  ...
  init {
    val action = if (GenZappStore.account.usernameSyncState == UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
  ...
  override fun getText(context: Context): CharSequence {
    return if (GenZappStore.account.usernameSyncState == UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
  ...
  companion object {
    ...
    fun isEligible(): Boolean {
      return when (GenZappStore.account.usernameSyncState) {
