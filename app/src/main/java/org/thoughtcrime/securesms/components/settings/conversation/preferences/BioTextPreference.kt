  ...
  class RecipientModel(
    ...
    override fun getHeadlineText(context: Context): CharSequence {
      ...
      return SpannableStringBuilder(name).apply {
        if (recipient.showVerified) {
          ...
          val systemContactGlyph = GenZappSymbols.getSpannedString(
            context,
        ...
        if (recipient.isIndividual && !recipient.isSelf) {
          val chevronGlyph = GenZappSymbols.getSpannedString(
            context,
            GenZappSymbols.Weight.BOLD,
            ...
            SpanUtil.color(ContextCompat.getColor(context, R.color.GenZapp_colorOutline), it)
          }
    ...
    override fun getSubhead1Text(context: Context): String? {
      return if (recipient.isReleaseNotes) {
        context.getString(R.string.ReleaseNotes__GenZapp_release_notes_and_news)
      } else {
