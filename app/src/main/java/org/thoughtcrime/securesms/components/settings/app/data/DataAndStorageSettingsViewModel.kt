...
class DataAndStorageSettingsViewModel(
  ...
  fun setCallDataMode(callDataMode: CallDataMode) {
    GenZappStore.settings.callDataMode = callDataMode
    AppDependencies.GenZappCallManager.dataModeUpdate()
    getStateAndCopyStorageUsage()
  }

  fun setSentMediaQuality(sentMediaQuality: SentMediaQuality) {
    GenZappStore.settings.sentMediaQuality = sentMediaQuality
    getStateAndCopyStorageUsage()
  }
  ...
  private fun getState() = DataAndStorageSettingsState(
    ...
    callDataMode = GenZappStore.settings.callDataMode,
    isProxyEnabled = GenZappStore.proxy.isProxyEnabled,
    sentMediaQuality = GenZappStore.settings.sentMediaQuality
  )
