...
class AppUpdatesSettingsFragment : DSLSettingsFragment(R.string.preferences_app_updates__title) {
    ...
    return configure {
      if (Build.VERSION.SDK_INT >= 31) {
        switchPref(
          ...
          isChecked = GenZappStore.apkUpdate.autoUpdate,
          onClick = {
            GenZappStore.apkUpdate.autoUpdate = !GenZappStore.apkUpdate.autoUpdate
          }
        )
      }
  ...
  private val lastSuccessfulUpdateString: String
    get() {
      val lastUpdateTime = GenZappStore.apkUpdate.lastSuccessfulCheck
