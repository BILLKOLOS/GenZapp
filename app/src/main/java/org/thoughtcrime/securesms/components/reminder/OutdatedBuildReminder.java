...
public class OutdatedBuildReminder extends Reminder {
  ...
  public @NonNull CharSequence getText(@NonNull Context context) {
    ...
    if (days == 0) {
      return context.getString(R.string.OutdatedBuildReminder_your_version_of_GenZapp_will_expire_today);
    } else {
      return context.getResources().getQuantityString(R.plurals.OutdatedBuildReminder_your_version_of_GenZapp_will_expire_in_n_days, days, days);
    }
  }
  ...
  private static int getDaysUntilExpiry() {
    return (int) TimeUnit.MILLISECONDS.toDays(Util.getTimeUntilBuildExpiry(GenZappStore.misc().getEstimatedServerTime()));
  }
}
