 ...
 */
class GenZappContextMenu private constructor(
  val anchor: View,
  ...
  LayoutInflater.from(anchor.context).inflate(R.layout.GenZapp_context_menu, null),
  ...
  private val contextMenuList = ContextMenuList(
    recyclerView = contentView.findViewById(R.id.GenZapp_context_menu_list),
    onItemClick = { dismiss() }
  )

  init {
    setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.GenZapp_context_menu_background))
  ...
  private fun show(): GenZappContextMenu {
  ...
  class Builder(
    ...
    fun show(items: List<ActionItem>): GenZappContextMenu {
      return GenZappContextMenu(
        anchor = anchor,
        container = container,
        items = items,
        baseOffsetX = offsetX,
