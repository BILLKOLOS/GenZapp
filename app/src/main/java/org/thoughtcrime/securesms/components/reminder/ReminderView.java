...
public final class ReminderView extends FrameLayout {
  ...
  public void showReminder(final Reminder reminder) {
    if (!TextUtils.isEmpty(reminder.getTitle(getContext()))) {
      ...
      text.setTextColor(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurface));
    }
    ...
    switch (reminder.getImportance()) {
      case NORMAL:
        title.setTextColor(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurface));
        text.setTextColor(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurfaceVariant));
        break;
      case ERROR:
      case TERMINAL:
        container.setStrokeWidth(0);
        container.setCardBackgroundColor(ContextCompat.getColor(getContext(), R.color.reminder_background));
        title.setTextColor(ContextCompat.getColor(getContext(), R.color.GenZapp_light_colorOnSurface));
        text.setTextColor(ContextCompat.getColor(getContext(), R.color.GenZapp_light_colorOnSurface));
        break;
      default:
    ...
    if (reminder.getImportance() == Reminder.Importance.NORMAL) {
      closeButton.setColorFilter(ContextCompat.getColor(getContext(), R.color.GenZapp_colorOnSurfaceVariant));
    }
