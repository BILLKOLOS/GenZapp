...
class DebugLogsPromptDialogFragment : FixedRoundedCornerBottomSheetDialogFragment() {
  ...
  companion object {
    ...
    fun show(activity: AppCompatActivity, purpose: Purpose) {
      ...
      if (NetworkUtil.isConnected(activity) && activity.supportFragmentManager.findFragmentByTag(BottomSheetUtil.STANDARD_BOTTOM_SHEET_FRAGMENT_TAG) == null) {
        ...
        when (purpose) {
          Purpose.NOTIFICATIONS -> GenZappStore.uiHints.lastNotificationLogsPrompt = System.currentTimeMillis()
          Purpose.CRASH -> GenZappStore.uiHints.lastCrashPrompt = System.currentTimeMillis()
          Purpose.CONNECTIVITY_WARNING -> GenZappStore.misc.lastConnectivityWarningTime = System.currentTimeMillis()
        }
      }
    }
  }
  ...
  override val themeResId: Int = R.style.Widget_GenZapp_FixedRoundedCorners_Messages
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.submit.setOnClickListener {
      val progressDialog = GenZappProgressDialog.show(requireContext())
    ...
    binding.decline.setOnClickListener {
      if (purpose == Purpose.NOTIFICATIONS) {
        GenZappStore.uiHints.markDeclinedShareNotificationLogs()
      }
  ...
  private fun submitLogs(debugLog: String, purpose: Purpose) {
    CommunicationActions.openEmail(
      ...
      getString(R.string.DebugLogsPromptDialogFragment__GenZapp_android_support_request),
  ...
  private fun getEmailBody(debugLog: String?, purpose: Purpose): String {
    ...
    return SupportEmailUtil.generateSupportEmailBody(
      requireContext(),
      R.string.DebugLogsPromptDialogFragment__GenZapp_android_support_request,
      " - $category",
      "\n\n",
      suffix.toString()
    )
