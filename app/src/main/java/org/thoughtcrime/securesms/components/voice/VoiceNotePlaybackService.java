...
public class VoiceNotePlaybackService extends MediaSessionService {
    ...
    ContextCompat.getMainExecutor(getApplicationContext()).execute(() -> {
      if (player != null) {
        ...
        try {
          ...
          final DatabaseAttachment attachment = GenZappDatabase.attachments().getAttachment(partId);
          if (attachment == null) {
            player.stop();
   ...
   * If that doesn't work, it returns null, GenZapping the {@link MediaSession} cannot be built on this device.
   *
  ...
  private void sendViewedReceiptForCurrentWindowIndex() {
        ...
        player.getCurrentWindowIndex() != C.INDEX_UNSET)
    {
      ...
      GenZappExecutors.BOUNDED.execute(() -> {
        ...
        MessageTable messageDatabase = GenZappDatabase.messages();
  ...
  /**
   * Receiver to stop playback and kill the notification if user locks GenZapp via screen lock.
