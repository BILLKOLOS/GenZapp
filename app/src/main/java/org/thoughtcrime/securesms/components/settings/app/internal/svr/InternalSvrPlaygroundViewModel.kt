...
class InternalSvrPlaygroundViewModel : ViewModel() {
    ...
    disposables += Single
      .fromCallable {
        _state.value.selected.toImplementation()
          .setPin(_state.value.userPin, GenZappStore.svr.getOrCreateMasterKey())
          .execute()
      }
  ...
  private fun SvrImplementation.toImplementation(): SecureValueRecovery {
    return when (this) {
      SvrImplementation.SVR2 -> AppDependencies.GenZappServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE)
      SvrImplementation.SVR3 -> AppDependencies.GenZappServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libGenZappNetwork)
    }
  }
}
