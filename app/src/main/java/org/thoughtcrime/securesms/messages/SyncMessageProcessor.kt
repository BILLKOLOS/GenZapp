...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.expireTimerDuration
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.groupId
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.groupMasterKey
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasGroupContext
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasRemoteDelete
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isEmptyGroupV2Message
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isEndSession
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isExpirationUpdate
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isGroupV2Update
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isMediaMessage
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isUnidentified
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.serviceIdsToUnidentifiedStatus
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.toMobileCoinMoney
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.toPointer
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.toPointersWithinLimit
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.toGenZappServiceAttachmentPointer
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.type
...
import org.whispersystems.GenZappservice.internal.push.StoryMessage
import org.whispersystems.GenZappservice.internal.push.SyncMessage
import org.whispersystems.GenZappservice.internal.push.SyncMessage.Blocked
import org.whispersystems.GenZappservice.internal.push.SyncMessage.CallLinkUpdate
import org.whispersystems.GenZappservice.internal.push.SyncMessage.CallLogEvent
import org.whispersystems.GenZappservice.internal.push.SyncMessage.Configuration
import org.whispersystems.GenZappservice.internal.push.SyncMessage.FetchLatest
import org.whispersystems.GenZappservice.internal.push.SyncMessage.MessageRequestResponse
import org.whispersystems.GenZappservice.internal.push.SyncMessage.Read
import org.whispersystems.GenZappservice.internal.push.SyncMessage.Request
import org.whispersystems.GenZappservice.internal.push.SyncMessage.Sent
import org.whispersystems.GenZappservice.internal.push.SyncMessage.StickerPackOperation
import org.whispersystems.GenZappservice.internal.push.SyncMessage.ViewOnceOpen
import org.whispersystems.GenZappservice.internal.push.Verified
...
object SyncMessageProcessor {
    ...
    try {
      ...
      when {
        ...
        dataMessage.isGroupV2Update -> {
          ...
          threadId = GenZappDatabase.threads.getOrCreateThreadIdFor(getSyncMessageDestination(sent))
        }
      ...
      if (groupId != null && GenZappDatabase.groups.isUnknownGroup(groupId)) {
      ...
      if (dataMessage.profileKey.isNotEmpty()) {
        ...
        if (!recipient.isSystemContact && !recipient.isProfileSharing) {
          GenZappDatabase.recipients.setProfileSharing(recipient.id, true)
        }
      }

      if (threadId != -1L) {
        GenZappDatabase.threads.setRead(threadId, true)
  ...
  private fun handlePniIdentityKeys(envelope: Envelope, sent: Sent) {
    for (status in sent.unidentifiedStatus) {
      ...
      val address = GenZappProtocolAddress(pni.toString(), GenZappServiceAddress.DEFAULT_DEVICE_ID)
  ...
  private fun getSyncMessageDestination(message: Sent): Recipient {
    return if (message.message.hasGroupContext) {
      ...
      Recipient.externalPush(GenZappServiceAddress(ServiceId.parseOrThrow(message.destinationServiceId!!), message.destinationE164))
    }
  }
  ...
  private fun handleSynchronizeSentEditMessage(
    ...
    val targetMessage: MessageRecord? = GenZappDatabase.messages.getMessageFor(targetSentTimestamp, senderRecipient.id)
  ...
  private fun handleSynchronizeSentEditTextMessage(
    ...
    if (isGroup) {
      ...
      messageId = GenZappDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, GroupReceiptTable.STATUS_UNKNOWN, null)
      ...
      messageId = GenZappDatabase.messages.insertMessageOutbox(outgoingTextMessage, threadId, false, null)
      GenZappDatabase.messages.markUnidentified(messageId, sent.isUnidentified(toRecipient.serviceId.orNull()))
    }

    GenZappDatabase.messages.markAsSent(messageId, true)
    if (targetMessage.expireStarted > 0) {
      GenZappDatabase.messages.markExpireStarted(messageId, targetMessage.expireStarted)
    ...
    if (toRecipient.isSelf) {
      GenZappDatabase.messages.incrementDeliveryReceiptCount(sent.timestamp!!, toRecipient.id, System.currentTimeMillis())
      GenZappDatabase.messages.incrementReadReceiptCount(sent.timestamp!!, toRecipient.id, System.currentTimeMillis())
    }
  }
  ...
  private fun handleSynchronizeSentEditMediaMessage(
    ...
    val messageId: Long = GenZappDatabase.messages.insertMessageOutbox(mediaMessage, threadId, false, GroupReceiptTable.STATUS_UNKNOWN, null)
    ...
    val attachments: List<DatabaseAttachment> = GenZappDatabase.attachments.getAttachmentsForMessage(messageId)
    ...
    if (syncAttachments.isNotEmpty()) {
      GenZappDatabase.runPostSuccessfulTransaction {
  ...
  private fun handleSynchronizeSentStoryMessage(envelope: Envelope, sent: Sent) {
    ...
    if (sent.isRecipientUpdate == true) {
      ...
      GenZappDatabase.storySends.applySentStoryManifest(manifest, sent.timestamp!!)
      return
    }
    ...
    for (distributionId in distributionIds) {
      val distributionRecipientId = GenZappDatabase.distributionLists.getOrCreateByDistributionId(distributionId, manifest)
    ...
    if (groupId != null) {
      val groupRecipient: Optional<RecipientId> = GenZappDatabase.recipients.getByGroupId(groupId)
  ...
  private fun insertSentStoryMessage(
    ...
    if (GenZappDatabase.messages.isOutgoingStoryAlreadyInDatabase(recipient.id, sentAtTimestamp)) {
    ...
    val messageId: Long = GenZappDatabase.messages.insertMessageOutbox(mediaMessage, threadId, false, GroupReceiptTable.STATUS_UNDELIVERED, null)
    ...
    if (groupId != null) {
      ...
      GenZappDatabase.messages.markUnidentified(messageId, sent.isUnidentified(recipient.serviceId.orNull()))
    }

    GenZappDatabase.messages.markAsSent(messageId, true)

    val allAttachments = GenZappDatabase.attachments.getAttachmentsForMessage(messageId)
    val attachments: List<DatabaseAttachment> = allAttachments.filterNot { it.isSticker }

    if (recipient.isSelf) {
      GenZappDatabase.messages.incrementDeliveryReceiptCount(sent.timestamp!!, recipient.id, System.currentTimeMillis())
      GenZappDatabase.messages.incrementReadReceiptCount(sent.timestamp!!, recipient.id, System.currentTimeMillis())
    }
  ...
  private fun handleGroupRecipientUpdate(sent: Sent, envelopeTimestamp: Long) {
    ...
    val record = GenZappDatabase.messages.getMessageFor(sent.timestamp!!, Recipient.self().id)
    if (record == null) {
  ...
  private fun updateGroupReceiptStatus(sent: Sent, messageId: Long, groupString: GroupId) {
    ...
    val members: List<RecipientId> = GenZappDatabase.groups.getGroupMembers(groupString, GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF).map { it.id }
    val localReceipts: Map<RecipientId, Int> = GenZappDatabase.groupReceipts.getGroupReceiptInfo(messageId).associate { it.recipientId to it.status }
    ...
    for (messageRecipientId in messageRecipientIds.keys) {
      if ((localReceipts[messageRecipientId] ?: GroupReceiptTable.STATUS_UNKNOWN) < GroupReceiptTable.STATUS_UNDELIVERED) {
        GenZappDatabase.groupReceipts.update(messageRecipientId, messageId, GroupReceiptTable.STATUS_UNDELIVERED, sent.timestamp!!)
      } else if (!localReceipts.containsKey(messageRecipientId)) {
        GenZappDatabase.groupReceipts.insert(listOf(messageRecipientId), messageId, GroupReceiptTable.STATUS_UNDELIVERED, sent.timestamp!!)
      }
    }

    val unidentifiedStatus = members.map { Pair(it, messageRecipientIds[it] ?: false) }

    GenZappDatabase.groupReceipts.setUnidentified(unidentifiedStatus, messageId)
  }

  private fun updateGroupReceiptStatusForDistributionList(sent: Sent, messageId: Long, distributionListId: DistributionListId) {
    val messageRecipientIds: Map<RecipientId, Boolean> = sent.serviceIdsToUnidentifiedStatus.mapKeys { RecipientId.from(it.key) }
    val members: List<RecipientId> = GenZappDatabase.distributionLists.getMembers(distributionListId)
    val localReceipts: Map<RecipientId, Int> = GenZappDatabase.groupReceipts.getGroupReceiptInfo(messageId).associate { it.recipientId to it.status }
  ...
  private fun handleSynchronizeSentEndSessionMessage(context: Context, sent: Sent, envelopeTimestamp: Long): Long {
    ...
    val threadId: Long = GenZappDatabase.threads.getOrCreateThreadIdFor(recipient)
  ...
  private fun handleSynchronizeSentGv2Update(context: Context, envelope: Envelope, sent: Sent) {
    ...
    if (MessageContentProcessor.updateGv2GroupFromServerOrP2PChange(context, envelope.timestamp!!, dataMessage.groupV2!!, GenZappDatabase.groups.getGroup(groupId)) == null) {
  ...
  private fun handleSynchronizeSentExpirationUpdate(sent: Sent, sideEffect: Boolean = false): Long {
    ...
    val threadId: Long = GenZappDatabase.threads.getOrCreateThreadIdFor(recipient)
    val messageId: Long = GenZappDatabase.messages.insertMessageOutbox(expirationUpdateMessage, threadId, false, null)

    GenZappDatabase.messages.markAsSent(messageId, true)

    GenZappDatabase.recipients.setExpireMessages(recipient.id, sent.message!!.expireTimerDuration.inWholeSeconds.toInt())
  ...
  private fun handleSynchronizeSentStoryReply(sent: Sent, envelopeTimestamp: Long): Long {
    ...
    try {
      ...
      val storyMessageId: Long = GenZappDatabase.messages.getStoryId(storyAuthorRecipient, sentTimestamp).id
      val story: MmsMessageRecord = GenZappDatabase.messages.getMessageRecord(storyMessageId) as MmsMessageRecord
      val threadRecipientId: RecipientId? = GenZappDatabase.threads.getRecipientForThreadId(story.threadId)?.id
      val groupStory: Boolean = threadRecipientId != null && (GenZappDatabase.groups.getGroup(threadRecipientId).orNull()?.isActive ?: false)
      ...
      if (dataMessage.expireTimerDuration > Duration.ZERO) {
        GenZappDatabase.messages.markExpireStarted(messageId, sent.expirationStartTimestamp ?: 0)
  ...
  private fun handleSynchronizeReadMessage(
    ...
    val unhandled: Collection<MessageTable.SyncMessageId> = GenZappDatabase.messages.setTimestampReadFromSyncMessage(readMessages, envelopeTimestamp, threadToLatestRead)
    val markedMessages: List<MarkedMessageInfo> = GenZappDatabase.threads.setReadSince(threadToLatestRead, false)

    if (Util.hasItems(markedMessages)) {
      log("Updating past GenZappDatabase.messages: " + markedMessages.size)
  ...
  private fun handleSynchronizeViewedMessage(context: Context, viewedMessages: List<SyncMessage.Viewed>, envelopeTimestamp: Long) {
    ...
    val records = viewedMessages
      .mapNotNull { message ->
        ...
        if (message.timestamp != null) {
          GenZappDatabase.messages.getMessageFor(message.timestamp!!, author)
        } else {
    ...
    GenZappDatabase.messages.setIncomingMessagesViewed(toMarkViewed)
    GenZappDatabase.messages.setOutgoingGiftsRevealed(toMarkViewed)
  ...
  private fun handleSynchronizeViewOnceOpenMessage(context: Context, openMessage: ViewOnceOpen, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val record: MessageRecord? = GenZappDatabase.messages.getMessageFor(timestamp, author)

    if (record != null) {
      GenZappDatabase.attachments.deleteAttachmentFilesForViewOnceMessage(record.id)
    } else {
    ...
    for (operation in stickerPackOperations) {
      if (operation.packId != null && operation.packKey != null && operation.type != null) {
        ...
        when (operation.type!!) {
          ...
          StickerPackOperation.Type.REMOVE -> GenZappDatabase.stickers.uninstallPack(packId)
        }
      } else {
  ...
  private fun handleSynchronizeConfigurationMessage(context: Context, configurationMessage: Configuration, envelopeTimestamp: Long) {
    ...
    if (configurationMessage.linkPreviews != null) {
      GenZappStore.settings.isLinkPreviewsEnabled = configurationMessage.linkPreviews!!
    }
  }

  private fun handleSynchronizeBlockedListMessage(blockMessage: Blocked) {
    val addresses: List<GenZappServiceAddress> = blockMessage.acis.mapNotNull { GenZappServiceAddress.fromRaw(it, null).orNull() }
    val groupIds: List<ByteArray> = blockMessage.groupIds.map { it.toByteArray() }

    GenZappDatabase.recipients.applyBlockedUpdate(addresses, groupIds)
  }
  ...
  private fun handleSynchronizeMessageRequestResponse(response: MessageRequestResponse, envelopeTimestamp: Long) {
    ...
    when (response.type) {
      MessageRequestResponse.Type.ACCEPT -> {
        GenZappDatabase.recipients.setProfileSharing(recipient.id, true)
        GenZappDatabase.recipients.setBlocked(recipient.id, false)
        GenZappDatabase.messages.insertMessageOutbox(
      ...
      MessageRequestResponse.Type.DELETE -> {
        GenZappDatabase.recipients.setProfileSharing(recipient.id, false)
        if (threadId > 0) {
          GenZappDatabase.threads.deleteConversation(threadId, syncThreadDelete = false)
        }
      }
  ...
  private fun handleSynchronizeKeys(storageKey: ByteString, envelopeTimestamp: Long) {
    ...
    GenZappStore.storageService.setStorageKeyFromPrimary(StorageKey(storageKey.toByteArray()))
  }
  ...
  private fun handleSynchronizeContacts(contactsMessage: SyncMessage.Contacts, envelopeTimestamp: Long) {
    ...
    val attachment: GenZappServiceAttachmentPointer = contactsMessage.blob!!.toGenZappServiceAttachmentPointer()
  ...
  private fun handleSynchronizeCallLogEvent(callLogEvent: CallLogEvent, envelopeTimestamp: Long) {
    ...
    val peer: RecipientId? = callLogEvent.conversationId?.let { byteString ->
      ACI.parseOrNull(byteString)?.let { RecipientId.from(it) }
        ?: GroupId.pushOrNull(byteString.toByteArray())?.let { GenZappDatabase.recipients.getByGroupId(it).orNull() }
        ?: CallLinkRoomId.fromBytes(byteString.toByteArray()).let { GenZappDatabase.recipients.getByCallLinkRoomId(it).orNull() }
    }

    if (callId != null && peer != null) {
      val call = GenZappDatabase.calls.getCallById(callId.longValue(), peer)
  ...
  private fun synchronizeCallLogEventViaTimestamp(envelopeTimestamp: Long, eventType: CallLogEvent.Type?, timestamp: Long, peer: RecipientId?) {
    when (eventType) {
      CallLogEvent.Type.CLEAR -> {
        GenZappDatabase.calls.deleteNonAdHocCallEventsOnOrBefore(timestamp)
        GenZappDatabase.callLinks.deleteNonAdminCallLinksOnOrBefore(timestamp)
      }

      CallLogEvent.Type.MARKED_AS_READ -> {
        GenZappDatabase.calls.markAllCallEventsRead(timestamp)
      }
      ...
      CallLogEvent.Type.MARKED_AS_READ_IN_CONVERSATION -> {
        ...
        GenZappDatabase.calls.markAllCallEventsWithPeerBeforeTimestampRead(peer, timestamp)
      }
  ...
  private fun handleSynchronizeCallLink(callLinkUpdate: CallLinkUpdate, envelopeTimestamp: Long) {
    ...
    if (callLinkUpdate.type == CallLinkUpdate.Type.DELETE) {
      ...
      GenZappDatabase.callLinks.deleteCallLink(roomId)

      return
    }

    if (GenZappDatabase.callLinks.callLinkExists(roomId)) {
      log(envelopeTimestamp, "Synchronize call link for a link we already know about. Updating credentials.")
      GenZappDatabase.callLinks.updateCallLinkCredentials(
        roomId,
        CallLinkCredentials(
  ...
  private fun handleSynchronizeOneToOneCallEvent(callEvent: SyncMessage.CallEvent, envelopeTimestamp: Long) {
    ...
    val call = GenZappDatabase.calls.getCallById(callId, recipientId)
    if (call != null) {
      ...
      if (typeMismatch || directionMismatch || peerMismatch || eventDowngrade) {
        ...
        GenZappDatabase.calls.updateOneToOneCall(callId, event)
      }
    } else if (event == CallTable.Event.DELETE) {
      GenZappDatabase.calls.insertDeletedCallFromSyncEvent(callId, recipientId, type, direction, timestamp)
    } else {
      GenZappDatabase.calls.insertOneToOneCall(callId, timestamp, recipientId, type, direction, event)
    }
  }
  ...
  private fun handleSynchronizeGroupOrAdHocCallEvent(callEvent: SyncMessage.CallEvent, envelopeTimestamp: Long) {
    ...
    if (hasConversationId && type == CallTable.Type.AD_HOC_CALL && callEvent.event == SyncMessage.CallEvent.Event.OBSERVED && direction != null) {
      ...
      GenZappDatabase.calls.insertOrUpdateAdHocCallFromObserveEvent(
    ...
    val call = GenZappDatabase.calls.getCallById(callId, recipient.id)
    ...
    if (call != null) {
      ...
      when (event) {
        CallTable.Event.DELETE -> GenZappDatabase.calls.markCallDeletedFromSyncEvent(call)
        CallTable.Event.ACCEPTED -> {
          if (call.timestamp > timestamp) {
            GenZappDatabase.calls.setTimestamp(call.callId, recipient.id, timestamp)
          }
          if (direction == CallTable.Direction.INCOMING) {
            GenZappDatabase.calls.acceptIncomingGroupCall(call)
          } else {
            GenZappDatabase.calls.acceptOutgoingGroupCall(call)
          }
        }
        CallTable.Event.NOT_ACCEPTED -> {
          ...
          if (callEvent.direction == SyncMessage.CallEvent.Direction.INCOMING) {
            GenZappDatabase.calls.declineIncomingGroupCall(call)
          } else {
      ...
      when (event) {
        CallTable.Event.DELETE -> GenZappDatabase.calls.insertDeletedCallFromSyncEvent(callEvent.id!!, recipient.id, type, direction, timestamp)
        CallTable.Event.ACCEPTED -> GenZappDatabase.calls.insertAcceptedGroupCall(callEvent.id!!, recipient.id, direction, timestamp)
        CallTable.Event.NOT_ACCEPTED -> {
          if (callEvent.direction == SyncMessage.CallEvent.Direction.INCOMING) {
            GenZappDatabase.calls.insertDeclinedGroupCall(callEvent.id!!, recipient.id, timestamp)
          } else {
  ...
  private fun resolveCallLinkRecipient(callEvent: SyncMessage.CallEvent): Recipient {
    ...
    val callLink = GenZappDatabase.callLinks.getOrCreateCallLinkByRoomId(callLinkRoomId)
  ...
  private fun handleSynchronizeMessageDeletes(messageDeletes: List<SyncMessage.DeleteForMe.MessageDeletes>, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val unhandled: List<MessageTable.SyncMessageId> = GenZappDatabase.messages.deleteMessages(messagesToDelete)
  ...
  private fun handleSynchronizeConversationDeletes(conversationDeletes: List<SyncMessage.DeleteForMe.ConversationDelete>, envelopeTimestamp: Long) {
    for (delete in conversationDeletes) {
      ...
      val threadId = GenZappDatabase.threads.getThreadIdFor(threadRecipientId)
      if (threadId == null) {
        log(envelopeTimestamp, "[handleSynchronizeDeleteForMe] No thread for matching conversation for recipient: $threadRecipientId")
        continue
      }

      var latestReceivedAt = GenZappDatabase.messages.getLatestReceivedAt(threadId, delete.mostRecentMessages.mapNotNull { it.toSyncMessageId(envelopeTimestamp) })
      ...
      if (latestReceivedAt == null && delete.mostRecentNonExpiringMessages.isNotEmpty()) {
        ...
        latestReceivedAt = GenZappDatabase.messages.getLatestReceivedAt(threadId, delete.mostRecentNonExpiringMessages.mapNotNull { it.toSyncMessageId(envelopeTimestamp) })
      }

      if (latestReceivedAt != null) {
        GenZappDatabase.threads.trimThread(threadId = threadId, syncThreadTrimDeletes = false, trimBeforeDate = latestReceivedAt, inclusive = true)

        if (delete.isFullDelete == true) {
          val deleted = GenZappDatabase.threads.deleteConversationIfContainsOnlyLocal(threadId)
  ...
  private fun handleSynchronizeAttachmentDeletes(attachmentDeletes: List<SyncMessage.DeleteForMe.AttachmentDelete>, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val unhandled: List<MessageTable.SyncMessageId> = GenZappDatabase.attachments.deleteAttachments(toDelete)
    ...
    return when {
      ...
      threadServiceId != null -> {
        ServiceId.parseOrNull(threadServiceId)?.let {
          GenZappDatabase.recipients.getOrInsertFromServiceId(it)
        }
      }

      threadE164 != null -> {
        GenZappDatabase.recipients.getOrInsertFromE164(threadE164!!)
      }
  ...
  private fun SyncMessage.DeleteForMe.AddressableMessage.toSyncMessageId(envelopeTimestamp: Long): MessageTable.SyncMessageId? {
    return if (this.sentTimestamp != null && (this.authorServiceId != null || this.authorE164 != null)) {
      ...
      val id = if (serviceId != null) {
        GenZappDatabase.recipients.getOrInsertFromServiceId(serviceId)
      } else {
        GenZappDatabase.recipients.getOrInsertFromE164(this.authorE164!!)
      }
