...
import org.GenZapp.libGenZapp.metadata.InvalidMetadataMessageException
import org.GenZapp.libGenZapp.metadata.InvalidMetadataVersionException
import org.GenZapp.libGenZapp.metadata.ProtocolDuplicateMessageException
import org.GenZapp.libGenZapp.metadata.ProtocolException
import org.GenZapp.libGenZapp.metadata.ProtocolInvalidKeyException
import org.GenZapp.libGenZapp.metadata.ProtocolInvalidKeyIdException
import org.GenZapp.libGenZapp.metadata.ProtocolInvalidMessageException
import org.GenZapp.libGenZapp.metadata.ProtocolInvalidVersionException
import org.GenZapp.libGenZapp.metadata.ProtocolLegacyMessageException
import org.GenZapp.libGenZapp.metadata.ProtocolNoSessionException
import org.GenZapp.libGenZapp.metadata.ProtocolUntrustedIdentityException
import org.GenZapp.libGenZapp.metadata.SelfSendException
import org.GenZapp.libGenZapp.protocol.GenZappProtocolAddress
import org.GenZapp.libGenZapp.protocol.groups.GroupSessionBuilder
import org.GenZapp.libGenZapp.protocol.groups.state.SenderKeyStore
import org.GenZapp.libGenZapp.protocol.message.CiphertextMessage
import org.GenZapp.libGenZapp.protocol.message.DecryptionErrorMessage
...
import org.whispersystems.GenZappservice.api.InvalidMessageStructureException
...
import org.whispersystems.GenZappservice.api.crypto.GenZappGroupSessionBuilder
import org.whispersystems.GenZappservice.api.crypto.GenZappServiceCipher
import org.whispersystems.GenZappservice.api.crypto.GenZappServiceCipherResult
import org.whispersystems.GenZappservice.api.messages.EnvelopeContentValidator
...
import org.whispersystems.GenZappservice.internal.push.PniSignatureMessage
...
object MessageDecryptor {
  ...
  /**
   * Decrypts an envelope and provides a [Result]. This method has side effects, but all of them are limited to [GenZappDatabase].
  ...
  fun decrypt(
    ...
    val selfAci: ACI = GenZappStore.account.requireAci()
    val selfPni: PNI = GenZappStore.account.requirePni()
    ...
    if (destination == selfPni && envelope.sourceServiceId != null) {
      ...
      if (sourceServiceId != null) {
        ...
        GenZappDatabase.recipients.markNeedsPniSignature(sender)
      } else {
    ...
    val localAddress = GenZappServiceAddress(selfAci, GenZappStore.account.e164)
    val cipher = GenZappServiceCipher(localAddress, GenZappStore.account.deviceId, bufferedStore, ReentrantSessionLock.INSTANCE, SealedSenderAccessUtil.getCertificateValidator())

    return try {
      val startTimeNanos = System.nanoTime()
      val cipherResult: GenZappServiceCipherResult? = cipher.decrypt(envelope, serverDeliveredTimestamp)
      ...
      if (envelope.reportingToken != null && envelope.reportingToken!!.size > 0) {
        ...
        GenZappDatabase.recipients.setReportingToken(sender, envelope.reportingToken!!.toByteArray())
      }
  ...
  private fun buildResultForDecryptionError(
    ...
    if (ServiceId.parseOrNull(envelope.destinationServiceId) == GenZappStore.account.pni) {
      ...
      ContentHint.RESENDABLE -> {
        ...
        followUpOperations += FollowUpOperation {
          ...
          val threadId: Long? = if (groupId != null) {
            if (GenZappDatabase.groups.getGroup(groupId).isAbsent()) {
            ...
            GenZappDatabase.threads.getThreadIdFor(groupRecipient.id)
          } else {
            GenZappDatabase.threads.getThreadIdFor(sender.id)
          }
  ...
  private fun handleSenderKeyDistributionMessage(envelope: Envelope, serviceId: ServiceId, deviceId: Int, message: SenderKeyDistributionMessage, senderKeyStore: SenderKeyStore) {
    ...
    val sender = GenZappProtocolAddress(serviceId.toString(), deviceId)
    GenZappGroupSessionBuilder(ReentrantSessionLock.INSTANCE, GroupSessionBuilder(senderKeyStore)).process(sender, message)
  }
  ...
  private fun handlePniSignatureMessage(envelope: Envelope, protocolStore: BufferedProtocolStore, aci: ACI, e164: String?, deviceId: Int, pniSignatureMessage: PniSignatureMessage) {
    ...
    if (GenZappDatabase.recipients.isAssociated(aci, pni)) {
      Log.i(TAG, "${logPrefix(envelope, aci)}[handlePniSignatureMessage] ACI ($aci) and PNI ($pni) are already associated.")
      return
    }

    val aciAddress = GenZappProtocolAddress(aci.toString(), deviceId)
    val pniAddress = GenZappProtocolAddress(pni.toString(), deviceId)
    ...
    if (pniIdentity == null) {
      ...
      if (deviceId != GenZappServiceAddress.DEFAULT_DEVICE_ID) {
        pniIdentity = protocolStore.getAciStore().getIdentity(GenZappProtocolAddress(pni.toString(), GenZappServiceAddress.DEFAULT_DEVICE_ID))
    ...
    if (pniIdentity.verifyAlternateIdentity(aciIdentity, pniSignatureMessage.signature!!.toByteArray())) {
      ...
      GenZappDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
    } else {
  ...
  private fun logPrefix(envelope: Envelope, cipherResult: GenZappServiceCipherResult): String {
  ...
  private fun GenZappServiceCipherResult.toErrorMetadata(): ErrorMetadata {
    return ErrorMetadata(
  ...
  sealed interface Result {
    ...
    /** The envelope used an invalid version of the GenZapp protocol. */
    ...
    /**
     * Indicates the that the [org.whispersystems.GenZappservice.internal.push.GenZappServiceProtos.DataMessage.getRequiredProtocolVersion]
