...
object EditMessageProcessor {
  fun process(
    ...
    var targetMessage: MmsMessageRecord? = GenZappDatabase.messages.getMessageFor(editMessage.targetSentTimestamp!!, senderRecipient.id) as? MmsMessageRecord
    val targetThreadRecipient: Recipient? = if (targetMessage != null) GenZappDatabase.threads.getRecipientForThreadId(targetMessage.threadId) else null
    ...
    val originalMessage = targetMessage.originalMessageId?.let { GenZappDatabase.messages.getMessageRecord(it.id) } ?: targetMessage
    ...
    targetMessage = targetMessage.withAttachments(GenZappDatabase.attachments.getAttachmentsForMessage(targetMessage.id))
  ...
  private fun handleEditMediaMessage(
    ...
    val insertResult = GenZappDatabase.messages.insertEditMessageInbox(mediaMessage, targetMessage).orNull()
  ...
  private fun handleEditTextMessage(
    ...
    return GenZappDatabase.messages.insertEditMessageInbox(textMessage, targetMessage).orNull()
  }
}
