...
import org.whispersystems.GenZappservice.api.websocket.WebSocketUnavailableException
...
class IncomingMessageObserver(private val context: Application) {
  ...
  companion object {
    ...
    private val censored: Boolean
      get() = AppDependencies.GenZappServiceNetworkAccess.isCensored()
  }
  ...
  init {
    ...
    if (!GenZappStore.account.fcmEnabled || GenZappStore.internal.isWebsocketModeForced) {
      try {
  ...
  private fun isConnectionNecessary(): Boolean {
    ...
    val registered = GenZappStore.account.isRegistered
    val fcmEnabled = GenZappStore.account.fcmEnabled
    val hasNetwork = NetworkConstraint.isMet(context)
    val hasProxy = GenZappStore.proxy.isProxyEnabled
    val forceWebsocket = GenZappStore.internal.isWebsocketModeForced
  ...
  private fun disconnect() {
    AppDependencies.GenZappWebSocket.disconnect()
  }
  ...
  private fun processMessage(bufferedProtocolStore: BufferedProtocolStore, envelope: Envelope, serverDeliveredTimestamp: Long): List<FollowUpOperation> {
    val localReceiveMetric = GenZappLocalMetrics.MessageReceive.start()
    ...
    GenZappLocalMetrics.MessageLatency.onMessageReceived(envelope.serverTimestamp!!, serverDeliveredTimestamp, envelope.urgent!!)
    when (result) {
  ...
  private fun processReceipt(envelope: Envelope) {
    ...
    GenZappDatabase.messages.incrementDeliveryReceiptCount(envelope.timestamp!!, senderId, System.currentTimeMillis())
    GenZappDatabase.messageLog.deleteEntryForRecipient(envelope.timestamp!!, senderId, envelope.sourceDevice!!)
  }
  ...
  private inner class MessageRetrievalThread : Thread("MessageRetrievalService"), Thread.UncaughtExceptionHandler {
    ...
    override fun run() {
      ...
      while (!terminated) {
        ...
        val GenZappWebSocket = AppDependencies.GenZappWebSocket
        val webSocketDisposable = AppDependencies.webSocketObserver.subscribe { state: WebSocketConnectionState ->
          ...
          if (state == WebSocketConnectionState.CONNECTED) {
            GenZappStore.misc.lastWebSocketConnectTime = System.currentTimeMillis()
          }
        }

        GenZappWebSocket.connect()
        try {
          while (isConnectionNecessary()) {
            try {
              Log.d(TAG, "Reading message...")

              val hasMore = GenZappWebSocket.readMessageBatch(websocketReadTimeout, 30) { batch ->
                ...
                GroupsV2ProcessingLock.acquireGroupProcessingLock().use {
                  ReentrantSessionLock.INSTANCE.acquire().use {
                    batch.forEach { response ->
                      ...
                      val followUpOperations = GenZappDatabase.runInTransaction { db ->
                      ...
                      GenZappWebSocket.sendAck(response)
                    }
                  }
                }
              ...
              attempts = 0
              GenZappLocalMetrics.PushWebsocketFetch.onProcessedBatch()
  ...
  class ForegroundService : Service() {
    ...
    private fun postForegroundNotification() {
      val notification = NotificationCompat.Builder(applicationContext, NotificationChannels.getInstance().BACKGROUND)
        .setContentTitle(applicationContext.getString(R.string.MessageRetrievalService_GenZapp))
        .setContentText(applicationContext.getString(R.string.MessageRetrievalService_background_connection_enabled))
        .setPriority(NotificationCompat.PRIORITY_MIN)
        .setWhen(0)
        .setSmallIcon(R.drawable.ic_GenZapp_background_connection)
        .build()
