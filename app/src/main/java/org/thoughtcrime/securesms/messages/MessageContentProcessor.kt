...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasDisallowedAnnouncementOnlyContent
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasGroupContext
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasSignedGroupChange
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.hasStarted
...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isValid
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.signedGroupChange
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.toDecryptionErrorMessage
...
import org.whispersystems.GenZappservice.internal.push.CallMessage
...
import org.whispersystems.GenZappservice.internal.push.TypingMessage
...
open class MessageContentProcessor(private val context: Context) {
  ...
  companion object {
      ...
      if (content.dataMessage != null) {
        ...
        if (content.typingMessage!!.groupId != null) {
          ...
          return if (groupRecipient.isBlocked || !groupRecipient.isActiveGroup) {
            ...
            val groupRecord = GenZappDatabase.groups.getGroup(groupId)
    ...
    private fun handlePendingRetry(pending: PendingRetryReceiptModel?, timestamp: Long, destination: Recipient): Long {
      ...
      if (pending != null) {
        ...
        val threadId = GenZappDatabase.threads.getThreadIdFor(destination.id)
        if (threadId != null) {
          val lastSeen = GenZappDatabase.threads.getConversationMetadata(threadId).lastSeen
    ...
    fun handleGv2PreProcessing(
      ...
      val preUpdateGroupRecord = GenZappDatabase.groups.getGroup(groupId)
      ...
      val groupRecord = if (groupUpdateResult.updateStatus == UpdateStatus.GROUP_CONSISTENT_OR_AHEAD) {
        ...
        GenZappDatabase.groups.getGroup(groupId)
      }
   ...
   */
  @JvmOverloads
  open fun process(envelope: Envelope, content: Content, metadata: EnvelopeMetadata, serverDeliveredTimestamp: Long, processingEarlyContent: Boolean = false, localMetric: GenZappLocalMetrics.MessageReceive? = null) {
    val senderRecipient = Recipient.externalPush(GenZappServiceAddress(metadata.sourceServiceId, metadata.sourceE164))
  ...
  fun processException(messageState: MessageState, exceptionMetadata: ExceptionMetadata, timestamp: Long) {
    ...
    when (messageState) {
      MessageState.DECRYPTION_ERROR -> {
        ...
        val threadId: Long? = GenZappDatabase.threads.getThreadIdFor(threadRecipient.id)
      ...
      MessageState.INVALID_VERSION -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          GenZappDatabase.messages.markAsInvalidVersionKeyExchange(messageId)
        }
      }
      ...
      MessageState.LEGACY_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          GenZappDatabase.messages.markAsLegacyVersion(messageId)
        }
      }
      ...
      MessageState.UNSUPPORTED_DATA_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          GenZappDatabase.messages.markAsUnsupportedProtocolVersion(messageId)
        }
      }
  ...
  private fun handleMessage(
    ...
    localMetric: GenZappLocalMetrics.MessageReceive?
  ) {
    ...
    when {
      ...
      content.callMessage != null -> {
        ...
        if (message.destinationDeviceId != null && message.destinationDeviceId != GenZappStore.account.deviceId) {
          log(envelope.timestamp!!, "Ignoring call message that is not for this device! intended: ${message.destinationDeviceId}, this: ${GenZappStore.account.deviceId}")
          return
        }
  ...
  private fun handleTypingMessage(
    ...
    val threadId: Long = if (typingMessage.groupId != null) {
      ...
      if (!GenZappDatabase.groups.isCurrentMember(groupId, senderRecipient.id)) {
        warn(envelope.timestamp!!, "Seen typing indicator for non-member " + senderRecipient.id)
        return
      }

      val groupRecipient = Recipient.externalPossiblyMigratedGroup(groupId)
      GenZappDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
    } else {
      GenZappDatabase.threads.getOrCreateThreadIdFor(senderRecipient)
    }
  ...
  private fun handleRetryReceipt(envelope: Envelope, metadata: EnvelopeMetadata, decryptionErrorMessage: DecryptionErrorMessage, senderRecipient: Recipient) {
    ...
    if (decryptionErrorMessage.deviceId != GenZappStore.account.deviceId) {
    ...
    val messageLogEntry = GenZappDatabase.messageLog.getLogEntry(senderRecipient.id, metadata.sourceDeviceId, sentTimestamp)
  ...
  private fun handleSenderKeyRetryReceipt(
    ...
    val threadRecipient = GenZappDatabase.threads.getRecipientForThreadId(relatedMessage.threadId)
    ...
    if (threadRecipient.isGroup) {
      ...
      groupId = null
      distributionId = GenZappDatabase.distributionLists.getDistributionId(threadRecipient.id)
    }
    ...
    val requesterAddress = GenZappProtocolAddress(requester.requireServiceId().toString(), metadata.sourceDeviceId)
    GenZappDatabase.senderKeyShared.delete(distributionId, setOf(requesterAddress))
  ...
  private fun findRetryReceiptRelatedMessage(messageLogEntry: MessageLogEntry?, sentTimestamp: Long): MessageRecord? {
    return if (messageLogEntry != null && messageLogEntry.hasRelatedMessage) {
      ...
      GenZappDatabase.messages.getMessageRecordOrNull(id)
    } else {
      GenZappDatabase.messages.getMessageFor(sentTimestamp, Recipient.self().id)
    }
  }
