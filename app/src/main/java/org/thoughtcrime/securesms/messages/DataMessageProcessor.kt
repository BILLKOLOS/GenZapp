...
import org.GenZapp.core.util.toOptional
...
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isInvalid
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isMediaMessage
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isPaymentActivated
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isPaymentActivationRequest
import org.thoughtcrime.securesms.messages.GenZappServiceProtoUtil.isStoryReaction
...
import org.whispersystems.GenZappservice.api.payments.Money
...
import org.whispersystems.GenZappservice.internal.push.Preview
...
object DataMessageProcessor {
  ...
  fun process(
    ...
    localMetrics: GenZappLocalMetrics.MessageReceive?
  ) {
    ...
    if (groupId != null) {
      val unknownGroup = when (groupProcessResult) {
        ...
        else -> GenZappDatabase.groups.isUnknownGroup(groupId)
      }
      if (unknownGroup) {
    ...
    if (groupId == null && senderRecipient.hiddenState == HiddenState.HIDDEN) {
      GenZappDatabase.recipients.markHidden(senderRecipient.id, clearProfileKey = false, showMessageRequest = true)
    }

    if (metadata.sealedSender && messageId != null) {
      GenZappExecutors.BOUNDED.execute { AppDependencies.jobManager.add(SendDeliveryReceiptJob(senderRecipient.id, message.timestamp!!, messageId)) }
      ...
      if (RecipientUtil.shouldHaveProfileKey(threadRecipient)) {
        ...
        if (groupId != null) {
          ...
          val profileSendJob = ProfileKeySendJob.create(GenZappDatabase.threads.getOrCreateThreadIdFor(threadRecipient), true)
    ...
    if (insertResult != null && insertResult.threadWasNewlyCreated && !threadRecipient.isGroup && !threadRecipient.isSelf && !senderRecipient.isSystemContact) {
      val timeSinceLastSync = System.currentTimeMillis() - GenZappStore.misc.lastCdsForegroundSyncTime
      if (timeSinceLastSync > RemoteConfig.cdsForegroundSyncInterval || timeSinceLastSync < 0) {
        ...
        GenZappStore.misc.lastCdsForegroundSyncTime = System.currentTimeMillis()
      } else {
  ...
  private fun handleProfileKey(
    ...
    if (senderRecipient.isSelf) {
      ...
      if (GenZappDatabase.recipients.setProfileKey(senderRecipient.id, messageProfileKey)) {
  ...
  fun handleUnknownGroupMessage(timestamp: Long, groupContextV2: GroupContextV2) {
    ...
    GenZappDatabase.groups.fixMissingMasterKey(groupContextV2.groupMasterKey)
  }
  ...
  private fun handleInvalidMessage(
    ...
    if (insertResult != null) {
      GenZappDatabase.messages.markAsInvalidMessage(insertResult.messageId)
  ...
  private fun handleEndSessionMessage(
    ...
    val insertResult: InsertResult? = GenZappDatabase.messages.insertMessageInbox(incomingMessage).orNull()
  ...
  private fun handleExpirationUpdate(
    ...
    if (GenZappDatabase.recipients.getExpiresInSeconds(threadRecipientId) == expiresIn.inWholeSeconds) {
    ...
    try {
      ...
      val insertResult: InsertResult? = GenZappDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
      GenZappDatabase.recipients.setExpireMessages(threadRecipientId, expiresIn.inWholeSeconds.toInt())
  ...
  private fun handleStoryReaction(
    ...
    GenZappDatabase.messages.beginTransaction()
    return try {
      ...
      try {
        val storyId = GenZappDatabase.messages.getStoryId(authorRecipientId, sentTimestamp).id

        if (groupId != null) {
          parentStoryId = GroupReply(storyId)
        } else if (GenZappDatabase.storySends.canReply(senderRecipientId, sentTimestamp)) {
          val story = GenZappDatabase.messages.getMessageRecord(storyId) as MmsMessageRecord
          var displayText = ""
      ...
      val insertResult: InsertResult? = GenZappDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
      if (insertResult != null) {
        GenZappDatabase.messages.setTransactionSuccessful()
      ...
      GenZappDatabase.messages.endTransaction()
    }
  }
  ...
  fun handleReaction(
    ...
    val targetMessage = GenZappDatabase.messages.getMessageFor(targetSentTimestamp, targetAuthor.id)
    ...
    val targetThread = GenZappDatabase.threads.getThreadRecord(targetMessage.threadId)
    ...
    val groupRecord = GenZappDatabase.groups.getGroup(targetThreadRecipientId).orNull()
    ...
    if (isRemove) {
      GenZappDatabase.reactions.deleteReaction(targetMessageId, senderRecipientId)
      AppDependencies.messageNotifier.updateNotification(context)
    } else {
      val reactionRecord = ReactionRecord(emoji!!, senderRecipientId, message.timestamp!!, System.currentTimeMillis())
      GenZappDatabase.reactions.addReaction(targetMessageId, reactionRecord)
  ...
  fun handleRemoteDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipientId: RecipientId, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    val targetMessage: MessageRecord? = GenZappDatabase.messages.getMessageFor(targetSentTimestamp, senderRecipientId)
    ...
    return if (targetMessage != null && MessageConstraintsUtil.isValidRemoteDeleteReceive(targetMessage, senderRecipientId, envelope.serverTimestamp!!)) {
      GenZappDatabase.messages.markAsRemoteDelete(targetMessage)
      if (targetMessage.isStory()) {
        GenZappDatabase.messages.deleteRemotelyDeletedStory(targetMessage.id)
      }
  ...
  private fun handlePaymentActivation(
    ...
    try {
      ...
      return GenZappDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
    return null
  ...
  private fun handlePayment(
    ...
    try {
      GenZappDatabase.payments.createIncomingPayment(
        uuid,
        senderRecipientId,
        message.timestamp!!,
    ...
    return try {
      ...
      try {
        if (selfId == storyAuthorRecipientId) {
          storyMessageId = GenZappDatabase.storySends.getStoryMessageFor(senderRecipient.id, sentTimestamp)
        }

        if (storyMessageId == null) {
          storyMessageId = GenZappDatabase.messages.getStoryId(storyAuthorRecipientId, sentTimestamp)
        }

        val story: MmsMessageRecord = GenZappDatabase.messages.getMessageRecord(storyMessageId.id) as MmsMessageRecord
        var threadRecipient: Recipient = GenZappDatabase.threads.getRecipientForThreadId(story.threadId)!!
        val groupRecord: GroupRecord? = GenZappDatabase.groups.getGroup(threadRecipient.id).orNull()
        ...
        } else if (groupStory || GenZappDatabase.storySends.canReply(senderRecipient.id, sentTimestamp)) {
  ...
  private fun handleGiftMessage(
    ...
    val insertResult: InsertResult? = try {
      ...
      GenZappDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
  ...
  private fun handleMediaMessage(
    ...
    try {
      ...
      insertResult = GenZappDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
      if (insertResult != null) {
        GenZappDatabase.messages.setTransactionSuccessful()
      }
    } catch (e: MmsException) {
  ...
  private fun handleTextMessage(
    ...
    val insertResult: InsertResult? = GenZappDatabase.messages.insertMessageInbox(textMessage).orNull()
  ...
  fun handleGroupCallUpdateMessage(
    ...
    val groupRecipientId = GenZappDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId)
  ...
  fun notifyTypingStoppedFromIncomingMessage(context: Context, senderRecipient: Recipient, threadRecipientId: RecipientId, device: Int) {
    val threadId = GenZappDatabase.threads.getThreadIdIfExistsFor(threadRecipientId)
  ...
  private fun insertPlaceholder(sender: RecipientId, timestamp: Long, groupId: GroupId?): InsertResult? {
    ...
    return GenZappDatabase.messages.insertMessageInbox(textMessage).orNull()
  }
  ...
  fun getValidatedQuote(context: Context, timestamp: Long, message: DataMessage): QuoteModel? {
    ...
    var quotedMessage = GenZappDatabase.messages.getMessageFor(quote.id!!, authorId) as? MmsMessageRecord
    ...
    if (quotedMessage != null && !quotedMessage.isRemoteDelete) {
      ...
      quotedMessage = quotedMessage.withAttachments(GenZappDatabase.attachments.getAttachmentsForMessage(quotedMessage.id))

      mentions.addAll(GenZappDatabase.mentions.getMentionsForMessage(quotedMessage.id))
      ...
      if (quotedMessage.isPaymentNotification) {
        quotedMessage = GenZappDatabase.payments.updateMessageWithPayment(quotedMessage) as MmsMessageRecord
      }
  ...
  fun getStickerAttachment(timestamp: Long, message: DataMessage): Attachment? {
    ...
    val stickerRecord: StickerRecord? = GenZappDatabase.stickers.getSticker(stickerLocator.packId, stickerLocator.stickerId, false)
