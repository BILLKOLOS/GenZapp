...
class ImageEditorHudV2 @JvmOverloads constructor(
  ...
  private fun setupWidthSeekBar() {
    ...
    widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
      override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        when (currentMode) {
          Mode.DRAW -> GenZappStore.imageEditor.setMarkerPercentage(progress)
          Mode.BLUR -> GenZappStore.imageEditor.setBlurPercentage(progress)
          Mode.HIGHLIGHT -> GenZappStore.imageEditor.setHighlighterPercentage(progress)
          else -> Unit
        }
      }
  ...
  private fun presentModeDraw() {
    ...
    widthSeekBar.progress = GenZappStore.imageEditor.getMarkerPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  private fun presentModeHighlight() {
    ...
    widthSeekBar.progress = GenZappStore.imageEditor.getHighlighterPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
      inSet = drawButtonRow + drawTools,
  ...
  private fun presentModeBlur() {
    ...
    widthSeekBar.progress = GenZappStore.imageEditor.getBlurPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  companion object {
    ...
    private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
      Mode.DRAW to GenZappStore.imageEditor.getMarkerWidthRange(),
      Mode.HIGHLIGHT to GenZappStore.imageEditor.getHighlighterWidthRange(),
      Mode.BLUR to GenZappStore.imageEditor.getBlurWidthRange()
    )
