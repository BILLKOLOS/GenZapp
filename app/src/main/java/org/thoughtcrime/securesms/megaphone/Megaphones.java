...
import org.GenZapp.core.util.MapUtil;
import org.GenZapp.core.util.SetUtil;
import org.GenZapp.core.util.TranslationDetection;
...
import org.thoughtcrime.securesms.lock.GenZappPinReminderDialog;
import org.thoughtcrime.securesms.lock.GenZappPinReminders;
...
public final class Megaphones {
  ...
  private static Map<Event, MegaphoneSchedule> buildDisplayOrder(@NonNull Context context, @NonNull Map<Event, MegaphoneRecord> records) {
    return new LinkedHashMap<>() {{
      ...
      put(Event.CLIENT_DEPRECATED, GenZappStore.misc().isClientDeprecated() ? ALWAYS : NEVER);
      ...
      put(Event.PIN_REMINDER, new GenZappPinReminderSchedule());
  ...
  private static boolean shouldShowLinkedDeviceInactiveMegaphone() {
    LeastActiveLinkedDevice device = GenZappStore.misc().getLeastActiveLinkedDevice();
    if (device == null) {
      return false;
    }
  ...
  private static @NonNull Megaphone buildPinsForAllMegaphone(@NonNull MegaphoneRecord record) {
    if (PinsForAllSchedule.shouldDisplayFullScreen(record.getFirstVisible(), System.currentTimeMillis())) {
      ...
      return new Megaphone.Builder(Event.PINS_FOR_ALL, Megaphone.Style.BASIC)
          ...
          .setBody(R.string.KbsMegaphone__pins_keep_information_thats_stored_with_GenZapp_encrytped)
  ...
  private static @NonNull Megaphone buildPinReminderMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.PIN_REMINDER, Megaphone.Style.BASIC)
        .setTitle(R.string.Megaphones_verify_your_GenZapp_pin)
        ...
        .setActionButton(R.string.Megaphones_verify_pin, (megaphone, controller) -> {
          GenZappPinReminderDialog.show(controller.getMegaphoneActivity(), controller::onMegaphoneNavigationRequested, new GenZappPinReminderDialog.Callback() {
            @Override
            public void onReminderDismissed(boolean includedFailure) {
              Log.i(TAG, "[PinReminder] onReminderDismissed(" + includedFailure + ")");
              if (includedFailure) {
                GenZappStore.pin().onEntrySkipWithWrongGuess();
              }
            }
            ...
            public void onReminderCompleted(@NonNull String pin, boolean includedFailure) {
              ...
              if (includedFailure) {
                GenZappStore.pin().onEntrySuccessWithWrongGuess(pin);
              } else {
                GenZappStore.pin().onEntrySuccess(pin);
              }

              controller.onMegaphoneSnooze(Event.PIN_REMINDER);
              controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(GenZappPinReminders.getReminderString(GenZappStore.pin().getCurrentInterval())));
            }
          });
        })
        .build();
  ...
  private static @NonNull Megaphone buildTurnOffCircumventionMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.TURN_OFF_CENSORSHIP_CIRCUMVENTION, Megaphone.Style.BASIC)
        ...
        .setBody(R.string.CensorshipCircumventionMegaphone_you_can_now_connect_to_the_GenZapp_service)
        .setActionButton(R.string.CensorshipCircumventionMegaphone_turn_off, (megaphone, listener) -> {
          GenZappStore.settings().setCensorshipCircumventionEnabled(false);
  ...
  private static boolean shouldShowOnboardingMegaphone(@NonNull Context context) {
    return GenZappStore.onboarding().hasOnboarding(context);
  }

  private static boolean shouldShowTurnOffCircumventionMegaphone() {
    return AppDependencies.getGenZappServiceNetworkAccess().isCensored() &&
           GenZappStore.misc().isServiceReachableWithoutCircumvention();
  }

  private static boolean shouldShowNotificationsMegaphone(@NonNull Context context) {
    boolean shouldShow = !GenZappStore.settings().isMessageNotificationsEnabled() ||
  ...
  private static boolean shouldShowAddAProfilePhotoMegaphone(@NonNull Context context) {
    if (GenZappStore.misc().getHasEverHadAnAvatar()) {
      return false;
    }

    boolean hasAnAvatar = AvatarHelper.hasAvatar(context, Recipient.self().getId());
    if (hasAnAvatar) {
      GenZappStore.misc().setHasEverHadAnAvatar(true);
      return false;
    }
  ...
  private static boolean shouldShowSetUpYourUsernameMegaphone(@NonNull Map<Event, MegaphoneRecord> records) {
    boolean                        hasUsername                    = GenZappStore.account().isRegistered() && GenZappStore.account().getUsername() != null;
    boolean                        hasCompleted                   = MapUtil.mapOrDefault(records, Event.SET_UP_YOUR_USERNAME, MegaphoneRecord::isFinished, false);
    long                           phoneNumberDiscoveryDisabledAt = GenZappStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityModeTimestamp();
    PhoneNumberDiscoverabilityMode listingMode                    = GenZappStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode();
  ...
  private static boolean shouldShowPnpLaunchMegaphone() {
    return TextUtils.isEmpty(GenZappStore.account().getUsername()) && !GenZappStore.uiHints().hasCompletedUsernameOnboarding();
  }
  ...
  private static boolean shouldShowBackupSchedulePermissionMegaphone(@NonNull Context context) {
    return Build.VERSION.SDK_INT >= 31 && GenZappStore.settings().isBackupEnabled() && !ServiceUtil.getAlarmManager(context).canScheduleExactAlarms();
  }
