...
object JumboEmoji {
  ...
  private val executor = GenZappExecutors.newCachedSingleThreadExecutor("jumbo-emoji", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  ...
  fun updateCurrentVersion(context: Context) {
    GenZappExecutors.BOUNDED.execute {
      ...
      if (EmojiFiles.getLatestEmojiData(context, version)?.format != null) {
        ...
        ThreadUtil.runOnMain { downloadedJumbos.addAll(GenZappStore.emoji.getJumboEmojiSheets(version.version)) }
      }
    }
  }
  ...
  fun loadJumboEmoji(context: Context, drawInfo: EmojiDrawInfo): LoadResult {
    ...
    val newTask = ListenableFutureTask<Bitmap> {
      ...
      if (uuid == null) {
        ...
        GenZappStore.emoji.addJumboEmojiSheet(version.version, drawInfo.jumboSheet)
      }
