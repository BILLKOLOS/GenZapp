...
class ContactSearchSelectionBuilderTest {
  ...
  @Test
  fun `Given registered, when I build, then I expect GenZapp_CONTACT`() {
    val result = RecipientTable.ContactSearchSelection.Builder()
      .withRegistered(true)
      .build()

    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.GenZapp_CONTACT))
  ...
  fun `Given all non group contacts, when I build, then I expect both CONTACT and FILTER_GROUP`() {
    ...
    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.NON_GenZapp_CONTACT))
  ...
  @Test
  fun `Given a query, when I build, then I expect QUERY_GenZapp_CONTACT`() {
    val result = RecipientTable.ContactSearchSelection.Builder()
      .withRegistered(true)
      .withGroups(false)
    ...
    Assert.assertTrue(result.where.contains(RecipientTable.ContactSearchSelection.QUERY_GenZapp_CONTACT))
