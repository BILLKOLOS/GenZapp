...
import org.GenZapp.core.util.Hex.fromStringCondensed
...
import org.GenZapp.storageservice.protos.groups.local.DecryptedString
import org.GenZapp.storageservice.protos.groups.local.DecryptedTimer
...
class GroupsV2StateProcessorTest {
  ...
  fun setUp() {
    ...
    every { AppDependencies.GenZappServiceAccountManager.getGroupsV2Api() } returns groupsV2API
    ...
    mockkObject(GenZappDatabase)
    every { GenZappDatabase.groups } returns groupTable
    every { GenZappDatabase.recipients } returns recipientTable
    every { GenZappDatabase.threads } returns threadTable
  ...
  fun updateFromServerIfUnableToApplyP2PChange() {
    given {
      ...
      serverState(
        revision = 2,
        title = "Breaking GenZapp for Science",
        members = selfAndOthers
      )
      changeSet {
        changeLog(2) {
    ...
    val signedChange = DecryptedGroupChange(
      revision = 2,
      newTitle = DecryptedString("Breaking GenZapp for Science")
    )
  ...
  fun `when freshly added to a group, with additional group changes after being added, then only update from server at the revision we were added, and then schedule pulling additional changes later`() {
    given {
      changeSet {
        changeLog(2) {
          fullSnapshot(
            title = "Baking GenZapp for Science",
            description = "We break stuff, because we must.",
            members = listOf(member(otherAci), member(selfAci, joinedAt = 2))
          )
        }
        changeLog(3) {
          change {
            setNewTitle("Breaking GenZapp for Science")
          }
        }
      }
      apiCallParameters(2, true)
    ...
    assertThat("title matches that as it was in revision added", result.latestServer!!.title, `is`("Baking GenZapp for Science"))
