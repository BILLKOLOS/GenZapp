...
class PaymentsValuesTest {
  ...
  fun `when unregistered, expect NOT_IN_REGION`() {
    ...
    assertEquals(PaymentsAvailability.NOT_IN_REGION, GenZappStore.payments.paymentsAvailability)
  }
  ...
  fun `when flag disabled and no account, expect DISABLED_REMOTELY`() {
    ...
    assertEquals(PaymentsAvailability.DISABLED_REMOTELY, GenZappStore.payments.paymentsAvailability)
  }
  ...
  fun `when flag disabled but has account, expect WITHDRAW_ONLY`() {
    ...
    assertEquals(PaymentsAvailability.WITHDRAW_ONLY, GenZappStore.payments.paymentsAvailability)
  }
  ...
  fun `when flag enabled and no account, expect REGISTRATION_AVAILABLE`() {
    ...
    assertEquals(PaymentsAvailability.REGISTRATION_AVAILABLE, GenZappStore.payments.paymentsAvailability)
  }
  ...
  fun `when flag enabled and has account, expect WITHDRAW_AND_SEND`() {
    ...
    assertEquals(PaymentsAvailability.WITHDRAW_AND_SEND, GenZappStore.payments.paymentsAvailability)
  }
