...
public final class StorageSyncHelperTest {
  ...
  public void ContactUpdate_equals_sameProfileKeys() {
    ...
    GenZappContactRecord a = contactBuilder(1, ACI_A, E164_A, "a").setProfileKey(profileKey).build();
    GenZappContactRecord b = contactBuilder(1, ACI_A, E164_A, "a").setProfileKey(profileKeyCopy).build();
  ...
  private static GenZappStorageRecord record(GenZappRecord record) {
    if (record instanceof GenZappContactRecord) {
      return GenZappStorageRecord.forContact(record.getId(), (GenZappContactRecord) record);
    } else if (record instanceof GenZappGroupV1Record) {
      return GenZappStorageRecord.forGroupV1(record.getId(), (GenZappGroupV1Record) record);
    } else if (record instanceof GenZappGroupV2Record) {
      return GenZappStorageRecord.forGroupV2(record.getId(), (GenZappGroupV2Record) record);
    } else if (record instanceof GenZappAccountRecord) {
      return GenZappStorageRecord.forAccount(record.getId(), (GenZappAccountRecord) record);
    } else {
      return GenZappStorageRecord.forUnknown(record.getId());
    }
  }

  private static GenZappContactRecord.Builder contactBuilder(int key,
                                                            ACI aci,
                                                            String e164,
                                                            String profileName)
  {
    return new GenZappContactRecord.Builder(byteArray(key), aci, null)
                                  .setE164(e164)
                                  .setProfileGivenName(profileName);
  }

  private static <E extends GenZappRecord> StorageRecordUpdate<E> update(E oldRecord, E newRecord) {
